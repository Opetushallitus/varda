...
def update_lokalisointi_data():
    ...
    for category_key, category in LOKALISOINTIPALVELU_DICT.items():
        ...
        for lokalisointi in lokalisointi_result_raw:
            update_datetime = max(update_datetime, lokalisointi['modified'])
            lokalisointi_result[lokalisointi['key']] = lokalisointi_result.get(lokalisointi['key'], [])
            lokalisointi_result[lokalisointi['key']].append({'locale': lokalisointi['locale'],
                                                             'value': lokalisointi['value']})

        koodisto_obj, created = Z2_Koodisto.objects.update_or_create(name=category_key.value, defaults={
            'name_koodistopalvelu': category, 'version': 1,
            # fromtimestamp takes seconds, koodistopalvelu has milliseconds
            'update_datetime': datetime.datetime.fromtimestamp(update_datetime / 1000, datetime.timezone.utc)
        })
        ...
        for lokalisointi_key, lokalisointi_value in lokalisointi_result.items():
            ...
            for translation in lokalisointi_value:
                ...
                Z2_CodeTranslation.objects.update_or_create(
                    code=code_obj, language=translation['locale'],
                    defaults={'name': translation['value'], 'description': ''}
                )
...
def get_translation(translation_key, locale=SupportedLanguage.FI.value, category=Lokalisointi.VIRKAILIJA.value):
    ...
    translation_qs = Z2_CodeTranslation.objects.filter(code__code_value__contains=translation_key,
                                                       language__iexact=locale, code__koodisto__name=category)
