...
from varda.cache import get_koodistot_cache, get_pulssi_cache, get_queryset_count_cache_key, set_koodistot_cache, set_pulssi_cache
...
from varda.filters import CustomParameter, CustomParametersFilterBackend
from varda.lokalisointipalvelu import get_localisation_data
from varda.misc import send_slack_message
from varda.misc_queries import get_active_filter, get_queryset_count
...
from varda.serializers_julkinen import KoodistotSerializer, LocalisationSerializer, PulssiSerializer
...
class KoodistotViewSet(GenericViewSet, ListModelMixin):
    ...
    custom_parameters = (
        CustomParameter(name="lang", required=False, location="query", data_type="string", description="Locale code (fi/sv)"),
    )
    ...
    def list(self, request, *args, **kwargs):
        ...
        if language.upper() not in SupportedLanguage.list():
        ...
        if not koodistot_list:
            ...
            active_annotation = {
                "active": Case(
                    When(Q(alkamis_pvm__gt=today) | Q(paattymis_pvm__lt=today), then=False),
                    default=True,
                    output_field=BooleanField(),
                )
            }
            translation_qs = codes_with_translation_qs.annotate(
                name=F("translations__name"), description=F("translations__description"), **active_annotation
            ).values("code_value", "name", "description", "active", "alkamis_pvm", "paattymis_pvm")
            placeholder_qs = codes_without_translation_qs.annotate(
                name=Value("", CharField()), description=Value("", CharField()), **active_annotation
            ).values("code_value", "name", "description", "active", "alkamis_pvm", "paattymis_pvm")
...
class LocalisationViewSet(GenericViewSet, ListModelMixin):
    ...
    custom_parameters = (
        ...
        CustomParameter(name="locale", required=False, location="query", data_type="string", description="Locale code (fi/sv)"),
    )
...
class PulssiViewSet(GenericViewSet, ListModelMixin):
    ...
    def get_data(self):
        ...
        yksityinen_count = Organisaatio.vakajarjestajat.filter(active_filter).exclude(yritysmuoto__in=YRITYSMUOTO_KUNTA).count()
        ...
        tm_codes = (
            Z2_Code.objects.filter(koodisto__name=Koodistot.toimintamuoto_koodit.value)
        ...
        toimipaikka_by_tm = (
            Toimipaikka.objects.filter(active_filter)
            # Avoid Django's default group by id with 'temp' field
            .annotate(temp=Value(0))
            .values("temp")
            .annotate(**tm_annotations)
            .values(*tm_codes)
        )[0]

        # Toimipaikka count by jarjestamismuoto_koodi
        jm_codes = (
            Z2_Code.objects.filter(koodisto__name=Koodistot.jarjestamismuoto_koodit.value)
            .annotate(code_lower=Lower("code_value"))
            .values_list("code_lower", flat=True)
            .order_by("code_lower")
        )
        ...
        toimipaikka_by_jm = (
            ...
            .annotate(temp=Value(0))
            .values("temp")
            .annotate(**jm_annotations)
            .values(*jm_codes)
        )[0]

        # Toimipaikka count by kasvatusopillinen_jarjestelma_koodi
        kj_codes = (
            Z2_Code.objects.filter(koodisto__name=Koodistot.kasvatusopillinen_jarjestelma_koodit.value)
            .annotate(code_lower=Lower("code_value"))
            .values_list("code_lower", flat=True)
            .order_by("code_lower")
        )
        ...
        for kj_code in kj_codes:
            kj_annotations[kj_code] = Sum(Case(When(kasvatusopillinen_jarjestelma_koodi__iexact=kj_code, then=1), default=0))
        toimipaikka_by_kj = (
            Toimipaikka.objects.filter(active_filter)
            # Avoid Django's default group by id with 'temp' field
            .annotate(temp=Value(0))
            .values("temp")
            .annotate(**kj_annotations)
            .values(*kj_codes)
        )[0]

        # Toimipaikka count by toimintakieli_koodi
        # PostgreSQL specific functionality (unnest)
        toimipaikka_by_ak = {
            row["toimintakieli_lower"]: row["count"]
            for row in Toimipaikka.objects.filter(active_filter)
            .annotate(toimintakieli_lower=Lower(Func(F("toimintakieli_koodi"), function="unnest")))
            .values("toimintakieli_lower")
            .annotate(count=Count("id"))
            .order_by("toimintakieli_lower")
        }
        # If toimintakieli is not one of predefined, count it to others category
        ...
        lapsi_active_filter = get_active_filter(today, prefix="varhaiskasvatuspaatokset") & get_active_filter(
            today, prefix="varhaiskasvatuspaatokset__varhaiskasvatussuhteet"
        )
        lapsi_count = Lapsi.objects.filter(lapsi_active_filter).distinct("henkilo_id").count()
        lapsi_kunta_count = (
            Lapsi.objects.filter(
                lapsi_active_filter
                & (Q(vakatoimija__yritysmuoto__in=YRITYSMUOTO_KUNTA) | Q(oma_organisaatio__yritysmuoto__in=YRITYSMUOTO_KUNTA))
            )
            .distinct("henkilo_id")
            .count()
        )
        lapsi_yksityinen_count = (
            Lapsi.objects.filter(
                lapsi_active_filter
                & ~(Q(vakatoimija__yritysmuoto__in=YRITYSMUOTO_KUNTA) | Q(oma_organisaatio__yritysmuoto__in=YRITYSMUOTO_KUNTA))
            )
        ...
        paivittainen_count = (
            Lapsi.objects.filter(lapsi_active_filter, varhaiskasvatuspaatokset__paivittainen_vaka_kytkin=True)
            .distinct("henkilo_id")
            .count()
        )
        kokopaivainen_count = (
            Lapsi.objects.filter(lapsi_active_filter, varhaiskasvatuspaatokset__kokopaivainen_vaka_kytkin=True)
            .distinct("henkilo_id")
            .count()
        )
        vuorohoito_count = (
            Lapsi.objects.filter(lapsi_active_filter, varhaiskasvatuspaatokset__vuorohoito_kytkin=True)
            .distinct("henkilo_id")
            .count()
        )
        ...
        for jm_code in jm_codes:
            jm_lapsi_annotations[jm_code] = Count(
                Case(When(varhaiskasvatuspaatokset__jarjestamismuoto_koodi__iexact=jm_code, then=F("henkilo_id"))), distinct=True
            )
        lapsi_by_jm = (
            Lapsi.objects.filter(lapsi_active_filter)
            .annotate(temp=Value(0))
            .values("temp")
            .annotate(**jm_lapsi_annotations)
            .values(*jm_codes)
        )[0]
        ...
        huoltaja_lapsi_active_filter = get_active_filter(today, prefix=huoltaja_vakapaatos_prefix) & get_active_filter(
            today, prefix=huoltaja_vakasuhde_prefix
        )
        huoltaja_count = (
            Huoltaja.objects.filter(huoltaja_lapsi_active_filter, huoltajuussuhteet__voimassa_kytkin=True)
            .distinct("henkilo_id")
            .count()
        )
        ...
        tn_codes = (
            Z2_Code.objects.filter(koodisto__name=Koodistot.tehtavanimike_koodit.value)
            .annotate(code_lower=Lower("code_value"))
            .values_list("code_lower", flat=True)
            .order_by("code_lower")
        )
        ...
        tyoskentelypaikka_active_filter = (
            active_filter
            & get_active_filter(today, prefix="palvelussuhde")
        ...
        distinct_tp_subquery = (
            Tyoskentelypaikka.objects.filter(tyoskentelypaikka_active_filter)
            .distinct("palvelussuhde__tyontekija__henkilo_id", "tehtavanimike_koodi")
            .values("id")
        )
        tyoskentelypaikka_by_tn = (
            Tyoskentelypaikka.objects.filter(id__in=distinct_tp_subquery)
            # Avoid Django's default group by id with 'temp' field
            .annotate(temp=Value(0))
            .values("temp")
            .annotate(**tn_annotations)
            .values(*tn_codes)
        )[0]

        # Tyontekija count with Tyoskentelypaikka in more than 1 Toimipaikka
        tyontekija_multi_count = (
            Tyoskentelypaikka.objects.filter(tyoskentelypaikka_active_filter)
            .values("palvelussuhde__tyontekija__henkilo_id")
            .annotate(
                toimipaikka_count=Count("toimipaikka_id", distinct=True),
                kiertava_count=Sum(Case(When(kiertava_tyontekija_kytkin=True, then=1), default=0)),
            )
        ...
        distinct_tt_subquery = TaydennyskoulutusTyontekija.objects.distinct(
            "tyontekija__henkilo_id", "taydennyskoulutus_id"
        ).values("id")
        koulutuspaiva_count = TaydennyskoulutusTyontekija.objects.filter(
            id__in=distinct_tt_subquery, taydennyskoulutus__suoritus_pvm__year=today.year
        ).aggregate(sum=Coalesce(Sum("taydennyskoulutus__koulutuspaivia"), 0.0, output_field=DecimalField()))

        taydennyskoulutus_count = Taydennyskoulutus.objects.filter(suoritus_pvm__year=today.year).count()

        # VuokrattuHenkilosto tyontekijamaara and tuntimaara sum
        vuokrattu_henkilosto_aggregate = VuokrattuHenkilosto.objects.filter(kuukausi__year=today.year).aggregate(
            tyontekijamaara=Coalesce(Sum("tyontekijamaara"), 0),
            tuntimaara=Coalesce(Sum("tuntimaara"), 0.0, output_field=DecimalField()),
        )

        # Varda UI related data
        user_count = (
            User.objects.annotate(kayttajatyyppi=F("additional_cas_user_fields__kayttajatyyppi"))
        ...
        valtuudet_login_count = 0  # Not supported anymore (since May 2026)

        # Number of created Varhaiskasvatuspaatos objects with lahdejarjestelma = '1'
        month_ago = today - datetime.timedelta(days=30)
        ui_new_paatos_count = Varhaiskasvatuspaatos.history.filter(
            history_type="+", history_date__gte=month_ago, lahdejarjestelma="1"
        ).count()
        ui_new_tyontekija_count = Tyontekija.history.filter(
            history_type="+", history_date__gte=month_ago, lahdejarjestelma="1"
        ).count()
        ui_new_maksutieto_count = Maksutieto.history.filter(
            history_type="+", history_date__gte=month_ago, lahdejarjestelma="1"
        ).count()

        data = {
            "organisaatio_count": organisaatio_count,
            "kunta_count": kunta_count,
            "yksityinen_count": yksityinen_count,
            "toimipaikka_count": toimipaikka_count,
            "toimipaikka_by_tm": toimipaikka_by_tm,
            "toimipaikka_by_jm": toimipaikka_by_jm,
            "toimipaikka_by_kj": toimipaikka_by_kj,
            "toimipaikka_by_ak": toimipaikka_by_ak_filtered,
            "toimipaikka_with_kp": toimipaikka_with_kp,
            "toimipaikka_with_tp": toimipaikka_with_tp,
            "lapsi_count": lapsi_count,
            "lapsi_kunta_count": lapsi_kunta_count,
            "lapsi_yksityinen_count": lapsi_yksityinen_count,
            "vakapaatos_count": vakapaatos_count,
            "paivittainen_count": paivittainen_count,
            "kokopaivainen_count": kokopaivainen_count,
            "vuorohoito_count": vuorohoito_count,
            "lapsi_by_jm": lapsi_by_jm,
            "huoltaja_count": huoltaja_count,
            "asiakasmaksu_avg": asiakasmaksu_avg["avg"],
            "tyontekija_count": tyontekija_count,
            "tyoskentelypaikka_by_tn": tyoskentelypaikka_by_tn,
            "tyontekija_multi_count": tyontekija_multi_count,
            "taydennyskoulutus_count": taydennyskoulutus_count,
            "koulutuspaiva_count": koulutuspaiva_count["sum"],
            "vuokrattu_henkilosto_tyontekijamaara": vuokrattu_henkilosto_aggregate["tyontekijamaara"],
            "vuokrattu_henkilosto_tuntimaara": vuokrattu_henkilosto_aggregate["tuntimaara"],
            "ui_login_count": ui_login_count,
            "oppija_login_count": oppija_login_count,
            "valtuudet_login_count": valtuudet_login_count,
            "ui_new_paatos_count": ui_new_paatos_count,
            "ui_new_tyontekija_count": ui_new_tyontekija_count,
            "ui_new_maksutieto_count": ui_new_maksutieto_count,
            "timestamp": timezone.now(),
        }
        set_pulssi_cache(data)
        return data

    def raise_alert_in_slack(self, data):
        if not settings.PRODUCTION_ENV:
            return None

        text = "❗Varoitus: Lapsi- tai työntekijämäärä on pudonnut. https://opintopolku.fi/varda/pulssi/"

        if data["lapsi_count"] < settings.LAPSI_COUNT_ALERT_THRESHOLD:
            logger.info(f"Sending a Slack-message to Varda-channel: {text}")
            send_slack_message(text)

        elif data["tyontekija_count"] < settings.TYONTEKIJA_COUNT_ALERT_THRESHOLD:
            logger.info(f"Sending a Slack-message to Varda-channel: {text}")
            send_slack_message(text)
    ...
    def list(self, request, *args, **kwargs):
        ...
        force_refresh = not settings.PRODUCTION_ENV and parse_query_parameter(request.query_params, "refresh", bool) is True
        data = get_pulssi_cache()
        if force_refresh or not data:
            data = self.get_data()
            self.raise_alert_in_slack(data)
