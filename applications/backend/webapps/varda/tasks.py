...
from django.db.models import (
    Case, Count, DateField, F, Func, Max, Q, Value, When, OuterRef, Exists)
...
from varda.misc import (
    encrypt_string, hash_string, list_to_chunks, memory_efficient_queryset_iterator,
...
from varda.models import (
    Aikaleima, BatchError, Henkilo, Huoltaja, Lapsi, Toimipaikka, Tutkinto, Organisaatio, ToiminnallinenPainotus,
    KieliPainotus, Palvelussuhde, Varhaiskasvatussuhde, Varhaiskasvatuspaatos, Tukipaatos, TukipaatosAikavali,
    Z2_Code, Z2_CodeTranslation, Z2_Koodisto, Z4_CasKayttoOikeudet, Z5_AuditLog, Z6_LastRequest, Z6_RequestCount,
...
from varda.viestintapalvelu import (send_no_paakayttaja_message, send_no_transfers_message,
                                    send_puutteelliset_tiedot_message, update_message_targets_and_paakayttaja_status)
...
def update_last_request_table_task(init=False):
    ...
    successful_options = [(Q(response_code__in=SUCCESSFUL_STATUS_CODE_LIST), True),
    ...
    for response_code_filter, is_successful in successful_options:
        request_log_qs = (Z6_RequestLog.objects
                          .filter(timestamp_filter & response_code_filter)
        ...
        for request_log in request_log_qs:
            last_request_defaults = ({'last_successful': request_log['timestamp']} if is_successful else
                                     {'last_unsuccessful': request_log['timestamp']})
            Z6_LastRequest.objects.update_or_create(
                user_id=request_log['user'], vakajarjestaja_id=request_log['vakajarjestaja'],
                lahdejarjestelma=request_log['lahdejarjestelma'], data_category=request_log['data_category'],
...
def get_ukranian_child_statistics(active_since='2022-02-24', swedish=False, has_kotikunta=False):
    ...
    toimintakieli_filter = (Q(lapsi__varhaiskasvatuspaatokset__varhaiskasvatussuhteet__toimipaikka__toimintakieli_koodi__icontains='sv')
                            if swedish else Q())
    kotikunta_filter = (Q(kotikunta_koodi__isnull=False) & ~Q(kotikunta_koodi='') &
                        ~Q(kotikunta_koodi__in=['198', '199', '200', '999'])
                        if has_kotikunta else Q())
    ...
    oid_list = (Henkilo.objects
                .filter(Q(lapsi__varhaiskasvatuspaatokset__alkamis_pvm__gte=active_since) &
                        ...
                        toimintakieli_filter & kotikunta_filter)
...
def init_last_request_data_category():
    ...
    TEMPORARY TASK
    """
    last_request_qs = (Z6_LastRequest.objects
                       .values('vakajarjestaja')
    ...
    for last_request in last_request_qs:
        for data_category in [DataCategory.VARHAISKASVATUS.value, DataCategory.HENKILOSTO.value]:
            Z6_LastRequest.objects.update_or_create(
                vakajarjestaja_id=last_request['vakajarjestaja'], user_id=last_request['user_instance_id'],
                data_category=data_category,
                defaults={'last_successful': last_request['last_successful_max'],
...
def set_all_vaka_avustajat_kelpoisuus_true_to_false():
    ...
    update_count = tyoskentelypaikat.update(
        kelpoisuus_kytkin=False,
        muutos_pvm=timezone.now())
...
def set_toimipaikkas_inactive_painotus_kytkin_to_false():
    ...
    toiminnallinenpainotus_toimipaikka_ids = ToiminnallinenPainotus.objects.values_list(
        'toimipaikka__id', flat=True).distinct()
    update_count_toiminnallinenpainotus = Toimipaikka.objects.filter(toiminnallinenpainotus_kytkin=True) \
    ...
    update_count_kielipainotus = Toimipaikka.objects.filter(kielipainotus_kytkin=True) \
    ...
    if update_count_toiminnallinenpainotus or update_count_kielipainotus:
        logger.info(f'Toimipaikka toiminnallinenpainotus_kytkin updated (count={update_count_toiminnallinenpainotus}). '
...
def transfer_toimipaikat_to_another_vakajarjestaja():
    ...
    new_vakajarjestaja = dict(
        y_tunnus='0201528-1', organisaatio_oid='1.2.246.562.10.64976109716')

    old_vakajarjestajas = [
        dict(y_tunnus='0217055-6', organisaatio_oid='1.2.246.562.10.41443287739'),
        dict(y_tunnus='0213679-6', organisaatio_oid='1.2.246.562.10.97302080193'),
    ]
...
def add_missing_vakasuhde_vakapaatos_permissions():
    ...
    vakasuhde_organisaatiot = Varhaiskasvatussuhde.objects.values_list(
        'toimipaikka__organisaatio_oid', flat=True).distinct()
    for org_oid in list(vakasuhde_organisaatiot):
        ...
        no_permission_vakasuhde_objs = Varhaiskasvatussuhde.objects.filter(toimipaikka__organisaatio_oid=org_oid) \
            .exclude(id__in=current_permission_vakasuhde_objs)
        ...
        for vakasuhde in no_permission_vakasuhde_objs:
            ...
            assign_lapsi_permissions(
                vakasuhde.varhaiskasvatuspaatos.lapsi,
    ...
    logger.info('Missing Vakasuhde/-paatos permissions checked and added. '
...
def remove_inactive_tutkintos(timeout_minutes=5):
    ...
    henkilo_ids_have_palvelussuhde = set(
        Palvelussuhde.objects.values_list('tyontekija__henkilo_id', flat=True).distinct())
    ...
    for tutkinto_koodi in list(tutkinto_koodis):
        ...
        palvelussuhde_objs_ids = Palvelussuhde.objects.filter(tutkinto_koodi=tutkinto_koodi).order_by('id') \
        ...
        for i in range(0, len(palvelussuhde_objs_ids), BATCH_SIZE):
            ...
            palvelussuhde_objs = Palvelussuhde.objects.filter(id__in=batch_ids) \
                .select_related('tyontekija').only('tyontekija__henkilo_id', 'tutkinto_koodi')
            active_henkilo_tutkintos |= {f'{obj.tyontekija.henkilo_id}-{obj.tutkinto_koodi}'
                                         for obj in palvelussuhde_objs}

        tutkinto_objs = Tutkinto.objects.filter(
            tutkinto_koodi=tutkinto_koodi,
            luonti_pvm__lt=datetime_90d_ago).select_related('henkilo')
        tutkinto_ids_to_delete = {
            obj.id for obj in tutkinto_objs if (
                f'{obj.henkilo.id}-{obj.tutkinto_koodi}' not in active_henkilo_tutkintos and
                obj.henkilo.id in henkilo_ids_have_palvelussuhde)}
        ...
        for i in range(0, len(tutkinto_ids_to_delete), BATCH_SIZE):
            ...
            logger.info('Inactive Tutkinto objects batch deleted. '
                        f'tutkinto_koodi={tutkinto_koodi} count={deleted_count}')
...
def add_missing_tukipaatos_paatosmaaras():
    ...
    TUENTASO_CODES = list(
        ...
        .order_by('code_value').values_list('code_value', flat=True))
...
def generate_active_lapsi_excel_by_date_task(date_str: str = None, password: str = None) -> None:
    ...
    excel_report = Z8_ExcelReport.objects.create(user=user, password=encrypt_string(password), status=ReportStatus.PENDING.value,
                                                 language=language, target_date=date, admin_user_all_organizations=True,
...
# Temporary task (can be removed after data removal in production)
@custom_shared_task(single_instance=True)
def remove_taydennyskoulutus_data_2025_01_01_and_later_task():
    from datetime import date
    from varda.models import Taydennyskoulutus, TaydennyskoulutusTyontekija

    cutoff_date = date(2025, 1, 1)

    taydennyskoulutustyontekija_objs_to_delete = TaydennyskoulutusTyontekija.objects.filter(
        taydennyskoulutus__suoritus_pvm__gte=cutoff_date)
    taydennyskoulutustyontekija_deleted_count, _ = taydennyskoulutustyontekija_objs_to_delete.delete()

    taydennyskoulutus_objs_to_delete = Taydennyskoulutus.objects.filter(suoritus_pvm__gte=cutoff_date)
    taydennyskoulutus_deleted_count, _ = taydennyskoulutus_objs_to_delete.delete()

    logger.info(
        f'TaydennyskoulutusTyontekija objects deleted. Count: {taydennyskoulutustyontekija_deleted_count}. '
        f'Taydennyskoulutus objects deleted. Count: {taydennyskoulutus_deleted_count}')
...
def remove_tutkintos_003_if_multiple_tutkinto_koodi():
    ...
    other_tutkinto_exists = Tutkinto.objects.filter(
        henkilo=OuterRef('henkilo'),
        vakajarjestaja=OuterRef('vakajarjestaja')
    ).exclude(tutkinto_koodi='003')

    # Main query to get the matching '003' objects
    tutkintos_to_delete = Tutkinto.objects.filter(
        tutkinto_koodi='003'
    ).filter(Exists(other_tutkinto_exists)).order_by('id')
...
def create_new_tukipaatos_aikavali(year=None):
    ...
    with transaction.atomic():
        ...
        conflict_exists = TukipaatosAikavali.objects.filter(
            Q(alkamis_pvm=alkamis_pvm) |
            Q(paattymis_pvm=paattymis_pvm) |
            Q(tilastointi_pvm=tilastointi_pvm)
        ).exists()
...
# Temporary task (can be removed after data removal in production)
@custom_shared_task(single_instance=True)
def remove_tukipaatoses_tilastointi_pvm_2025_05_31_task():
    deleted_count, _ = Tukipaatos.objects.filter(tilastointi_pvm=datetime.date(2025, 5, 31)).delete()
    logger.info(f'Tukipaatos objects deleted. tilastointi_pvm="2025-05-31" count={deleted_count}')
