...
class UiVakajarjestajatViewSet(GenericViewSet, ListModelMixin):
    ...
    def tyontekija_list(self, request, pk=None):
        ...
        self.queryset = (
            ...
            .prefetch_related(
                Prefetch("tyontekijat", queryset=Tyontekija.objects.filter(id__in=Subquery(tyontekija_id_subquery)).distinct())
            )
    ...
    def get_tyontekija_filters(self, vakajarjestaja_id):
        ...
        if (voimassa_pvm := parse_query_parameter(query_params, "voimassa_pvm", datetime.date)) is not None:
            filter_condition &= (
                ...
                & ~Exists(
                    PidempiPoissaolo.objects.filter(get_active_filter(voimassa_pvm), palvelussuhde=OuterRef("palvelussuhteet"))
                )
            )
...
class NestedToimipaikkaViewSet(GenericViewSet, ListModelMixin):
    ...
    def get_queryset(self):
        ...
        return (
            Toimipaikka.objects.filter(toimipaikka_filter)
            .values(
                "id",
                "nimi",
                "organisaatio_oid",
    ...
    def paos_jarjestajat(self, request, organisaatio_pk=None, pk=None):
        ...
        kunta_qs = PaosToiminta.objects.filter(paos_toimipaikka=toimipaikka, voimassa_kytkin=True).values_list(
            "oma_organisaatio_id", flat=True
        )
        jarjestaja_id_qs = PaosOikeus.objects.filter(
            jarjestaja_kunta_organisaatio__in=kunta_qs, tallentaja_organisaatio=vakajarjestaja, voimassa_kytkin=True
