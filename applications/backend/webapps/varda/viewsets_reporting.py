...
from varda.constants import MAXIMUM_ASIAKASMAKSU, SUCCESSFUL_STATUS_CODE_LIST, YRITYSMUOTO_KUNTA
...
from varda.excel_export import (
    ExcelReportType,
    create_excel_report_task,
    create_mass_excel_report_task,
...
from varda.permissions import (
    AdminOrOPHUser,
    IsCertificateAccess,
    MassahakuPermissions,
    RaportitPermissions,
    ReadAdminOrOPHUser,
    get_organisaatio_oids_from_groups,
    get_vakajarjestajat_filter_for_raportit,
    is_oph_staff,
    user_permission_groups_in_organization,
)
from varda.request_logging import auditlogclass, request_log_viewset_decorator_factory, save_audit_log
from varda.serializers_reporting import (
    ...
    MassahakuExcelReportListSerializer,
    MassahakuExcelReportRequestSerializer,
    MassahakuExcelReportResponseSerializer,
...
class ErrorReportLapsetViewSet(AbstractErrorReportViewSet):
    ...
    def get_error_nested_list(self):
        ...
        huoltajatiedot_error_nested_list = [
            [
                ErrorMessages.MA015,
                """la.paos_organisaatio_id IS DISTINCT FROM %s
                AND ma.paattymis_pvm IS NULL
                AND NOT EXISTS(SELECT id FROM varda_varhaiskasvatuspaatos WHERE lapsi_id = la.id
                AND alkamis_pvm <= %s
                AND (paattymis_pvm IS NULL OR paattymis_pvm >= %s))
                AND (ma.alkamis_pvm <= %s)""",
            ...
            [
                ErrorMessages.MA021,
                """la.paos_organisaatio_id IS DISTINCT FROM %s
                AND la.paos_kytkin = TRUE
                AND vapa.jarjestamismuoto_koodi ILIKE 'jm03'
                AND ma.maksun_peruste_koodi ILIKE 'mp03'
                AND ma.palveluseteli_arvo = 0
                AND (ma.paattymis_pvm IS NULL or ma.paattymis_pvm > '2019-12-31')
                AND daterange(vapa.alkamis_pvm, vapa.paattymis_pvm, '[]')
                    && daterange(ma.alkamis_pvm, ma.paattymis_pvm, '[]') """,
...
def resolve_massahaku_organization(request, data, view):
    if AdminOrOPHUser().has_permission(request, view):
        organization_oid = data.get("organisaatio_oid")
        if not organization_oid:
            raise ValidationError({"organisaatio_oid": ["This field is required."]})
    else:
        if "organisaatio_oid" in data:
            raise ValidationError({"organisaatio_oid": ["This field is not allowed."]})
        organization_oids = get_organisaatio_oids_from_groups(request.user, f"{Z4_CasKayttoOikeudet.RAPORTTIEN_KATSELIJA}_")
        if len(organization_oids) != 1:
            raise PermissionDenied("Mass report access must resolve to exactly one organization.")
        organization_oid = organization_oids[0]

    try:
        return Organisaatio.objects.using(DEFAULT_DB_ALIAS).get(organisaatio_oid=organization_oid)
    except Organisaatio.DoesNotExist as exception:
        raise ValidationError({"organisaatio_oid": ["Organisaatio does not exist."]}) from exception


@auditlogclass
@request_log_viewset_decorator_factory()
class MassahakuExcelReportViewSet(GenericViewSet, ListModelMixin, RetrieveModelMixin, CreateModelMixin):
    serializer_class = MassahakuExcelReportRequestSerializer
    permission_classes = (MassahakuPermissions,)
    pagination_class = ChangeablePageSizePagination

    def get_serializer_class(self):
        if self.action == "list":
            return MassahakuExcelReportListSerializer
        if self.action == "retrieve":
            return MassahakuExcelReportResponseSerializer
        return super().get_serializer_class()

    def get_queryset(self):
        queryset = Z8_ExcelReport.objects.using(DEFAULT_DB_ALIAS).filter(user=self.request.user)
        if getattr(self, "action", None) == "list":
            queryset = queryset.select_related("organisaatio", "toimipaikka").order_by("-id")
        return queryset
    ...
    def create(self, request, *args, **kwargs):
        ...
        organisaatio = resolve_massahaku_organization(request, data, self)

        toimipaikka = None
        if data.get("toimipaikka_oid") is not None:
            try:
                toimipaikka = Toimipaikka.objects.using(DEFAULT_DB_ALIAS).get(organisaatio_oid=data["toimipaikka_oid"])
            except Toimipaikka.DoesNotExist as exception:
                raise ValidationError({"toimipaikka_oid": ["Toimipaikka does not exist."]}) from exception
            if toimipaikka.vakajarjestaja_id != organisaatio.id:
                raise ValidationError({"toimipaikka_oid": [ErrorMessages.ER001.value]})

        with transaction.atomic(using=DEFAULT_DB_ALIAS):
            excel_report = Z8_ExcelReport.objects.using(DEFAULT_DB_ALIAS).create(
                user=request.user,
            ...
            transaction.on_commit(lambda: create_mass_excel_report_task.delay(excel_report.id), using=DEFAULT_DB_ALIAS)

        response_serializer = MassahakuExcelReportResponseSerializer(excel_report)
        return Response(response_serializer.data, status=status.HTTP_202_ACCEPTED)

    @action(methods=["get"], detail=True, url_path="file", url_name="file")
    def file(self, request, pk=None):
        instance = self.get_object()
        if instance.status != ReportStatus.FINISHED.value:
            return Response({"detail": "REPORT_NOT_READY"}, status=status.HTTP_409_CONFLICT)
        ...
        try:
            ...
            presigned_url = s3_client.create_presigned_url(get_s3_object_name(instance), expiration=3600)
        except Exception as exception:
            logger.error("Unable to create presigned URL for mass report %s: %s", instance.id, exception)
            return Response({"detail": "REPORT_FILE_UNAVAILABLE"}, status=status.HTTP_503_SERVICE_UNAVAILABLE)
        if not presigned_url:
            logger.error("S3 returned no presigned URL for mass report %s", instance.id)
            return Response({"detail": "REPORT_FILE_UNAVAILABLE"}, status=status.HTTP_503_SERVICE_UNAVAILABLE)
