...
from varda.misc_queries import get_active_filter, get_related_object_changed_id_qs, get_tuentaso_codes_by_tilastointi_pvm, get_ikaryhma_codes_by_tilastointi_pvm
from varda.misc_viewsets import ParentObjectByOidMixin, ViewSetValidator, parse_query_parameter
from varda.models import (
    Henkilo, KieliPainotus, Lapsi, Maksutieto, Organisaatio, Palvelussuhde, PaosOikeus, PaosToiminta, PidempiPoissaolo,
    Taydennyskoulutus, TaydennyskoulutusTyontekija, VuokrattuHenkilosto, ToiminnallinenPainotus, Toimipaikka,
    Tukipaatos, Tutkinto, Tyontekija, Tyoskentelypaikka, Varhaiskasvatuspaatos, Varhaiskasvatussuhde,
    Z10_KelaVarhaiskasvatussuhde, Z2_Code, Z2_Koodisto, Z4_CasKayttoOikeudet, Z6_LastRequest, Z6_RequestLog,
    Z6_RequestSummary, Z8_ExcelReport, Z9_RelatedObjectChanged)
from varda.pagination import (
    ChangeablePageSizePagination, ChangeablePageSizePaginationLarge, ChangeableReportingPageSizePagination,
    DateCursorPagination, DateReverseCursorPagination, HistoricalLargePagination, IdCursorPagination,
    IdReverseCursorPagination, HistoricalCursorPagination)
from varda.permissions import (get_vakajarjestajat_filter_for_raportit, is_oph_staff, IsCertificateAccess,
                               RaportitPermissions, ReadAdminOrOPHUser, user_permission_groups_in_organization)
from varda.request_logging import auditlogclass
from varda.serializers_reporting import (
    DuplicateLapsiSerializer, ErrorReportLapsetSerializer, ErrorReportOrganisaatioSerializer,
    ErrorReportToimipaikatSerializer, ErrorReportTyontekijatSerializer, ExcelReportSerializer,
    KelaEtuusmaksatusAloittaneetSerializer, KelaEtuusmaksatusKorjaustiedotPoistetutSerializer,
    KelaEtuusmaksatusKorjaustiedotSerializer, KelaEtuusmaksatusLopettaneetSerializer,
    KelaEtuusmaksatusMaaraaikaisetSerializer, KelaEtuusmaksatusAloittaneetV2Serializer,
    KelaEtuusmaksatusKorjaustiedotPoistetutV2Serializer, KelaEtuusmaksatusKorjaustiedotV2Serializer,
    KelaEtuusmaksatusLopettaneetV2Serializer, KelaEtuusmaksatusMaaraaikaisetV2Serializer, RequestSummaryGroupSerializer,
    RequestSummarySerializer, TiedonsiirtoSerializer, TiedonsiirtotilastoSerializer, TiedonsiirtoYhteenvetoSerializer,
    TkHenkilostotiedotSerializer, TkOrganisaatiotSerializer, TkVakatiedotSerializer, TransferOutageReportSerializer,
    ValssiOrganisaatioSerializer, ValssiTaustatiedotSerializer, ValssiToimipaikkaSerializer, ValssiTyontekijaSerializer,
    VipunenKielipainotusSerializer, VipunenLapsiSerializer, VipunenMaksutietoSerializer, VipunenOrganisaatioSerializer,
    VipunenPalvelussuhdeSerializer, VipunenPidempiPoissaoloSerializer, VipunenTaydennyskoulutusSerializer,
    VipunenTaydennyskoulutusTyontekijaSerializer, VipunenVuokrattuHenkilostoSerializer,
    VipunenToiminnallinenPainotusSerializer, VipunenToimipaikkaSerializer, VipunenTutkintoSerializer,
    VipunenTyontekijaSerializer, VipunenTyoskentelypaikkaSerializer, VipunenVarhaiskasvatuspaatosSerializer,
    VipunenVarhaiskasvatussuhdeSerializer, VipunenTuenTiedotSerializer)
...
class KelaEtuusmaksatusAloittaneetViewset(KelaBaseViewSet):
    ...
    def get_queryset(self):
        return Z10_KelaVarhaiskasvatussuhde.objects.using(settings.READER_DB).raw(f'''
...
class AbstractErrorReportViewSet(GenericViewSet, ListModelMixin):
    ...
    def get_errors(self):
        ...
        for error in self.get_error_nested_list():
            ...
            if (not error_code_tuple or (error_code.startswith(error_code_tuple) and not exclude_errors) or
                    (not error_code.startswith(error_code_tuple) and exclude_errors)):
...
class ErrorReportLapsetViewSet(AbstractErrorReportViewSet):
    ...
    def get_error_nested_list(self):
        ...
        huoltajatiedot_error_nested_list = [
            ...
            [ErrorMessages.MA020, '''la.paos_organisaatio_id IS DISTINCT FROM %s AND asiakasmaksu > %s AND
             org.yritysmuoto = ANY(%s)''', [self.vakajarjestaja_id, MAXIMUM_ASIAKASMAKSU, YRITYSMUOTO_KUNTA], 'ma.id',
             Maksutieto.get_name()],
            [ErrorMessages.MA021, '''la.paos_organisaatio_id IS DISTINCT FROM %s AND la.paos_kytkin = TRUE AND
            ...
            AND (ma.paattymis_pvm IS NULL or ma.paattymis_pvm > '2019-12-31') ''',
             [self.vakajarjestaja_id], 'ma.id', Maksutieto.get_name()],
        ]
    ...
    def get_queryset(self):
        ...
        return Lapsi.objects.using(settings.READER_DB).raw(
            raw_query, [*parameter_list, self.vakajarjestaja_id, self.vakajarjestaja_id,
...
class ErrorReportToimipaikatViewSet(AbstractErrorReportViewSet):
    ...
    def get_error_nested_list(self):
        ...
        vakatiedot_error_nested_list = [
            ...
            [ErrorMessages.KP006, '''kipa.id IS NOT NULL AND kipa.kielipainotustyyppi_koodi IS NULL AND
             kipa.paattymis_pvm IS NULL''', [], 'kipa.id', KieliPainotus.get_name()],
        ]
    ...
    def get_queryset(self):
        ...
        return Toimipaikka.objects.using(settings.READER_DB).raw(
            raw_query, [*parameter_list, *join_parameter_list, self.vakajarjestaja_id,
...
class ErrorReportOrganisaatioViewSet(AbstractErrorReportViewSet):
    ...
    def get_error_nested_list(self):
        # [[ErrorMessages, filter condition, parameters[], ID lookup, model name for ID lookup]]
        return [
            [ErrorMessages.VJ010, "org.sahkopostiosoite = '' OR org.sahkopostiosoite IS NULL", [], 'org.id',
...
class TiedonsiirtoYhteenvetoViewSet(GenericViewSet, ListModelMixin):
    ...
    def get_queryset(self):
        vakajarjestaja_filter = get_vakajarjestajat_filter_for_raportit(self.request)
        queryset = (Z6_RequestLog.objects.using(settings.READER_DB)
...
class DuplicateLapsiViewSet(GenericViewSet, ListModelMixin):
    ...
    def get_queryset(self):
        duplicate_lapsi_qs = (Lapsi.objects.using(settings.READER_DB)
...
class TransferOutageReportViewSet(GenericViewSet, ListModelMixin):
    ...
    def get_queryset(self):
        ...
        return (Z6_LastRequest.objects.using(settings.READER_DB)
...
class RequestSummaryViewSet(GenericViewSet, ListModelMixin):
    ...
    def get_queryset(self):
        ...
        if self.request.query_params.get('group', '').lower() == 'true':
            return (Z6_RequestSummary.objects.using(settings.READER_DB)
...
class ValssiTaustatiedotViewSet(ParentObjectByOidMixin, GenericViewSet, ListModelMixin):
    ...
    def get_toimipaikka_data(self, result):
        toimipaikka_qs = (Toimipaikka.objects.using(settings.READER_DB)
                          .filter(get_active_filter(self.today), vakajarjestaja=self.organisaatio))
        result['toimipaikat'] = {
        ...
        toimintamuoto_codes = (Z2_Code.objects.using(settings.READER_DB)
                               ...
                               .order_by('code_lower'))
        jarjestamismuoto_codes = (Z2_Code.objects.using(settings.READER_DB)
    ...
    def get_henkilosto_data(self, result):
        ...
        tehtavanimike_codes = (Z2_Code.objects.using(settings.READER_DB)
        ...
        tyoskentelypaikka_qs = (
            ...
            .filter(active_filter & get_active_filter(self.today, prefix='palvelussuhde') &
                    ~Exists(PidempiPoissaolo.objects.using(settings.READER_DB)
        ...
        koulutuspaivia_qs = (Taydennyskoulutus.objects.using(settings.READER_DB)
    ...
    def get_tuen_tiedot_data(self, result):
        ...
        latest_tukipaatos = Tukipaatos.objects.filter(
            vakajarjestaja=self.organisaatio).order_by('-tilastointi_pvm').first()
        ...
        tukipaatoses = Tukipaatos.objects.filter(
            vakajarjestaja=self.organisaatio,
...
class ValssiTyontekijaViewSet(ParentObjectByOidMixin, GenericViewSet, ListModelMixin):
    ...
    def list(self, request, *args, **kwargs):
        ...
        self.queryset = (Tyontekija.objects.using(settings.READER_DB)
...
class VipunenLapsiViewSet(SimpleHistoricalViewSet):
    ...
    def get_queryset(self):
        ...
        id_qs = get_related_object_changed_id_qs(
            Lapsi.get_name(), self.datetime_gt, self.datetime_lte,
...
class VipunenTyontekijaViewSet(SimpleHistoricalViewSet):
    ...
    def get_queryset(self):
        ...
        id_qs = get_related_object_changed_id_qs(
            Tyontekija.get_name(), self.datetime_gt, self.datetime_lte,
...
class VipunenTuenTiedotViewSet(GenericViewSet, ListModelMixin):
    ...
    def get_tuen_tiedot_data(self):
        ...
        for organisaatio_oid in organisaatio_oids:
            ...
            org_tuen_tiedot = dict(
                organisaatio_oid=organisaatio_oid,
                tuen_tiedot=tuen_tiedot)
