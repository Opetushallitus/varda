...
def get_elasticache_iam_token(replication_group_id, user_id):
    ...
    """
    aws_request = AWSRequest(method='GET', url=f'http://{replication_group_id}/',
...
class IAMTokenCredentialProvider(CredentialProvider):
    ...
    def get_credentials(self):
        ...
        if (not IAMTokenCredentialProvider.current_iam_token or
                IAMTokenCredentialProvider.token_datetime < now - datetime.timedelta(seconds=600)):
            # Generate new token if it does not exist or token is older than 600 seconds
            IAMTokenCredentialProvider.current_iam_token = get_elasticache_iam_token(self.replication_group_id,
                                                                                     self.user_id)
