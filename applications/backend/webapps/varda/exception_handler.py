...
error_templates = {
    status.HTTP_400_BAD_REQUEST: {
        ...
        OTHER: {
            ...
            "Enter a number.": ErrorMessages.GE025.value,
        },
    },
    ...
    status.HTTP_429_TOO_MANY_REQUESTS: {
        DYNAMIC: {"throttled": (re.compile(r"available in ([\d]+) second"), ErrorMessages.DY008.value)}
    },
}
...
def varda_exception_handler(exc, context):
    ...
    if response.status_code in [status.HTTP_400_BAD_REQUEST, status.HTTP_403_FORBIDDEN, status.HTTP_429_TOO_MANY_REQUESTS]:
        """
...
def _parse_error_messages(error_messages, status_code=status.HTTP_400_BAD_REQUEST):
    if isinstance(error_messages, dict):
        ...
        if (
            isinstance(error_messages[0], dict)
            and "error_code" not in error_messages[0]
            and "description" not in error_messages[0]
        ):
...
def _parse_error_message(message, status_code):
    ...
    logger.warning(
        "Varda error message missing for the following error.\n"
...
def _add_error_translations(error_message):
    ...
    translations = (
        Z2_CodeTranslation.objects.filter(code__code_value=error_code, code__koodisto__name=Koodistot.virhe_koodit.value)
...
def _log_throttle_error_message(request):
    """
    Log error message for 429 responses. Only log at most every 60 minutes to prevent spam.
    """
    ...
    if not cache.get(cache_key):
        ...
        logger.warning(f"Request throttled for user/IP {identifier}, tried to access {request.path}")
        # Set cache entry that expires in 60 * 60 seconds
