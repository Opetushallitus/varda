...
from django.db.models import Case, CharField, Exists, OuterRef, Q, Subquery, TextField, Value, When
...
from varda.permissions import TYONTEKIJA_GROUPS, VAKA_GROUPS, parse_toimipaikka_id_list
...
class CustomParametersFilterBackend(BaseFilterBackend):
    ...
    def _get_schema(self, data_type, description):
        return openapi.Schema(type=self.TYPE_MAPPING.get(data_type, openapi.TYPE_STRING), description=description)
        ...
        for custom_parameter in custom_parameters:
            schema_fields.append(
                openapi.Parameter(
                    ...
                    in_=custom_parameter.location,
                    type=self.TYPE_MAPPING.get(custom_parameter.data_type, openapi.TYPE_STRING),
                    required=custom_parameter.required,
                    description=custom_parameter.description,
                )
            )
...
class CustomCharFilter(django_filters.CharFilter):
    ...
    e.g. /api/v2/toimipaikat/?oid_tunniste=EMPTY
...
class CaseInsensitiveContainsFilter(django_filters.BaseCSVFilter):
    def filter(self, qs, value):
        ...
        return qs.annotate(
            **{
                field_name: Cast(
                    Lower(Cast(self.field_name, output_field=TextField())), output_field=ArrayField(base_field=TextField())
                )
            }
...
class ToimipaikkaFilter(django_filters.FilterSet):
    ...
    toimintakieli_koodi = CaseInsensitiveContainsFilter()
...
class YksiloimattomatHenkilotFilter(django_filters.FilterSet):
    ...
    def filter_vakatoimija_oid(self, queryset, name, value):
        return queryset.filter(
            Q(lapsi__vakatoimija__organisaatio_oid=value) | Q(tyontekijat__vakajarjestaja__organisaatio_oid=value)
        )
...
class VuokrattuHenkilostoFilter(django_filters.FilterSet):
    vakajarjestaja = OrganisaatioFieldFilter()
...
class TaydennyskoulutusFilter(django_filters.FilterSet):
    ...
    vakajarjestaja_oid = django_filters.CharFilter(
        field_name="tyontekijat__vakajarjestaja__organisaatio_oid", lookup_expr="exact", distinct=True
    )
    nimi = django_filters.CharFilter(lookup_expr="icontains")
...
class TaydennyskoulutusTyontekijaListFilter(django_filters.FilterSet):
    vakajarjestaja_oid = django_filters.CharFilter(
        field_name="vakajarjestaja__organisaatio_oid", lookup_expr="exact", distinct=True, label="vakajarjestaja_oid"
    )
    henkilo_oid = django_filters.CharFilter(field_name="henkilo__henkilo_oid", lookup_expr="exact", label="henkilo_oid")
    toimipaikka_oid = django_filters.CharFilter(
        field_name="palvelussuhteet__tyoskentelypaikat__toimipaikka__organisaatio_oid",
        lookup_expr="exact",
        distinct=True,
        label="toimipaikka_oid",
    )
...
class TyontekijaFilter(django_filters.FilterSet):
    vakajarjestaja_id = django_filters.NumberFilter(
        field_name="vakajarjestaja__id", lookup_expr="exact", label="vakajarjestaja_id"
    )
    vakajarjestaja_oid = django_filters.CharFilter(
        field_name="vakajarjestaja__organisaatio_oid", lookup_expr="exact", label="vakajarjestaja_oid"
    )
...
class UiTyontekijaFilter(django_filters.FilterSet):
    ...
    def get_rajaus_filters(self):
        ...
        if (
            not rajaus
            or voimassaolo_none_and_required
            or not (alkamis_pvm := parse_query_parameter(query_params, "alkamis_pvm", datetime.date))
            or not (paattymis_pvm := parse_query_parameter(query_params, "paattymis_pvm", datetime.date))
        ):
        ...
        if rajaus in ["tyoskentelypaikat", "palvelussuhteet", "poissaolot"]:
            ...
            tyontekija_filter = Q(
                taydennyskoulutukset__suoritus_pvm__gte=alkamis_pvm, taydennyskoulutukset__suoritus_pvm__lte=paattymis_pvm
            )
            if tehtavanimike_taydennyskoulutus:
                tyontekija_filter = tyontekija_filter & Q(
                    taydennyskoulutukset_tyontekijat__tehtavanimike_koodi=tehtavanimike_taydennyskoulutus
                )
    ...
    def filter_queryset(self, queryset):
        ...
        if tutkinto_arg := query_params.get("tutkinto", None):
            tyontekija_filter &= Q(henkilo__tutkinnot__tutkinto_koodi__iexact=tutkinto_arg)

        if tyosuhde_arg := query_params.get("tyosuhde", None):
            tyontekija_filter &= Q(palvelussuhteet__tyosuhde_koodi__iexact=tyosuhde_arg)

        if (
            parse_query_parameter(query_params, "aktiiviset", bool) is True
            and (alkamis_pvm := parse_query_parameter(query_params, "alkamis_pvm", datetime.date))
            and (paattymis_pvm := parse_query_parameter(query_params, "paattymis_pvm", datetime.date))
        ):
            tyontekija_filter &= (
                get_active_filter(alkamis_pvm, target_date_secondary=paattymis_pvm, prefix="palvelussuhteet")
                & get_active_filter(alkamis_pvm, target_date_secondary=paattymis_pvm, prefix="palvelussuhteet__tyoskentelypaikat")
                &
                ...
                ~Exists(
                    PidempiPoissaolo.objects.filter(
                        Q(palvelussuhde=OuterRef("palvelussuhteet"))
                        & (get_active_filter(alkamis_pvm) | (Q(alkamis_pvm__gte=alkamis_pvm) & Q(alkamis_pvm__lte=paattymis_pvm)))
                    )
                )
            )
...
class UiLapsiFilter(django_filters.FilterSet):
    """
    Can't utilize djangofilters provided filters (e.g. CharFilter), because all filters need to be inside a single
    .filter() call. For example if we had multiple filter() calls, user has selected
    toimipaikat=1,2&rajaus=vakasuhteet&voimassaolo=voimassa&alkamis_pvm=2022-01-01&paattymis_pvm=2022-01-01
    we may get lapsi that has passive varhaiskasvatussuhde in toimipaikka=1 and
    active varhaiskasvatussuhde in toimipaikka=3.
    """
    ...
    def get_rajaus_filters(self):
        ...
        prefixes = {
            ...
            "vakapaatokset": "varhaiskasvatuspaatokset__",
            "maksutiedot": "huoltajuussuhteet__maksutiedot__",
        }

        if rajaus in ["vakasuhteet", "vakapaatokset", "maksutiedot"]:
            prefix = prefixes[rajaus]
        else:
            return Q()
    ...
    def filter_queryset(self, queryset):
        ...
        toimipaikka_id_list = parse_toimipaikka_id_list(user, query_params.get("toimipaikat", ""), VAKA_GROUPS, include_paos=True)
        if len(toimipaikka_id_list) > 0:
        ...
        if (palveluseteli := parse_query_parameter(query_params, "palveluseteli", bool)) is not None:
            lapsi_filter &= (
                Q(huoltajuussuhteet__maksutiedot__palveluseteli_arvo__gt=0)
                if palveluseteli
                else (
        ...
        if toimintamuoto_arg := query_params.get("toimintamuoto", None):
            lapsi_filter &= Q(
                varhaiskasvatuspaatokset__varhaiskasvatussuhteet__toimipaikka__toimintamuoto_koodi__iexact=toimintamuoto_arg
            )
        ...
        if (
            ...
            lapsi_filter &= get_active_filter(
                alkamis_pvm, target_date_secondary=paattymis_pvm, prefix="varhaiskasvatuspaatokset"
            ) & get_active_filter(
                alkamis_pvm, target_date_secondary=paattymis_pvm, prefix="varhaiskasvatuspaatokset__varhaiskasvatussuhteet"
            )
...
class UiAllVakajarjestajaFilter(django_filters.FilterSet):
    ...
    def filter_search(self, queryset, name, value):
        matching_kunta_koodit = Z2_Code.objects.filter(
            koodisto__name=Koodistot.kunta_koodit.value, translations__name__iexact=value
        ).values_list("code_value", flat=True)
        return queryset.filter(
            Q(nimi__icontains=value)
            | Q(postitoimipaikka__iexact=value)
            | Q(organisaatio_oid=value)
            | Q(y_tunnus=value)
            | Q(kunta_koodi__in=matching_kunta_koodit)
        )


class KelaEtuusmaksatusFilter(django_filters.FilterSet):
    kunta_koodi = django_filters.CharFilter(
        field_name="varhaiskasvatuspaatos__lapsi__henkilo__kotikunta_koodi", lookup_expr="exact", label="kotikunta_koodi"
    )
    syntyma_pvm = django_filters.DateFilter(
        field_name="varhaiskasvatuspaatos__lapsi__henkilo__syntyma_pvm", lookup_expr="gte", label="syntyma_pvm"
    )
...
class TiedonsiirtoFilter(django_filters.FilterSet):
    ...
    def filter_target(self, queryset, name, value):
        ...
        search_filter = reduce(operator.and_, (Q(search_target__icontains=search_value) for search_value in value.split(" ")))
        ...
        return (
            queryset.filter(target_model__isnull=False, target_id__isnull=False)
            .annotate(
                search_target=Case(
                    When(
                        target_model=lapsi_model_name,
                        then=self._get_target_filter_annotation(
                            Lapsi, ["henkilo__henkilo_oid", "henkilo__etunimet", "henkilo__sukunimi"]
                        ),
                    ),
                    When(
                        target_model=tyontekija_model_name,
                        then=self._get_target_filter_annotation(
                            Tyontekija, ["henkilo__henkilo_oid", "henkilo__etunimet", "henkilo__sukunimi"]
                        ),
                    ),
                    When(
                        target_model=toimipaikka_model_name,
...
class ExcelReportFilter(django_filters.FilterSet):
    ...
    def filter_organisaatio(self, queryset, name, value):
        ...
        return queryset.filter(Q(organisaatio_id=value) | Q(organisaatio__organisaatio_oid=value) | Q(organisaatio=None))
...
class TukipaatosFilter(django_filters.FilterSet):
    vakajarjestaja = OrganisaatioFieldFilter()
    vuosi = django_filters.NumberFilter(field_name="luonti_pvm__year", lookup_expr="exact")
    ikaryhma_koodi = django_filters.CharFilter(lookup_expr="iexact")
    tuentaso_koodi = django_filters.CharFilter(lookup_expr="iexact")
    tilastointi_pvm = django_filters.DateFilter(lookup_expr="exact")
