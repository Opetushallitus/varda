...
def send_read_audit_log(aws_log_client, datetime_end, hostname, environment):
    ...
    audit_log_qs = (Z5_AuditLog.objects
                    .filter(time_of_event__gte=aikaleima.aikaleima)
            ...
            audit_log_list.append(
                {
                    ...
                    'message': get_audit_log_row(
                        log_data.log_seq, hostname, environment, audit_log.time_of_event, 'GET',
                        audit_log.successful_get_request_path, audit_log.query_params,
            ...
            historical_data_list.append(
                {
                    ...
                    'message': get_audit_log_row(
                        log_data.log_seq, hostname, environment, historical_data.history_date, operation, change_url,
                        None, username
                    )
                }
            )
...
def send_data_access_log():
    ...
    data_access_log_qs = (Z12_DataAccessLog.objects
                          .filter(timestamp__gt=aikaleima.aikaleima, timestamp__lte=end_timestamp)
                          .order_by('timestamp'))
