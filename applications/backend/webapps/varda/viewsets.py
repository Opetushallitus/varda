...
from django.db.models import Count, Exists, F, OuterRef, ProtectedError, Q, Subquery, Sum
...
from varda.api_token import create_token_for_user
from varda.cache import (
    delete_cache_keys_related_model,
    get_object_ids_user_has_permissions,
    get_queryset_count_cache_key,
    get_yhteenveto_cache,
...
from varda.clients.organisaatio_client import get_organisaatio
from varda.constants import YHTEENVETO_TUEN_TIEDOT_DATE_FORMAT, YRITYSMUOTO_KUNTA
...
from varda.misc import (
    ...
    check_active_celery_workers,
    encrypt_string,
    get_latest_tilastointi_pvm,
    get_next_timeframe,
    hash_string,
    update_painotus_kytkin,
)
from varda.misc_queries import get_active_filter, get_paos_toimipaikat, get_queryset_count
from varda.misc_viewsets import IncreasedModifyThrottleMixin, ObjectByTunnisteMixin, ParentObjectMixin, PermissionListMixin
...
from varda.organisaatiopalvelu import (
    check_if_toimipaikka_exists_in_organisaatiopalvelu,
    create_toimipaikka_in_organisaatiopalvelu,
)
from varda.pagination import ChangeablePageSizeCursorPagination, UiYksiloimattomatChangeablePageSizeCursorPaginationLarge
...
from varda.permissions import (
    ...
    FrontendAccessOnly,
    ReadAdminOrOPHUser,
    ToimipaikkaPermissions,
    assign_henkilo_permissions,
    assign_kielipainotus_permissions,
    assign_lapsi_permissions,
    assign_maksutieto_permissions,
    assign_paos_toiminta_permissions,
    assign_toiminnallinen_painotus_permissions,
    assign_toimipaikka_permissions,
    assign_tukipaatos_permissions,
    assign_varhaiskasvatuspaatos_permissions,
    assign_varhaiskasvatussuhde_permissions,
    ...
    is_oph_staff,
    remove_paos_toiminta_permissions,
    throw_if_not_tallentaja_permissions,
    user_belongs_to_correct_groups,
    user_has_huoltajatieto_tallennus_permissions_to_correct_organization,
    user_has_tallentaja_permission_in_organization,
    user_permission_groups_in_organization,
    user_permission_groups_in_organizations,
)
from varda.request_logging import auditlog, auditlogclass, request_log_viewset_decorator_factory, save_data_access_log
from varda.serializers import (
    ActiveUserSerializer,
    AuthTokenSerializer,
    ClearCacheSerializer,
    ExternalPermissionsSerializer,
    GroupSerializer,
    HaeHenkiloSerializer,
    HenkiloSerializer,
    HenkiloSerializerAdmin,
    HuoltajaSerializer,
    HuoltajuussuhdeSerializer,
    KieliPainotusSerializer,
    LapsiKoosteSerializer,
    LapsiSerializer,
    LapsiSerializerAdmin,
    MaksutietoPostSerializer,
    MaksutietoSerializer,
    MaksutietoUpdateSerializer,
    OrganisaatioSerializer,
    OrganisaatioYhteenvetoSerializer,
    PaosOikeusSerializer,
    PaosToimijatSerializer,
    PaosToimintaSerializer,
    PaosToimipaikatSerializer,
    PulssiVakajarjestajatSerializer,
    ToiminnallinenPainotusSerializer,
    ToimipaikkaKoosteSerializer,
    ToimipaikkaKoosteV2Serializer,
    ToimipaikkaSerializer,
    ToimipaikkaUpdateSerializer,
    ToimipaikkaUpdateV2Serializer,
    ToimipaikkaV2Serializer,
    TukipaatosSerializer,
    UpdateHenkiloWithOidSerializer,
    UserSerializer,
    VarhaiskasvatuspaatosSerializer,
    VarhaiskasvatussuhdeSerializer,
    YksiloimattomatHenkilotSerializer,
)
...
class HaeYksiloimattomatHenkilotViewSet(GenericViewSet, ListModelMixin):
    ...
    queryset = Henkilo.objects.using(settings.READER_DB).filter(Q(vtj_yksiloity=False) & Q(vtj_yksilointi_yritetty=True))
    serializer_class = YksiloimattomatHenkilotSerializer
    permission_classes = (ReadAdminOrOPHUser,)
    pagination_class = UiYksiloimattomatChangeablePageSizeCursorPaginationLarge
    ...
    def get_queryset(self):
        return self.queryset.annotate(
            lapsi__vakatoimija__organisaatio_oid=F("lapsi__vakatoimija__organisaatio_oid"),
            tyontekijat__vakajarjestaja__organisaatio_oid=F("tyontekijat__vakajarjestaja__organisaatio_oid"),
        ).order_by("lapsi__vakatoimija__organisaatio_oid", "tyontekijat__vakajarjestaja__organisaatio_oid")
...
class NestedHuoltajaViewSet(GenericViewSet, ListModelMixin):
    ...
    def get_lapsi(self, request, lapsi_pk=None):
        lapsi = get_object_or_404(Lapsi.objects.using(settings.READER_DB).all(), pk=lapsi_pk)
        user = request.user
    ...
    def list(self, request, *args, **kwargs):
        ...
        queryset = self.filter_queryset(
            Huoltaja.objects.using(settings.READER_DB).filter(huoltajuussuhteet__lapsi__id=kwargs["lapsi_pk"])
        ).order_by("id")
...
class HuoltajuussuhdeViewSet(ModelViewSet):
    ...
    def get_queryset(self):
        user = self.request.user
        if user.is_superuser:
            return Huoltajuussuhde.objects.using(settings.READER_DB).all().order_by("id")
        else:
...
class NestedLapsiViewSet(GenericViewSet, ListModelMixin):
    ...
    def get_huoltaja(self, request, huoltaja_pk=None):
        huoltaja = get_object_or_404(Huoltaja.objects.using(settings.READER_DB).all(), pk=huoltaja_pk)
        user = request.user
    ...
    def list(self, request, *args, **kwargs):
        ...
        queryset = self.filter_queryset(
            Lapsi.objects.using(settings.READER_DB).filter(huoltajuussuhteet__huoltaja__id=kwargs["huoltaja_pk"])
        ).order_by("id")
...
class ActiveUserViewSet(GenericViewSet, ListModelMixin):
    ...
    def list(self, request, *args, **kwargs):
        ...
        if (
            (additional_cas_user_fields := getattr(user_object, "additional_cas_user_fields", None))
            and getattr(additional_cas_user_fields, "kayttajatyyppi", None) == Kayttajatyyppi.VIRKAILIJA.value
            and (settings.PRODUCTION_ENV or settings.QA_ENV)
        ):
...
class ApikeyViewSet(GenericViewSet, ListModelMixin, CreateModelMixin):
    ...
    def get_token_from_request(self):
        if (
            self.request.auth
            and (auth_header := get_authorization_header(self.request).decode("utf-8").split())[0].lower() == "token"
        ):
    ...
    def list(self, request, *args, **kwargs):
        ...
        auth_token = create_token_for_user(request.user)
    ...
    def perform_create(self, serializer):
        if self.get_token_from_request():
            # Logout-operation deletes the current token
            knox_views.LogoutView.as_view()(self.request._request)

        # Create a new token
        auth_token = create_token_for_user(self.request.user)

        # Attach token data to serializer
        serializer._data = {"token": auth_token}
...
@request_log_viewset_decorator_factory(log_anonymous=True)
class ExternalPermissionsViewSet(GenericViewSet, CreateModelMixin):
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            ...
            return Response(
                {"accessAllowed": permit_identification, "errorMessage": "loggedInUserOid was not found"},
        ...
        if henkilot_qs_length == 0:
            return Response(
                {"accessAllowed": permit_identification, "errorMessage": "Person not found"},
                status=status.HTTP_200_OK,
                headers=headers,
            )
...
class ToimipaikkaViewSet(
    IncreasedModifyThrottleMixin,
    ObjectByTunnisteMixin,
    PermissionListMixin,
    GenericViewSet,
    CreateModelMixin,
    RetrieveModelMixin,
    ...
    queryset = Toimipaikka.objects.all().select_related("vakajarjestaja").order_by("id")
    ...
    def perform_create(self, serializer):
        ...
        skip_create_toimipaikka_in_organisaatiopalvelu = False

        toimipaikka_oid = check_if_toimipaikka_exists_in_organisaatiopalvelu(vakajarjestaja_id, validated_data["nimi"])
        if toimipaikka_oid is None:
            pass  # Not found, create one
        elif toimipaikka_oid == "":  # An error occured
            raise ValidationError({"errors": [ErrorMessages.TP007.value]})
        else:
            if settings.PRODUCTION_ENV:
                raise ValidationError({"nimi": [ErrorMessages.TP008.value]})  # HTTP_400_BAD_REQUEST
            else:
                # Custom handling in test environments. When an existing toimipaikka is found, let's return its oid to user.
                skip_create_toimipaikka_in_organisaatiopalvelu = True

        try:
            with transaction.atomic():
                if skip_create_toimipaikka_in_organisaatiopalvelu:
                    organisaatio_data = get_organisaatio(toimipaikka_oid)
                    toimipaikka_organisaatio_oid = organisaatio_data["oid"]
                else:  # Toimipaikka was not found in Organisaatiopalvelu. We can POST it there.
                ...
                # Validate created (or fetched) oid
                validators.validate_organisaatio_oid(toimipaikka_organisaatio_oid, custom_raise_500=True)
            ...
            raise ValidationError({"toimipaikka": [ErrorMessages.TP002.value]})  # HTTP_400_BAD_REQUEST
...
@auditlogclass
@request_log_viewset_decorator_factory(target_path=[])
class ToimipaikkaV2ViewSet(ToimipaikkaViewSet):
    """
    list:
        Nouda kaikki toimipaikat.
    ...
    def get_serializer_class(self):
        request = self.request
        if request.method == "PUT" or request.method == "PATCH":
            return ToimipaikkaUpdateV2Serializer
        elif self.action == "kooste":
            return ToimipaikkaKoosteV2Serializer
        else:
            return ToimipaikkaV2Serializer


@auditlogclass
@request_log_viewset_decorator_factory(target_path=[])
class ToimipaikkaV3ViewSet(ToimipaikkaV2ViewSet):
    def perform_update(self, serializer):
        if "organisaatio_oid" in self.request.data:
            raise ValidationError({"organisaatio_oid": [ErrorMessages.GE013.value]}, code="invalid")
...
class ToiminnallinenPainotusViewSet(IncreasedModifyThrottleMixin, ObjectByTunnisteMixin, PermissionListMixin, ModelViewSet):
    """
    list:
...
class HenkiloViewSet(IncreasedModifyThrottleMixin, GenericViewSet, RetrieveModelMixin, CreateModelMixin):
    ...
    create:
        Luo yksi uusi henkilö tai hae olemassa oleva henkilö.
        HTTP 200 jos henkilö on jo Vardassa, HTTP 201 jos henkilö lisättiin uutena.
    """
    ...
    def validate_henkilo_data(self, henkilo_data):
        ...
        if henkilo_data:
            ...
            is_hetuton_yksiloity = not henkilo_data.get("hetu", None) and henkilo_data.get("yksiloity", None)
            if not is_hetullinen and not is_hetuton_yksiloity:
                raise ValidationError(
                    {
                        "henkilo_oid": [
                            ErrorMessages.HE002.value,
                        ]
                    }
                )

    def validate_henkilo_oid(self, henkilo_data, henkilo_oid):
        """
        Validate if henkilo from Oppijanumerorekisteri has an oid if user did not provide one.
        """
        if henkilo_data and not henkilo_oid:
            if not henkilo_data.get("oidHenkilo"):
                logger.error("No henkilo oid returned from ONR")
                raise ValidationError(
                    {
                        "henkilo_oid": [
                            ErrorMessages.HE018.value,
                        ]
                    }
                )
    ...
    @swagger_auto_schema(
        responses={status.HTTP_200_OK: HenkiloSerializer(many=False), status.HTTP_201_CREATED: HenkiloSerializer(many=False)}
    )
    def create(self, request, *args, **kwargs):
    ...
    def perform_create(self, serializer):
        ...
        # select first etunimi for kutsumanimi if it isn't given
        kutsumanimi = (
            validated_data["kutsumanimi"]
            if "kutsumanimi" in validated_data and validated_data["kutsumanimi"]
            else etunimet.split(" ")[0]
        )
        ...
        henkilo_data = (
            self._get_henkilo_data_by_henkilotunnus(validated_data, henkilotunnus, etunimet, kutsumanimi, sukunimi)
            if henkilotunnus
            else self._get_henkilo_data_by_oid(henkilo_oid, etunimet, sukunimi)
        )
        ...
        self.validate_henkilo_oid(henkilo_data, henkilo_oid)

        # Validate ONR henkilo oid
        if onr_henkilo_oid := henkilo_data.get("oidHenkilo", None) if henkilo_data else None:
            validators.validate_henkilo_oid(onr_henkilo_oid, custom_raise_500=True)
...
class LapsiViewSet(IncreasedModifyThrottleMixin, ObjectByTunnisteMixin, PermissionListMixin, ModelViewSet):
    ...
    def return_lapsi_if_already_created(self, validated_data, toimipaikka_oid, paos_oikeus):
        ...
        if "paos_organisaatio" in validated_data and validated_data["paos_organisaatio"] is not None:
            q_obj = Q(
                henkilo=validated_data["henkilo"],
        ...
        if lapsi_obj:
            ...
            if (
                validated_data["lahdejarjestelma"] != lapsi_obj.lahdejarjestelma
                or validated_data.get("tunniste", None) != lapsi_obj.tunniste
            ):
        ...
        for lapsi in saved_object.henkilo.lapsi.all():
            ...
            if huoltajuussuhteet_qs:
                for huoltajuussuhde in huoltajuussuhteet_qs:
                    Huoltajuussuhde(
                        lapsi=saved_object, huoltaja=huoltajuussuhde.huoltaja, voimassa_kytkin=huoltajuussuhde.voimassa_kytkin
                    ).save()
        ...
        with transaction.atomic():
            for object_list in [vakasuhde_qs, vakapaatos_qs, maksutieto_qs]:
                for object_instance in object_list:
                    ...
                    if isinstance(object_instance, Maksutieto):
...
class VarhaiskasvatuspaatosViewSet(IncreasedModifyThrottleMixin, ObjectByTunnisteMixin, PermissionListMixin, ModelViewSet):
    """
    list:
    ...
    queryset = Varhaiskasvatuspaatos.objects.all().select_related("lapsi").order_by("id")
    ...
    def perform_create(self, serializer):
        ...
        with transaction.atomic():
            if lapsi.paos_kytkin:
                paos_oikeus = check_if_oma_organisaatio_and_paos_organisaatio_have_paos_agreement(
                    lapsi.oma_organisaatio, lapsi.paos_organisaatio
                )
...
class VarhaiskasvatussuhdeViewSet(IncreasedModifyThrottleMixin, ObjectByTunnisteMixin, PermissionListMixin, ModelViewSet):
    """
    list:
    ...
    queryset = Varhaiskasvatussuhde.objects.all().select_related("varhaiskasvatuspaatos", "toimipaikka").order_by("id")
    ...
    def perform_create(self, serializer):
        ...
        with transaction.atomic():
            ...
            delete_cache_keys_related_model(Varhaiskasvatuspaatos.get_name(), varhaiskasvatussuhde.varhaiskasvatuspaatos.id)
...
class MaksutietoViewSet(IncreasedModifyThrottleMixin, ObjectByTunnisteMixin, PermissionListMixin, ModelViewSet):
    ...
    huoltajuussuhde_prefetch_qs = MaksutietoHuoltajuussuhde.objects.select_related(
        "huoltajuussuhde__huoltaja__henkilo", "huoltajuussuhde__lapsi"
    )
    # Only query distinct results, as related object filters can return the same object multiple times
    queryset = (
        Maksutieto.objects.all()
    ...
    def perform_create(self, serializer):
        ...
        if lapsi.paos_kytkin:
            if not user_has_huoltajatieto_tallennus_permissions_to_correct_organization(
                user, lapsi.oma_organisaatio.organisaatio_oid
            ):
            ...
            if not user_has_huoltajatieto_tallennus_permissions_to_correct_organization(
                user, lapsi.vakatoimija.organisaatio_oid, toimipaikka_qs
            ):
                raise PermissionDenied({"errors": [ErrorMessages.MA010.value]})
...
class PaosToimintaViewSet(
    ...
    def get_paos_toiminta_is_active_q(self, validated_data):
        ...
        if "paos_organisaatio" in validated_data and validated_data["paos_organisaatio"] is not None:
            ...
            paos_toiminta_is_active_q = Q(oma_organisaatio=validated_data["paos_organisaatio"]) & Q(
                paos_toimipaikka__vakajarjestaja=validated_data["oma_organisaatio"]
            )
            ...
            paos_toiminta_is_active_q = Q(oma_organisaatio=validated_data["paos_toimipaikka"].vakajarjestaja) & Q(
                paos_organisaatio=validated_data["oma_organisaatio"]
            )

            toimipaikka_jarjestamismuoto_koodit = validated_data["paos_toimipaikka"].jarjestamismuoto_koodi
            toimipaikka_jarjestamismuoto_koodit = [
                jarjestamismuoto_koodi.lower() for jarjestamismuoto_koodi in toimipaikka_jarjestamismuoto_koodit
            ]
    ...
    def get_tuottaja_organisaatio(self, validated_data):
        paos_toimipaikka = validated_data.get("paos_toimipaikka")
        if paos_toimipaikka:
            return paos_toimipaikka.vakajarjestaja

        return validated_data["oma_organisaatio"]

    def get_existing_paos_toiminta_obj(self, validated_data):
        if "paos_toimipaikka" in validated_data:
            paos_toiminta_query = Q(oma_organisaatio=validated_data["oma_organisaatio"]) & Q(
                paos_toimipaikka=validated_data["paos_toimipaikka"]
            )
        else:
            paos_toiminta_query = Q(oma_organisaatio=validated_data["oma_organisaatio"]) & Q(
                paos_organisaatio=validated_data["paos_organisaatio"]
    ...
    def perform_destroy(self, instance):
        ...
        with transaction.atomic():
            if instance.paos_organisaatio is not None:
                ...
                if not Varhaiskasvatussuhde.objects.filter(
                    toimipaikka=instance.paos_toimipaikka,
                    varhaiskasvatuspaatos__lapsi__oma_organisaatio=instance.oma_organisaatio,
                ).exists():
                    is_delete = True
...
@auditlogclass
@request_log_viewset_decorator_factory()
class PaosOikeusViewSet(
    IncreasedModifyThrottleMixin, PermissionListMixin, GenericViewSet, UpdateModelMixin, ListModelMixin, RetrieveModelMixin
):
    """
    list:
    ...
    def perform_update(self, serializer):
        ...
        if (
            validated_data["tallentaja_organisaatio"] != paos_oikeus_obj.jarjestaja_kunta_organisaatio
            and validated_data["tallentaja_organisaatio"] != paos_oikeus_obj.tuottaja_organisaatio
        ):
...
@auditlogclass
@request_log_viewset_decorator_factory()
class TukipaatosViewSet(IncreasedModifyThrottleMixin, ObjectByTunnisteMixin, PermissionListMixin, ModelViewSet):
    """
    list:
        Retrieve all support decision records.

    create:
        Create a new support decision record.

    delete:
        Delete a support decision record.

    retrieve:
        Retrieve an individual support decision record.

    partial_update:
        Update one or more fields of a support decision entry.

    update:
        Update all fields of a single support decision entry.
    """

    serializer_class = TukipaatosSerializer
    permission_classes = (
        CustomModelPermissions,
        CustomObjectPermissions,
    )
    filter_backends = (DjangoFilterBackend,)
    filterset_class = filters.TukipaatosFilter
    queryset = Tukipaatos.objects.all().order_by("id")
    ...
    def perform_create(self, serializer):
        with transaction.atomic():
            ...
            assign_tukipaatos_permissions(instance)
    ...
    def lists(self, request):
        tilastointi_pvm_list = Tukipaatos.objects.values_list("tilastointi_pvm", flat=True).distinct().order_by("tilastointi_pvm")

        today = datetime.date.today()
        timeframes = TukipaatosAikavali.objects.all().order_by("tilastointi_pvm")
        alkamis_pvm, paattymis_pvm, next_tilastointi_pvm = get_next_timeframe(today, timeframes)

        ikaryhma_codes = list(
            Z2_Code.objects.filter(koodisto__name=Koodistot.ikaryhma_koodit.value)
            .order_by("code_value")
            .values_list("code_value", flat=True)
        )

        tuentaso_codes = list(
            Z2_Code.objects.filter(koodisto__name=Koodistot.tuentaso_koodit.value)
            .order_by("code_value")
            .values_list("code_value", flat=True)
        )
        ...
        return Response(
            {
                "ikaryhma_koodi_list": ikaryhma_codes,
                "tuentaso_koodi_list": tuentaso_codes,
                "tilastointi_pvm_list": tilastointi_pvm_list,
                "next_aikavali": {
                    "alkamis_pvm": alkamis_pvm,
                    "paattymis_pvm": paattymis_pvm,
                    "tilastointi_pvm": next_tilastointi_pvm,
                },
            }
        )
...
class NestedOrganisaatioYhteenvetoViewSet(GenericViewSet, ListModelMixin):
    ...
    def list(self, request, *args, **kwargs):
        ...
        if not data:
            ...
            vuokrattu_henkilosto_data = self.get_vuokrattu_henkilosto_data()
            tilastointi_pvm = get_latest_tilastointi_pvm(vakajarjestaja_obj)
            is_kunta = vakajarjestaja_obj.yritysmuoto in YRITYSMUOTO_KUNTA
            data = {
                ...
                "vuokrattu_henkilosto_maara_kuluva_vuosi": vuokrattu_henkilosto_data["tyontekijamaara_sum"],
                "vuokrattu_henkilosto_tunnit_kuluva_vuosi": vuokrattu_henkilosto_data["tuntimaara_sum"],
                "is_kunta": is_kunta,
                "timestamp": timezone.now(),
                **self.get_tuen_tiedot_data(tilastointi_pvm),
            }
    ...
    def get_lapsi_data(self):
        ...
        """
        organisaatio_filter = (
            Q(vakatoimija_id=self.organisaatio_id)
            | Q(oma_organisaatio_id=self.organisaatio_id)
            | Q(paos_organisaatio_id=self.organisaatio_id)
        )
        active_filter = get_active_filter(self.today, prefix="varhaiskasvatuspaatokset") & get_active_filter(
            self.today, prefix="varhaiskasvatuspaatokset__varhaiskasvatussuhteet"
        )
        return (
            Lapsi.objects.filter(organisaatio_filter & active_filter)
            ...
            .aggregate(
                ...
                paos_count=Count(
                    "id",
                    distinct=True,
                    filter=(~Q(vakatoimija_id=self.organisaatio_id) & Q(jarjestamismuoto_koodi_lower__in=("jm02", "jm03"))),
                ),
                paivittainen_count=Count("id", distinct=True, filter=Q(varhaiskasvatuspaatokset__paivittainen_vaka_kytkin=True)),
                kokopaivainen_count=Count(
                    "id", distinct=True, filter=Q(varhaiskasvatuspaatokset__kokopaivainen_vaka_kytkin=True)
                ),
            )
        )
    ...
    def get_vakapaatos_count(self):
        ...
        """
        organisaatio_filter = (
            Q(lapsi__vakatoimija_id=self.organisaatio_id)
    ...
    def get_vakasuhde_count(self):
        ...
        """
        organisaatio_filter = (
            Q(varhaiskasvatuspaatos__lapsi__vakatoimija_id=self.organisaatio_id)
    ...
    def get_maksutieto_count(self):
        ...
        organisaatio_filter = Q(huoltajuussuhteet__lapsi__vakatoimija_id=self.organisaatio_id) | Q(
            huoltajuussuhteet__lapsi__oma_organisaatio_id=self.organisaatio_id
        )
    ...
    def get_tyontekija_count(self):
        ...
        active_filter = (
            ...
            & ~Exists(PidempiPoissaolo.objects.filter(self.active_filter, palvelussuhde=OuterRef("palvelussuhteet")))
        )
    ...
    def get_palvelussuhde_data(self):
        ...
        return Palvelussuhde.objects.filter(organisatio_filter & self.active_filter).aggregate(
            count=Count("id", distinct=True), maaraaikainen_count=Count("id", distinct=True, filter=Q(tyosuhde_koodi=2))
        )
    ...
    def get_tyoskentelypaikka_data(self):
        ...
        return Tyoskentelypaikka.objects.filter(organisaatio_filter & self.active_filter).aggregate(
            count=Count("id", distinct=True), kelpoinen_count=Count("id", distinct=True, filter=Q(kelpoisuus_kytkin=True))
        )
    ...
    def get_vuokrattu_henkilosto_data(self):
        """
        :return: aggregated VuokrattuHenkilosto related data
        tyontekijamaara_sum: sum of tyontekijamaara
        tuntimaara_sum: tuntimaara for Organisaatio this year
        """
        return VuokrattuHenkilosto.objects.filter(
            kuukausi__year=self.today.year, vakajarjestaja__id=self.organisaatio_id
        ).aggregate(tyontekijamaara_sum=Sum("tyontekijamaara"), tuntimaara_sum=Sum("tuntimaara"))

    def get_tuen_tiedot_paatosmaara(self, tilastointi_pvm, tuentaso_koodi, yksityinen_jarjestaja):
        """
        :return: sum of paatosmaaras for Tukipaatos with given tilastointi_pvm, tuentaso_koodi and yksityinen_jarjestaja
        """
        if tilastointi_pvm is None:
            return None

        return Tukipaatos.objects.filter(
            vakajarjestaja__id=self.organisaatio_id,
            yksityinen_jarjestaja=yksityinen_jarjestaja,
            tuentaso_koodi=tuentaso_koodi,
            tilastointi_pvm=tilastointi_pvm,
        ).aggregate(Sum("paatosmaara"))["paatosmaara__sum"]

    def get_tuen_tiedot_data(self, tilastointi_pvm):
        tilastointi_pvm_str = tilastointi_pvm.strftime(YHTEENVETO_TUEN_TIEDOT_DATE_FORMAT) if tilastointi_pvm else None

        tuen_tiedot_data = {}
        for yksityinen in [False, True]:
            dict_key_prefix = "tuen_tiedot_"
            dict_key_prefix += "yksityinen" if yksityinen else "kunnallinen"
            for tuen_taso_no in ["01", "02", "03"]:
                tuen_taso_code = f"TT{tuen_taso_no}"
                dict_key = f"{dict_key_prefix}_tt_{tuen_taso_no}"
                paatosmaara_value = self.get_tuen_tiedot_paatosmaara(tilastointi_pvm, tuen_taso_code, yksityinen)

                tuen_tiedot_data[dict_key] = paatosmaara_value
                tuen_tiedot_data[f"{dict_key}_date"] = tilastointi_pvm_str if paatosmaara_value is not None else None
...
@auditlogclass
class NestedVarhaiskasvatussuhdeToimipaikkaViewSet(ParentObjectMixin, PermissionListMixin, GenericViewSet, ListModelMixin):
    """
    list:
...
class NestedVarhaiskasvatuspaatosViewSet(ParentObjectMixin, PermissionListMixin, GenericViewSet, ListModelMixin):
    ...
    def list(self, request, *args, **kwargs):
        ...
        self.queryset = Varhaiskasvatuspaatos.objects.using(settings.READER_DB).filter(lapsi=lapsi_obj).order_by("id")
        return super().list(request, *args, **kwargs)
...
class NestedLapsiKoosteViewSet(ObjectByTunnisteMixin, GenericViewSet, ListModelMixin):
    ...
    queryset = Lapsi.objects.using(settings.READER_DB).all().order_by("id")
    ...
    def list(self, request, *args, **kwargs):
        ...
        lapsi_data = {
            ...
            "muutos_pvm": lapsi.muutos_pvm,
            "luonti_pvm": lapsi.luonti_pvm,
        }
        ...
        if lapsi.vakatoimija:
            ...
            tallentaja_organisaatio_oid = (
                ...
                .values_list("tallentaja_organisaatio__organisaatio_oid", flat=True)
                .first()
            )
        ...
        vakatiedot_organization_groups_qs = user_permission_groups_in_organizations(
            user,
            (
        ...
        vakapaatokset = (
            Varhaiskasvatuspaatos.objects.using(settings.READER_DB).filter(vakapaatos_filter).distinct().order_by("-alkamis_pvm")
        )
        ...
        vakasuhteet = (
            Varhaiskasvatussuhde.objects.using(settings.READER_DB).filter(vakasuhde_filter).distinct().order_by("-alkamis_pvm")
        )
        lapsi_data["varhaiskasvatussuhteet"] = vakasuhteet

        # Get maksutiedot
        maksutieto_filter = Q(huoltajuussuhteet__lapsi=lapsi)
        huoltajatiedot_organization_groups_qs = user_permission_groups_in_organization(
            user,
            oma_organisaatio_oid,
            (
                Z4_CasKayttoOikeudet.PALVELUKAYTTAJA,
        ...
        maksutiedot = Maksutieto.objects.using(settings.READER_DB).filter(maksutieto_filter).distinct().order_by("-alkamis_pvm")
        lapsi_data["maksutiedot"] = maksutiedot

        # Save data access log
        save_data_access_log(user, DataAccessType.KOOSTE.value, henkilo_obj=lapsi.henkilo)

        return Response(self.get_serializer(lapsi_data).data)
...
class NestedLapsenVarhaiskasvatussuhdeViewSet(ParentObjectMixin, PermissionListMixin, GenericViewSet, ListModelMixin):
    ...
    def list(self, request, *args, **kwargs):
        lapsi_obj = self.get_parent_object()
        self.queryset = (
            Varhaiskasvatussuhde.objects.using(settings.READER_DB).filter(varhaiskasvatuspaatos__lapsi=lapsi_obj).order_by("id")
        )
        return super().list(request, *args, **kwargs)
...
class NestedLapsiMaksutietoViewSet(ParentObjectMixin, GenericViewSet, ListModelMixin):
    ...
    def list(self, request, *args, **kwargs):
        lapsi_obj = self.get_parent_object()
        self.queryset = (
            Maksutieto.objects.using(settings.READER_DB).filter(huoltajuussuhteet__lapsi=lapsi_obj).distinct().order_by("id")
        )
        return super().list(request, *args, **kwargs)
...
class NestedVakajarjestajaPaosToimijatViewSet(ParentObjectMixin, GenericViewSet, ListModelMixin):
    ...
    def list(self, request, *args, **kwargs):
        ...
        self.queryset = PaosToiminta.objects.filter(
            Q(voimassa_kytkin=True) & Q(oma_organisaatio=vakajarjestaja_obj, paos_organisaatio__isnull=False)
        ).order_by("id")
        return super().list(request, *args, **kwargs)
...
class NestedVakajarjestajaPaosToimipaikatViewSet(ParentObjectMixin, GenericViewSet, ListModelMixin):
    ...
    def list(self, request, *args, **kwargs):
        ...
        paos_toiminta_qs = PaosToiminta.objects.filter(
            Q(voimassa_kytkin=True) & Q(oma_organisaatio=vakajarjestaja_obj, paos_toimipaikka__isnull=False)
        ...
        paos_toiminta_qs = paos_toiminta_qs.filter(
            pk__in=Subquery(paos_toiminta_qs.distinct("paos_toimipaikka__id").values("pk"))
...
class HealthViewSet(ViewSet):
    ...
    def list(self, request):
        is_workers_active, active_worker_count = check_active_celery_workers()
        health_response = dict(
            workers_active=is_workers_active,
            active_worker_count=active_worker_count,
        )

        if not is_workers_active:
...
class HealthVakajarjestajatViewSet(GenericViewSet, ListModelMixin):
    """
    list:
    ...
    def list(self, request, *args, **kwargs):
        return Response({"number_of_vakajarjestajat": self.get_queryset().count()})
