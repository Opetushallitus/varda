...
from varda.misc import (
    CustomServerErrorException, encrypt_string, hash_string, update_painotus_kytkin, get_next_timeframe,
    is_within_date_range, check_active_celery_workers)
...
from varda.models import (Organisaatio, PidempiPoissaolo, Toimipaikka, ToiminnallinenPainotus, KieliPainotus, Henkilo,
                          ...
                          Palvelussuhde, Taydennyskoulutus, Tyoskentelypaikka, VuokrattuHenkilosto, Tutkinto,
                          MaksutietoHuoltajuussuhde, Tukipaatos, TukipaatosAikavali, Z2_Code)
...
from varda.permissions import (assign_henkilo_permissions, assign_kielipainotus_permissions, assign_lapsi_permissions,
                               ...
                               FrontendAccessOnly, remove_paos_toiminta_permissions,
                               ...
                               user_has_tallentaja_permission_in_organization, ToimipaikkaPermissions, is_oph_staff,
                               user_permission_groups_in_organizations, user_permission_groups_in_organization,
                               CustomObjectPermissions, VAKA_LAPSI_GROUPS, assign_tukipaatos_permissions,
...
from varda.serializers import (ExternalPermissionsSerializer, GroupSerializer, UpdateHenkiloWithOidSerializer,
                               ...
                               ToimipaikkaV2Serializer, ToimipaikkaKoosteV2Serializer, ToimipaikkaUpdateV2Serializer,
...
class ExternalPermissionsViewSet(GenericViewSet, CreateModelMixin):
    ...
    permission_classes = (permissions.AllowAny, FrontendAccessOnly,)
...
class HenkiloViewSet(IncreasedModifyThrottleMixin, GenericViewSet, RetrieveModelMixin, CreateModelMixin):
    ...
    def validate_names_match(self, henkilo, etunimet, sukunimi):
        ...
        if set(etunimet_list_lowercase) & set(henkilo_etunimet_list_lowercase) or \
                sukunimi.lower() == henkilo.sukunimi.lower():
    ...
    def validate_henkilo_oid(self, henkilo_data, henkilo_oid):
        ...
        if henkilo_data and not henkilo_oid:
            if not henkilo_data.get('oidHenkilo'):
                ...
                raise ValidationError({'henkilo_oid': [ErrorMessages.HE018.value, ]})
    ...
    @swagger_auto_schema(responses={
        status.HTTP_200_OK: HenkiloSerializer(many=False),
        status.HTTP_201_CREATED: HenkiloSerializer(many=False)})
    def create(self, request, *args, **kwargs):
    ...
    def perform_create(self, serializer):
        ...
        henkilo_data = (self._get_henkilo_data_by_henkilotunnus(
            validated_data, henkilotunnus, etunimet, kutsumanimi, sukunimi)
            if henkilotunnus else
...
@auditlogclass
@request_log_viewset_decorator_factory()
class TukipaatosViewSet(IncreasedModifyThrottleMixin, ObjectByTunnisteMixin, PermissionListMixin,
                        ModelViewSet):
    """
    list:
    ...
    def lists(self, request):
        ...
        ikaryhma_codes = list(
            Z2_Code.objects.filter(koodisto__name=Koodistot.ikaryhma_koodit.value).order_by('code_value').values_list(
                'code_value', flat=True))

        tuentaso_codes = list(
            Z2_Code.objects.filter(koodisto__name=Koodistot.tuentaso_koodit.value).order_by('code_value').values_list(
                'code_value', flat=True))
...
class NestedOrganisaatioYhteenvetoViewSet(GenericViewSet, ListModelMixin):
    ...
    def list(self, request, *args, **kwargs):
        ...
        if not data:
            ...
            tt_args = get_next_timeframe(self.today, TukipaatosAikavali.objects.all())
            ...
            data = {
                ...
                'tuen_tiedot_kunnallinen_tt_01': self.get_tuen_tiedot_kunnallinen_tt_01(*tt_args),
                'tuen_tiedot_kunnallinen_tt_02': self.get_tuen_tiedot_kunnallinen_tt_02(*tt_args),
                'tuen_tiedot_kunnallinen_tt_03': self.get_tuen_tiedot_kunnallinen_tt_03(*tt_args),
                'tuen_tiedot_yksityinen_tt_01': self.get_tuen_tiedot_yksityinen_tt_01(*tt_args),
                'tuen_tiedot_yksityinen_tt_02': self.get_tuen_tiedot_yksityinen_tt_02(*tt_args),
                'tuen_tiedot_yksityinen_tt_03': self.get_tuen_tiedot_yksityinen_tt_03(*tt_args),
                'is_active_timeframe': is_within_date_range(self.today, tt_args[0], tt_args[1]),
                'is_kunta': is_kunta,
    ...
    def get_vuokrattu_henkilosto_data(self):
        ...
        """
        return (VuokrattuHenkilosto.objects
    ...
    def get_tuen_tiedot_kunnallinen_tt_01(self, tt_start_date, tt_end_date, statistical_date):
        return self.get_tuen_tiedot(tt_start_date, tt_end_date, statistical_date, 'TT01', False)

    def get_tuen_tiedot_kunnallinen_tt_02(self, tt_start_date, tt_end_date, statistical_date):
        return self.get_tuen_tiedot(tt_start_date, tt_end_date, statistical_date, 'TT02', False)

    def get_tuen_tiedot_kunnallinen_tt_03(self, tt_start_date, tt_end_date, statistical_date):
        return self.get_tuen_tiedot(tt_start_date, tt_end_date, statistical_date, 'TT03', False)

    def get_tuen_tiedot_yksityinen_tt_01(self, tt_start_date, tt_end_date, statistical_date):
        return self.get_tuen_tiedot(tt_start_date, tt_end_date, statistical_date, 'TT01', True)

    def get_tuen_tiedot_yksityinen_tt_02(self, tt_start_date, tt_end_date, statistical_date):
        return self.get_tuen_tiedot(tt_start_date, tt_end_date, statistical_date, 'TT02', True)

    def get_tuen_tiedot_yksityinen_tt_03(self, tt_start_date, tt_end_date, statistical_date):
        return self.get_tuen_tiedot(tt_start_date, tt_end_date, statistical_date, 'TT03', True)

    def get_tuen_tiedot(self, tt_start_date, tt_end_date, statistical_date, tuentaso_koodi, yksityinen_jarjestaja):
        """
        :return: number of paatosmaara for Tukipaatos with given tuentaso_koodi and yksityinen_jarjestaja
        """
        if is_within_date_range(self.today, tt_start_date, tt_end_date):
            return Tukipaatos.objects.filter(
                vakajarjestaja__id=self.organisaatio_id, yksityinen_jarjestaja=yksityinen_jarjestaja,
                tuentaso_koodi=tuentaso_koodi,
                tilastointi_pvm=statistical_date
...
class NestedLapsiKoosteViewSet(ObjectByTunnisteMixin, GenericViewSet, ListModelMixin):
    ...
    def list(self, request, *args, **kwargs):
        ...
        vakatiedot_organization_groups_qs = user_permission_groups_in_organizations(
            user, (oma_organisaatio_oid, paos_organisaatio_oid,), (
