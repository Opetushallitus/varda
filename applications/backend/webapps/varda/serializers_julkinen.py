...
class KoodistotSerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, instance):
        ...
        if instance['name'] == 'posti':
            # Filter out codes where code_value starts with '000' or ends with other than '0'
            # ('00099' is not filtered out)
            filtered_codes = []
            for code in representation['codes']:
                if code['code_value'] == '00099':
                    pass
                elif code['code_value'].startswith('000') or not code['code_value'].endswith('0'):
                    continue
