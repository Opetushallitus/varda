...
from varda.serializers_admin import (
    AnonymisointiYhteenvetoSerializer, SetPaattymisPvmPostSerializer, SetPaattymisPvmGetSerializer)
...
class MuistutusReportViewSet(GenericViewSet):
    ...
    def get_messagelog_report_csv_by_dates(self, request, alkupvm, loppupvm):
        ...
        msg_logs = Z11_MessageLog.objects.filter(
        ...
        ).select_related('organisaatio').order_by('organisaatio__nimi').distinct('organisaatio__nimi')
