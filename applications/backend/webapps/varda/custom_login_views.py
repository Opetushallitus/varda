...
class RatelimitedLoginView(views.LoginView):
    ...
    @method_decorator(ratelimit(key="header:x-real-ip", rate=settings.CUSTOM_LOGIN_RATELIMIT["header_x_real_ip"], block=False))
    @method_decorator(
        ratelimit(key="post:username", rate=settings.CUSTOM_LOGIN_RATELIMIT["post_form"], method=["POST"], block=False)
    )
    @method_decorator(
        ratelimit(key="post:password", rate=settings.CUSTOM_LOGIN_RATELIMIT["post_form"], method=["POST"], block=False)
    )
    ...
    def form_valid(self, form):
        ...
        if user.is_staff and "HTTP_X_REAL_IP" in request.META and check_if_ip_not_allowed_for_admin_access(request):
            logger.error(
                "Security warning! "
                + user.username
                + " tried to log in from disallowed IP: HTTP_X_REAL_IP="
