...
def add_henkilo_to_oppijanumerorekisteri(etunimet, kutsumanimi, sukunimi, henkilotunnus=None):
    default_henkilo_query_error_result = {"new_henkilo": False, "result": None}
    if henkilotunnus:
        new_henkilo_oid_in_oppijanumerorekisteri = _add_henkilo_to_oppijanumerorekisteri(
            etunimet, kutsumanimi, sukunimi, henkilotunnus=henkilotunnus
        )
    else:
...
def _add_henkilo_to_oppijanumerorekisteri(etunimet, kutsumanimi, sukunimi, henkilotunnus=None):
    ...
    """
    new_henkilo = {"etunimet": etunimet, "kutsumanimi": kutsumanimi, "sukunimi": sukunimi}
    if henkilotunnus:
        new_henkilo["hetu"] = henkilotunnus

    reply_msg = post_json_to_external_service(
        SERVICE_NAME, "/henkilo/", json.dumps(new_henkilo), status.HTTP_201_CREATED, auth=True, reply_type="text"
    )
    if reply_msg["is_ok"]:
        ...
        logger.error(
            "Couldn't add a new henkilo to Oppijanumerorekisteri. Henkilotunnus: {}. Id: {}. Added by: {}".format(
                henkilotunnus_prefix_hided, henkilo_id, added_by
            )
        )
        return None


def get_henkilo_data_by_oid(henkilo_oid):
    """
    ...
    """
    response = get_json_from_external_service(SERVICE_NAME, f"/henkilo/{henkilo_oid}/master")
    response_body = response["json_msg"]
    if not response["is_ok"]:
        logger.error(f"Could not fetch henkilo data for {henkilo_oid}: {response_body}")
        return None
...
def get_henkilot_changed_since(start_datetime, offset, amount=5000):
    ...
    return get_json_from_external_service(SERVICE_NAME, url, large_query=True)


def get_huoltajuussuhde_changed_child_oids(start_datetime, offset, amount=5000):
    """
    Fetch changed huoltajuussuhteet from oppijanumerorekisteri.
    :param start_datetime: Date time (in ISO 8601) since when we fetch changes
    :param offset: position of the paginated (and ordered) dataset
    :param amount: How many (max) are fetched
...
def fetch_changed_henkilot(start_datetime):
    ...
    while True:
        ...
        if not changed_henkilo_oids_msg["is_ok"]:
            ...
            """
            logger.error("Could not fetch the changed henkilot, {}, {}, {}.".format(start_datetime, offset, amount))
        ...
        if len_of_fetched_henkilo_oids < amount:  # We got everything. Return the list of all changed henkilo oids
            ...
            loop_number += 1
            if loop_number % 20 == 0:
                logger.warning("Very large queries to changed henkilot. Current amount: {}".format(loop_number * amount))
            continue  # We (probably) didn't get everything yet. Fetch another batch.
        else:
            logger.error(
                "Changed henkilot: We got more than we requested. Received: {}, Requested: {}".format(
                    len_of_fetched_henkilo_oids, amount
                )
            )
            return get_reply_json(is_ok=False, json_msg=None)
...
def fetch_henkilo_data_for_oid_list(oid_list):
    ...
    reply_msg = post_json_to_external_service(
        SERVICE_NAME, "/henkilo/henkilotByHenkiloOidList", json.dumps(oid_list), status.HTTP_200_OK, large_query=True
    )
...
def fetch_yhteystiedot(henkilo_oid_list):
    """
    Example respone:
    [
        {
            ...
            "yhteystiedotRyhma": [
                {
                    ...
                    "yhteystieto": [
                        {
                            "yhteystietoTyyppi": "YHTEYSTIETO_SAHKOPOSTI",
                            "yhteystietoArvo": "test@example.com"
                        }
                    ]
                }
            ]
    ...
    :return: response body if request is successful, else None
    """
    post_body = {"henkiloOids": henkilo_oid_list}
    response = post_json_to_external_service(
        SERVICE_NAME, "/henkilo/yhteystiedot", json.dumps(post_body), status.HTTP_200_OK, large_query=True
    )
    response_body = response["json_msg"]
    if not response["is_ok"]:
        logger.error(f"Error fetching yhteystiedot: {response_body}")
        return None
    return response_body
