...
def get_organisaatiopalvelu_info(organisaatio_oid):
    url = f"{ORGANISAATIOPALVELU_API}{organisaatio_oid}"

    response_wrapper = get_json_from_external_service(SERVICE_NAME, url, auth=True)
    if not response_wrapper["is_ok"]:
        return None

    response = response_wrapper["json_msg"]

    # Current default value
    kieli = "fi"
    if ytjkieli_value := response.get("ytjkieli", None):
        try:
            kieli = ytjkieli_value.split("_")[1].split("#")[0]
        except IndexError:
            # Use default value
            pass
    ...
    try:
        ...
        kayntiosoite_value = response.get("kayntiosoite", {})
        postiososoite_value = response.get("postiosoite", {})
        # Get yritysmuoto code from koodisto since API returns translated value and not the code itself
        yritysmuoto_code = Z2_Code.objects.filter(
            koodisto__name=Koodistot.yritysmuoto_koodit.value, translations__name=response.get("yritysmuoto", None)
        ).first()
        ...
        return {
            ...
            "kayntiosoite": kayntiosoite_value.get("osoite", ""),
            "kayntiosoite_postinumero": kayntiosoite_value.get("postinumeroUri", "_").split("_")[1],
            "kayntiosoite_postitoimipaikka": kayntiosoite_value.get("postitoimipaikka", ""),
            "postiosoite": postiososoite_value.get("osoite", ""),
            "postinumero": postiososoite_value.get("postinumeroUri", "_").split("_")[1],
            "postitoimipaikka": postiososoite_value.get("postitoimipaikka", ""),
            "ytjkieli": kieli,
        ...
        logger.error(f"Could not parse organisaatio data for Organisaatio {organisaatio_oid} with cause: {e}")
        return None
...
def check_if_toimipaikka_exists_by_name(toimipaikka_name, parent_oid):
    ...
    """
    http_url_suffix = (
        ORGANISAATIOPALVELU_API
        + "hae?aktiiviset=true&suunnitellut=true&lakkautetut=true&oidRestrictionList="
        + parent_oid
        + "&organisaatiotyyppi=organisaatiotyyppi_08&searchStr="
        + toimipaikka_name
    )
...
def create_organisaatio(organisaatio_json):
    ...
    logger.info("Creating new organisation into Org-palvelu.")
    response = post_json_to_external_service(
        "organisaatio-service", ORGANISAATIOPALVELU_API, organisaatio_json, expected_status_code
    )
    if response["is_ok"]:
        oid = response["json_msg"]["organisaatio"]["oid"]
        logger.info(f"New organisation created successfully into Org-palvelu. ({oid})")
        return {"toimipaikka_created": True, "organisaatio_oid": oid}
    else:
        logger.warning("Creating new organisation failed into Org-palvelu.")
...
def update_organisaatio(organisaatio_oid, organisaatio_update_json, internal_id=None):
    ...
    response = put_json_to_external_service(
        "organisaatio-service", http_url_suffix_put, organisaatio_update_json, expected_status_code
    )
