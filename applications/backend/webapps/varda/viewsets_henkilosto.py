...
from varda.cache import delete_cache_keys_related_model, get_object_ids_user_has_permissions, get_queryset_count_cache_key
...
from varda.misc_queries import get_queryset_count
...
from varda.permissions import (
    assign_palvelussuhde_permissions,
    assign_pidempi_poissaolo_permissions,
    assign_taydennyskoulutus_permissions,
    assign_vuokrattu_henkilosto_permissions,
    assign_tutkinto_permissions,
    assign_tyontekija_permissions,
    assign_tyoskentelypaikka_permissions,
    CustomModelPermissions,
    delete_object_permissions,
    is_correct_taydennyskoulutus_tyontekija_permission,
    filter_authorized_taydennyskoulutus_tyontekijat_list,
...
from varda.serializers_henkilosto import (
    TaydennyskoulutusCreateV2Serializer,
    TaydennyskoulutusUpdateV2Serializer,
    TaydennyskoulutusV2Serializer,
    TyoskentelypaikkaSerializer,
    PalvelussuhdeSerializer,
    PidempiPoissaoloSerializer,
    VuokrattuHenkilostoSerializer,
    TutkintoSerializer,
    TyontekijaSerializer,
    TyoskentelypaikkaUpdateSerializer,
    TaydennyskoulutusSerializer,
...
class TyontekijaViewSet(IncreasedModifyThrottleMixin, ObjectByTunnisteMixin, PermissionListMixin, ModelViewSet):
    ...
    def return_tyontekija_if_already_created(self, validated_data, toimipaikka_oid):
        ...
        if tyontekija_obj:
            ...
            if (
                validated_data["lahdejarjestelma"] != tyontekija_obj.lahdejarjestelma
                or validated_data.get("tunniste", None) != tyontekija_obj.tunniste
                or validated_data.get("sahkopostiosoite", None) != tyontekija_obj.sahkopostiosoite
            ):
    ...
    def delete_all(self, request, pk=None):
        ...
        with transaction.atomic():
            for object_list in [tyoskentelypaikka_qs, pidempi_poissaolo_qs, palvelussuhde_qs, tutkinto_qs, taydennyskoulutus_qs]:
...
class NestedTyontekijaKoosteViewSet(ObjectByTunnisteMixin, GenericViewSet, ListModelMixin):
    ...
    queryset = Tyontekija.objects.using(settings.READER_DB).all().order_by("id")
    ...
    def get_object(self):
        ...
        if (
            user.has_perm("view_tyontekija", tyontekija)
            or Tyontekija.objects.using(settings.READER_DB)
            .filter(Q(pk=tyontekija.id) & tyontekija_taydennyskoulutus_filters)
            .exists()
        ):
            return tyontekija
        else:
    ...
    def list(self, request, *args, **kwargs):
        ...
        tyontekija_data = {"tyontekija": tyontekija}
        ...
        palvelussuhteet = (
            Palvelussuhde.objects.using(settings.READER_DB).filter(palvelussuhde_filter).distinct().order_by("-alkamis_pvm")
        )
        ...
        if not is_superuser_or_oph_staff and not taydennyskoulutus_organization_groups_qs.exists():
            ...
            if not user_belongs_to_correct_groups(
                user,
                tyontekija.vakajarjestaja,
        ...
        taydennyskoulutukset = (
            ...
            .annotate(
                contains_other_tyontekija=Exists(
                    TaydennyskoulutusTyontekija.objects.filter(taydennyskoulutus=OuterRef("taydennyskoulutus")).exclude(
                        tyontekija=tyontekija
                    )
                ),
                tehtavanimike_koodi_lower=Lower(F("tehtavanimike_koodi")),
                read_only=Case(
                    When(tehtavanimike_koodi_lower__in=available_tehtavanimike_codes, then=Value(False)),
        ...
        tutkinnot = set(Tutkinto.objects.using(settings.READER_DB).filter(tutkinto_filter).distinct().order_by("-luonti_pvm"))
...
class VuokrattuHenkilostoViewSet(IncreasedModifyThrottleMixin, ObjectByTunnisteMixin, PermissionListMixin, ModelViewSet):
    """
    list:
...
class TutkintoViewSet(
    ...
    def perform_destroy(self, tutkinto):
        ...
        palvelussuhde_condition = Q(
            tyontekija__vakajarjestaja=vakajarjestaja,
...
class TyoskentelypaikkaViewSet(IncreasedModifyThrottleMixin, ObjectByTunnisteMixin, PermissionListMixin, ModelViewSet):
    ...
    def _validate_relation_to_taydennyskoulutus(self, tyoskentelypaikka, error_message):
        ...
        tyoskentelypaikka_qs = Tyoskentelypaikka.objects.filter(
            palvelussuhde__tyontekija=tyontekija, tehtavanimike_koodi=tehtavanimike
...
class TaydennyskoulutusViewSet(IncreasedModifyThrottleMixin, ObjectByTunnisteMixin, PermissionListMixin, ModelViewSet):
    ...
    def tyontekija_list(self, request, *args, **kwargs):
        ...
        """
        user = request.user
        queryset = Tyontekija.objects.using(settings.READER_DB).all().prefetch_related("palvelussuhteet").order_by("id")
    ...
    def perform_update(self, serializer):
        ...
        if "taydennyskoulutukset_tyontekijat" in validated_data:
            ...
            is_correct_taydennyskoulutus_tyontekija_permission(
                user, taydennyskoulutus.taydennyskoulutukset_tyontekijat.all(), throws=True
            )
