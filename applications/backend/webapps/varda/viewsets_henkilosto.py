...
class NestedTyontekijaKoosteViewSet(ObjectByTunnisteMixin, GenericViewSet, ListModelMixin):
    ...
    def get_object(self):
        ...
        if (
            ...
            or Tyontekija.objects.filter(Q(pk=tyontekija.id) & tyontekija_taydennyskoulutus_filters).exists()
        ):
            return tyontekija
        else:
            raise Http404
    ...
    def list(self, request, *args, **kwargs):
        ...
        taydennyskoulutukset = (
            TaydennyskoulutusTyontekija.objects.filter(taydennyskoulutus_filter)
            .annotate(
        ...
        tutkinnot = set(Tutkinto.objects.filter(tutkinto_filter).distinct().order_by("-luonti_pvm"))
