...
from varda.models import (Henkilo, VuokrattuHenkilosto, Tutkinto, Tyontekija, Organisaatio, Palvelussuhde,
...
class VuokrattuHenkilostoSerializer(serializers.HyperlinkedModelSerializer):
    ...
    def verify_unique_month(self, data, validator):
        vuokrattu_henkilosto_qs = VuokrattuHenkilosto.objects.filter(vakajarjestaja=data['vakajarjestaja'],
...
class TyoskentelypaikkaSerializer(serializers.HyperlinkedModelSerializer):
    ...
    palvelussuhde_tunniste = TunnisteRelatedField(
        object_type=Palvelussuhde,
        parent_field='palvelussuhde',
        prevalidator=validators.validate_tunniste,
        either_required=True)
    toimipaikka = ToimipaikkaPermissionCheckedHLField(
        required=False, allow_null=True, view_name='toimipaikka-detail',
    ...
    def validate(self, data):
        ...
        with ViewSetValidator() as validator:
            validate_tyoskentelypaikka_general(
                validator, data, toimipaikka, palvelussuhde,
...
class TaydennyskoulutusSerializer(serializers.HyperlinkedModelSerializer):
    ...
    def validate(self, data):
        ...
        for tyontekija in data['taydennyskoulutukset_tyontekijat']:
            validate_instance_uniqueness(Taydennyskoulutus, data, ErrorMessages.TK017.value,
...
class TaydennyskoulutusUpdateSerializer(serializers.HyperlinkedModelSerializer):
    ...
    taydennyskoulutus_tyontekijat_add = tk_tyontekija_serializer(
        write_only=True, required=False, allow_empty=False, many=True)
    taydennyskoulutus_tyontekijat_remove = tk_tyontekija_serializer(
        write_only=True, required=False, allow_empty=False, many=True)
    taydennyskoulutus_tyontekijat_count = serializers.SerializerMethodField()
    ...
    def validate(self, data):
        ...
        if data.get('taydennyskoulutus_tyontekijat_add'):
            ...
            tyontekija_ids = [item['tyontekija'].id for item in tyontekija_list if
                              isinstance(item.get('tyontekija'), Tyontekija)]
            ...
            tyontekija_ids = Tyontekija.objects.filter(
                taydennyskoulutukset__id=instance.id).values_list('id', flat=True)

        for tyontekija_id in tyontekija_ids:
            validate_instance_uniqueness(
                Taydennyskoulutus, data, ErrorMessages.TK017.value,
                instance_id=instance.id, instance_id_m2m=tyontekija_id)
        ...
        with ViewSetValidator() as validator:
            ...
            for tyontekija in tyontekijat_add:
                if instance.taydennyskoulutukset_tyontekijat.filter(
                        tyontekija=tyontekija['tyontekija'].id,
                ...
                validate_instance_uniqueness(
                    Taydennyskoulutus, data, ErrorMessages.TK017.value,
                    instance_id=instance.id, instance_id_m2m=tyontekija['tyontekija'].id)
...
class TyontekijaKoosteTyoskentelypaikkaSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('id', 'toimipaikka_id', 'toimipaikka_oid', 'toimipaikka_nimi', 'tehtavanimike_koodi',
                  ...
                  'tunniste', 'muutos_pvm', 'luonti_pvm')
...
class TyontekijaKoosteTaydennyskoulutusSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('id', 'tehtavanimike_koodi', 'nimi', 'suoritus_pvm', 'koulutuspaivia', 'contains_other_tyontekija',
                  'read_only', 'lahdejarjestelma', 'tunniste', 'muutos_pvm', 'luonti_pvm')
