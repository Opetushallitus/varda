...
from varda.permissions import (
    ...
    is_oph_staff,
    get_available_tehtavanimike_codes_for_user,
)
from varda.related_object_validations import (
    create_date_range,
    date_range_overlap,
    check_if_admin_mutable_object_is_changed,
    check_overlapping_palvelussuhde,
    check_overlapping_tyoskentelypaikka,
    check_overlapping_pidempi_poissaolo,
...
from varda.validators import (
    ...
    validate_paattymispvm_same_or_after_alkamispvm,
...
class VuokrattuHenkilostoSerializer(serializers.HyperlinkedModelSerializer):
    id = serializers.IntegerField(read_only=True)
    vakajarjestaja = OrganisaatioPermissionCheckedHLField(
        view_name="organisaatio-detail", required=False, permission_groups=[Z4_CasKayttoOikeudet.HENKILOSTO_VUOKRATTU_TALLENTAJA]
    )
    vakajarjestaja_oid = OidRelatedField(
        object_type=Organisaatio,
        parent_field="vakajarjestaja",
    ...
    @caching_to_representation("vuokrattuhenkilosto")
    def to_representation(self, instance):
        return super().to_representation(instance)

    def validate(self, data):
        with ViewSetValidator() as validator:
            # Validate only when creating vuokrattu henkilosto
            ...
            # Validate only when updating existing vuokrattu henkilosto
            if self.instance:
                instance = self.instance
    ...
    def verify_unique_month(self, data, validator):
        vuokrattu_henkilosto_qs = VuokrattuHenkilosto.objects.filter(
            vakajarjestaja=data["vakajarjestaja"], kuukausi__year=data["kuukausi"].year, kuukausi__month=data["kuukausi"].month
        )
        if vuokrattu_henkilosto_qs.exists():
    ...
    def validate_date_in_allowed_period(self, data, validator):
        ...
        # VuokrattuHenkilosto can be saved for the current period or in the past
...
class PalvelussuhdeSerializer(TyontekijaOptionalToimipaikkaMixin, serializers.HyperlinkedModelSerializer):
    ...
    tyontekija_tunniste = TunnisteRelatedField(
        object_type=Tyontekija, parent_field="tyontekija", prevalidator=validators.validate_tunniste, either_required=True
    )
        ...
        with ViewSetValidator() as validator:
            if self.instance:
                ...
                if "tyontekija" in data:
                    check_if_admin_mutable_object_is_changed(self.context["request"].user, self.instance, data, "tyontekija")
            else:
            ...
            with validator.wrap():
                ...
                validate_instance_uniqueness(
                    Palvelussuhde, data, ErrorMessages.PS010.value, instance_id=getattr(self.instance, "id", None)
                )
        return data

    def validate_tutkinto(self, tyontekija, tutkinto_koodi, validator):
        if not tyontekija.henkilo.tutkinnot.filter(
            tutkinto_koodi=tutkinto_koodi, vakajarjestaja=tyontekija.vakajarjestaja
        ).exists():
...
class TyoskentelypaikkaSerializer(serializers.HyperlinkedModelSerializer):
    ...
    palvelussuhde_tunniste = TunnisteRelatedField(
        object_type=Palvelussuhde, parent_field="palvelussuhde", prevalidator=validators.validate_tunniste, either_required=True
    )
    toimipaikka = ToimipaikkaPermissionCheckedHLField(
        required=False,
        allow_null=True,
...
def validate_tyoskentelypaikka_general(
    validator, data, toimipaikka, palvelussuhde, kiertava_tyontekija_kytkin, tyoskentelypaikka_id=None
):
    ...
    with validator.wrap():
        ...
        validators.validate_tehtavanimike_koodi(data["tehtavanimike_koodi"], data["alkamis_pvm"], data.get("paattymis_pvm"))

    # force kelpoisuus_kytkin=False for VAKA_AVUSTAJA code
    if data["tehtavanimike_koodi"] == TEHTAVANIMIKE_KOODI_VAKA_AVUSTAJA:
        data["kelpoisuus_kytkin"] = False
...
def validate_tyoskentelypaikka_dates(validated_data, palvelussuhde, validator):
    ...
    if not validators.validate_paivamaara1_after_paivamaara2(
        tyoskentelypaikka_alkamis_pvm, palvelussuhde.alkamis_pvm, can_be_same=True
    ):
        validator.error("alkamis_pvm", ErrorMessages.TA008.value)
    if not validators.validate_paivamaara1_before_paivamaara2(
        tyoskentelypaikka_alkamis_pvm, palvelussuhde.paattymis_pvm, can_be_same=True
    ):
...
def validate_overlapping_kiertavyys(data, palvelussuhde, kiertava_tyontekija_kytkin, validator):
    ...
    related_palvelussuhde_tyoskentelypaikat = Tyoskentelypaikka.objects.filter(
        palvelussuhde=palvelussuhde, kiertava_tyontekija_kytkin=inverse_kiertava_kytkin
    )
...
class TyoskentelypaikkaUpdateSerializer(serializers.HyperlinkedModelSerializer):
    ...
    palvelussuhde_tunniste = TunnisteRelatedField(
        object_type=Palvelussuhde, parent_field="palvelussuhde", prevalidator=validators.validate_tunniste, read_only=True
    )
    ...
    def validate(self, data):
        ...
        with ViewSetValidator() as validator:
            validate_tyoskentelypaikka_general(
                validator, data, toimipaikka, palvelussuhde, kiertava_tyontekija_kytkin, tyoskentelypaikka_id=tyoskentelypaikka.id
            )

        return data


class TyoskentelypaikkaUpdateV2Serializer(TyoskentelypaikkaUpdateSerializer):
    def validate(self, data):
        tyoskentelypaikka = self.instance
        check_if_immutable_object_is_changed(tyoskentelypaikka, data, "kiertava_tyontekija_kytkin", compare_id=False)
...
class PermissionCheckedTaydennyskoulutusTyontekijaListSerializer(serializers.ListSerializer):
    ...
    def _find_tyontekija_by_henkilo_oid(self, validator, henkilo_oid, vakajarjestaja_oid, index, tyontekija=None):
        try:
            tyontekija_by_henkilo_oid = Tyontekija.objects.get(
                henkilo__henkilo_oid=henkilo_oid, vakajarjestaja__organisaatio_oid=vakajarjestaja_oid
            )
...
class NestedTaydennyskoulutusTyontekijaSerializer(serializers.ModelSerializer):
    ...
    lahdejarjestelma = serializers.CharField(
        max_length=2, required=False, validators=[validators.validate_lahdejarjestelma_koodi]
    )
    ...
    def create(self, data):
        ...
        return TaydennyskoulutusTyontekija.objects.create(
            tyontekija=tyontekija, tehtavanimike_koodi=data["tehtavanimike_koodi"], taydennyskoulutus=data["taydennyskoulutus"]
        )
    ...
    def _validate_tehtavanimike_koodi(self, data):
        ...
        with ViewSetValidator() as validator:
            ...
            if not Tyontekija.objects.filter(
                id=tyontekija.id, palvelussuhteet__tyoskentelypaikat__tehtavanimike_koodi=tehtavanimike_koodi
            ).exists():
...
class TaydennyskoulutusSerializer(serializers.HyperlinkedModelSerializer):
    ...
    id = serializers.IntegerField(read_only=True)
    taydennyskoulutus_tyontekijat = tk_tyontekija_serializer(
        required=True, allow_empty=False, many=True, source="taydennyskoulutukset_tyontekijat"
    )
    ...
    def validate(self, data):
        with ViewSetValidator() as validator:
            tyontekija_set = {
                (tyontekija["tyontekija"].id, tyontekija["tehtavanimike_koodi"])
                for tyontekija in data["taydennyskoulutukset_tyontekijat"]
            }
        ...
        # Check if any Tyontekija has duplicate values for nimi, suoritus_pvm, and koulutuspaivia
        for tyontekija in data["taydennyskoulutukset_tyontekijat"]:
            validate_instance_uniqueness(
                Taydennyskoulutus, data, ErrorMessages.TK017.value, instance_id_m2m=tyontekija["tyontekija"].id
            )
        return data
...
class TaydennyskoulutusUpdateSerializer(serializers.HyperlinkedModelSerializer):
    ...
    id = serializers.IntegerField(read_only=True)
    taydennyskoulutus_tyontekijat = tk_tyontekija_serializer(
        required=False, allow_empty=False, many=True, source="taydennyskoulutukset_tyontekijat"
    )
    taydennyskoulutus_tyontekijat_add = tk_tyontekija_serializer(write_only=True, required=False, allow_empty=False, many=True)
    taydennyskoulutus_tyontekijat_remove = tk_tyontekija_serializer(write_only=True, required=False, allow_empty=False, many=True)
    taydennyskoulutus_tyontekijat_count = serializers.SerializerMethodField()
    ...
    def validate(self, data):
        ...
        if data.get("taydennyskoulutus_tyontekijat_add"):
            # Extracting Tyontekija instances when updating through taydennyskoulutus view
            tyontekija_list = data.get("taydennyskoulutus_tyontekijat_add", [])
            tyontekija_ids = [item["tyontekija"].id for item in tyontekija_list if isinstance(item.get("tyontekija"), Tyontekija)]
        elif data.get("taydennyskoulutus_tyontekijat_remove"):
            # No need to check uniqueness when only removing Tyontekija instances
            tyontekija_ids = []
        else:
            # Get Tyontekija instances when updating through tyontekija view
            tyontekija_ids = Tyontekija.objects.filter(taydennyskoulutukset__id=instance.id).values_list("id", flat=True)

        for tyontekija_id in tyontekija_ids:
            validate_instance_uniqueness(
                Taydennyskoulutus, data, ErrorMessages.TK017.value, instance_id=instance.id, instance_id_m2m=tyontekija_id
            )
    ...
    def _validate_tyontekijat_add(self, instance, data):
        ...
        with ViewSetValidator() as validator:
            tyontekijat_add_set = {
                (tyontekija["tyontekija"].id, tyontekija["tehtavanimike_koodi"]) for tyontekija in tyontekijat_add
            }
            ...
            for tyontekija in tyontekijat_add:
                if instance.taydennyskoulutukset_tyontekijat.filter(
                    tyontekija=tyontekija["tyontekija"].id, tehtavanimike_koodi=tyontekija["tehtavanimike_koodi"]
                ).exists():
                ...
                validate_instance_uniqueness(
                    Taydennyskoulutus,
                    data,
                    ErrorMessages.TK017.value,
                    instance_id=instance.id,
                    instance_id_m2m=tyontekija["tyontekija"].id,
                )
    ...
    def _validate_tyontekijat_remove(self, instance, data, is_new_tyontekijat_added):
        if "taydennyskoulutus_tyontekijat_remove" in data:
            with ViewSetValidator() as validator:
                ...
                for tyontekija in data["taydennyskoulutus_tyontekijat_remove"]:
                    if not instance.taydennyskoulutukset_tyontekijat.filter(
                        tyontekija=tyontekija["tyontekija"].id, tehtavanimike_koodi=tyontekija["tehtavanimike_koodi"]
                    ).exists():
                        validator.error("taydennyskoulutus_tyontekijat_remove", ErrorMessages.TK012.value)
                if (
                    len(tyontekijat_remove_set) == instance.taydennyskoulutukset_tyontekijat.count()
                    and not is_new_tyontekijat_added
                ):
    ...
    def _update_tyontekijat(self, instance, validated_data):
        ...
        if "taydennyskoulutus_tyontekijat_remove" in validated_data:
            ...
            for tyontekija_remove in tyontekijat_remove:
                instance.taydennyskoulutukset_tyontekijat.filter(
                    tyontekija=tyontekija_remove["tyontekija"], tehtavanimike_koodi=tyontekija_remove["tehtavanimike_koodi"]
                ).delete()
...
class PermissionCheckedTaydennyskoulutusTyontekijaListV2Serializer(PermissionCheckedTaydennyskoulutusTyontekijaListSerializer):
    def to_internal_value(self, data):
        ...
        taydennyskoulutus_tyontekija_dicts = super(
            PermissionCheckedTaydennyskoulutusTyontekijaListSerializer, self
        ...
        with ViewSetValidator() as validator:
            ...
            for index, taydennyskoulutus_tyontekija in enumerate(taydennyskoulutus_tyontekija_dicts):
                ...
                if validator.messages:
                    ...
                    if (
                        ErrorMessages.TK004.value["error_code"] in error_string
                        or ErrorMessages.TK006.value["error_code"] in error_string
                    ):
    ...
    def _validate_tehtavanimike_koodi(self, data):
        tyontekija = data.get("tyontekija", None)
        tehtavanimike_koodi = data.get("tehtavanimike_koodi", None)
        return Tyontekija.objects.filter(
            id=getattr(tyontekija, "id", None), palvelussuhteet__tyoskentelypaikat__tehtavanimike_koodi=tehtavanimike_koodi
        ).exists()
...
class TaydennyskoulutusUpdateV2Serializer(TaydennyskoulutusUpdateSerializer):
    ...
    taydennyskoulutus_tyontekijat_add = tk_tyontekija_serializer(
        write_only=True, required=False, allow_empty=False, many=True, is_partial_update_field=True
    )
    taydennyskoulutus_tyontekijat_remove = tk_tyontekija_serializer(write_only=True, required=False, allow_empty=False, many=True)
    taydennyskoulutus_tyontekijat_failed = serializers.SerializerMethodField(read_only=True)
...
class TyontekijaKoosteHenkiloSerializer(serializers.ModelSerializer):
    class Meta:
        model = Henkilo
        fields = ("id", "etunimet", "sukunimi", "henkilo_oid", "turvakielto", "muutos_pvm", "luonti_pvm")
...
class TyontekijaKoostePidempiPoissaoloSerializer(serializers.ModelSerializer):
    class Meta:
        model = PidempiPoissaolo
        fields = ("id", "alkamis_pvm", "paattymis_pvm", "lahdejarjestelma", "tunniste", "muutos_pvm", "luonti_pvm")
...
def _get_permission_checked_henkilosto_tyontekija_objects(serializer_instance, queryset):
    ...
    tyontekija_groups_qs = user_permission_groups_in_organization(
        user,
        vakajarjestaja_oid,
        [Z4_CasKayttoOikeudet.HENKILOSTO_TYONTEKIJA_KATSELIJA, Z4_CasKayttoOikeudet.HENKILOSTO_TYONTEKIJA_TALLENTAJA],
    )
