...
def update_message_targets_and_paakayttaja_status():
    ...
    for paakayttaja_data in paakayttaja_list:
        for organisaatio_id in paakayttaja_data['organisaatio_id_set']:
            ...
            (Aikaleima.objects
             .filter(avain=AikaleimaAvain.NO_PAAKAYTTAJA.value, organisaatio_id=organisaatio_id)
             .delete())
            ...
            if email := yhteystieto_data['email']:
                ...
                Z11_MessageTarget.objects.get_or_create(
                    organisaatio_id=organisaatio_id, user_type=Kayttajatyyppi.PAAKAYTTAJA.value, email=email,
    ...
    for organisaatio in Organisaatio.vakajarjestajat.exclude(id__in=organisaatio_id_set_with_paakayttaja):
        ...
        Aikaleima.objects.get_or_create(avain=AikaleimaAvain.NO_PAAKAYTTAJA.value, organisaatio_id=organisaatio.id,
                                        defaults={'aikaleima': timezone.now()})
...
def send_no_paakayttaja_message():
    ...
    for aikaleima in aikaleima_qs:
        ...
        last_message_qs = (Z11_MessageLog.objects
                           .filter(message_type=message_type, organisaatio=organisaatio, timestamp__gt=frequency_limit))
        if organisaatio.id in active_organisaatio_id_list and not last_message_qs.exists():
            ...
            if email := organisaatio.sahkopostiosoite:
                _send_message(message_type, organisaatio, email, organisaatio.ytjkieli,
...
def send_puutteelliset_tiedot_message():
    ...
    for organisaatio in organisaatio_qs:
        ...
        for viewset, error_dict in [(ErrorReportLapsetViewSet(), vaka_errors,),
                                    (ErrorReportTyontekijatViewSet(), henkilosto_errors,),
                                    (ErrorReportToimipaikatViewSet(), toimipaikka_errors,)]:
        ...
        if vaka_errors or henkilosto_errors or toimipaikka_errors:
            ...
            if message_target_qs.exists():
                error_string_dict = _parse_puutteelliset_tiedot_message(vaka_errors, henkilosto_errors,
                                                                        toimipaikka_errors)
                for message_target in message_target_qs:
                    error_string = error_string_dict.get(message_target.language.upper(),
                                                         error_string_dict[SupportedLanguage.FI.value])
                    _send_message(message_type, organisaatio, message_target.email, message_target.language,
...
def _parse_puutteelliset_tiedot_message(vaka_errors, henkilosto_errors, toimipaikka_errors):
    ...
    error_category_tuple = (
        (vaka_errors, Translation.EMAIL_PUUTTEELLISET_MESSAGE_VARHAISKASVATUS.value,),
        (henkilosto_errors, Translation.EMAIL_PUUTTEELLISET_MESSAGE_HENKILOSTO.value,),
        (toimipaikka_errors, Translation.EMAIL_PUUTTEELLISET_MESSAGE_TOIMIPAIKKA.value,),
    )
    for error_dict, category_title in error_category_tuple:
        ...
        for language in SupportedLanguage.list():
            ...
            for error_key, error_value in error_dict.items():
                ...
                error_template = get_translation(Translation.EMAIL_PUUTTEELLISET_MESSAGE_ERROR_TEMPLATE.value,
                                                 locale=language)
...
def send_no_transfers_message():
    ...
    for organisaatio in organisaatio_qs:
        ...
        last_request_vaka = (Z6_LastRequest.objects
                             .filter(vakajarjestaja=organisaatio, data_category=DataCategory.VARHAISKASVATUS.value,
                                     last_successful__isnull=False)
        ...
        last_request_henkilosto = (Z6_LastRequest.objects
                                   .filter(vakajarjestaja=organisaatio, data_category=DataCategory.HENKILOSTO.value,
                                           last_successful__isnull=False)
                                   .order_by('-last_successful')
                                   .first())
        ...
        if ((last_successful_vaka <= error_limit or last_successful_henkilosto <= error_limit) and
                organisaatio.luonti_pvm < error_limit and not last_message_qs.exists()):
            ...
            if message_target_qs.exists():
                for message_target in message_target_qs:
                    _send_message(message_type, organisaatio, message_target.email, message_target.language,
                                  Translation.EMAIL_NO_TRANSFERS_SUBJECT.value, message_key,
                                  [organisaatio.nimi, last_successful_vaka.strftime(datetime_format),
...
def _send_message(message_type, organisaatio, email, language, subject_key, message_key, message_argument_list):
    ...
    Z11_MessageLog.objects.create(message_type=message_type, organisaatio=organisaatio, email=email,
...
def _send_undelivered_message_to_oph(message_type, organisaatio):
    ...
    logger.info(
        f'Could not deliver {message_type} for Organisaatio with ID {organisaatio.id}, sending notification to {email}'
    )
    message = (get_translation(Translation.EMAIL_COULD_NOT_DELIVER_MESSAGE.value)
               .format(message_type, organisaatio.nimi, organisaatio.id))
    send_mail(
    ...
    Z11_MessageLog.objects.create(message_type=message_type, organisaatio=organisaatio,
                                  email=email, timestamp=timezone.now())
