...
from varda.models import Henkilo, Organisaatio, Toimipaikka
...
class AbstractCustomRelatedField(serializers.Field):
    ...
    def run_validation(self, data=serializers.empty):
        ...
        if object_id and (
            (parent_value_id and object_id != parent_value_id) or (secondary_value_id and object_id != secondary_value_id)
        ):
    ...
    def to_internal_value(self, value):
        ...
        if referenced_object is None:
            ...
            if (
                check_permission and not user.has_perm(check_permission, referenced_object)
            ) or not user_belongs_to_correct_groups(
                user,
                referenced_object,
                permission_groups=getattr(parent_field, "permission_groups", ()),
        ...
        return {self.parent_field: referenced_object}
...
class PermissionCheckedHLFieldMixin:
    ...
    def get_object(self, view_name, view_args, view_kwargs):
        ...
        if not user.has_perm(self.check_permission, hlfield_object) or not user_belongs_to_correct_groups(
            user,
            hlfield_object,
            permission_groups=getattr(self, "permission_groups", ()),
