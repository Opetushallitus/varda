...
def request_log_viewset_decorator_factory(target_path=None, log_anonymous=False):
    def _request_log_viewset_decorator(cls):
        ...
        setattr(cls, function_name,
                _request_log_dispatch_decorator(original_function, target_path, log_anonymous))
...
def _request_log_dispatch_decorator(function, target_path, log_anonymous):
    ...
    def decorator(*args, **kwargs):
        ...
        request_log_object = Z6_RequestLog(request_url=request.path, request_method=request.method,
                                           ...
                                           user=user, vakajarjestaja=vakajarjestaja,
...
def save_data_access_log(user, access_type, henkilo_obj=None, henkilo_id_oid_list=()):
    ...
    if organisaatio_qs.count() != 1:
        ...
        Z12_DataAccessLog.objects.create(henkilo_id=henkilo_obj.id, henkilo_oid=henkilo_obj.henkilo_oid, user=user,
                                         organisaatio=organisaatio_qs.first(), access_type=access_type)
    elif henkilo_id_oid_list:
        for henkilo_id, henkilo_oid in henkilo_id_oid_list:
            Z12_DataAccessLog.objects.create(henkilo_id=henkilo_id, henkilo_oid=henkilo_oid, user=user,
                                             organisaatio=organisaatio_qs.first(), access_type=access_type)
