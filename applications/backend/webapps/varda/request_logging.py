...
from varda.models import Z5_AuditLog, Z6_RequestLog, Z12_DataAccessLog
...
def _sanitize_for_log(value: str, max_length: int = MAX_LOG_LENGTH) -> str:
    return value[:max_length].replace("\n", "\\n").replace("\r", "\\r")
...
def _parse_request_body(request_body):
    ...
    if request_body:
        try:
            ...
            logger.warning(f"{decodeError}, request body: {_sanitize_for_log(request_body_parsed)}")
...
def _parse_response_body(response_data):
    ...
    try:
        ...
        if "QuerySet" in str(type_error):
            ...
            logger.warning(f"{type_error}, response body: {response_data}")
            response_body = ""
