...
from drf_yasg.utils import force_real_str, force_serializer_instance, no_body
...
class TunnisteIdSchema(ReadWriteAutoSchema):
    ...
    param_description = (
        "A unique integer value identifying this {}. Can also be lahdejarjestelma and "
    ...
    def get_query_parameters(self):
        ...
        if match and match.group(1):
            ...
            query_params.append(
                openapi.Parameter(
                    match.group(1), openapi.IN_PATH, description=self.param_description.format(model_name), type=self.param_type
                )
            )
        return query_params
