...
from varda.models import (Henkilo, Huoltajuussuhde, KieliPainotus, Lapsi, Maksutieto, MaksutietoHuoltajuussuhde,
                          ...
                          Tyoskentelypaikka, VuokrattuHenkilosto, ToiminnallinenPainotus, Toimipaikka, Tyontekija,
...
class KelaBaseV2Serializer(serializers.Serializer):
    ...
    def to_representation(self, instance):
        ...
        try:
            ...
            lapsi = (Lapsi.history.filter(id=instance.lapsi_id, history_date__lte=self.datetime_lte).distinct('id')
                     .order_by('id', '-history_date').first())
...
class KelaBaseV2KPSerializer(KelaBaseV2Serializer):
    ...
    def get_vakasuhde_muutos_pvm(self, instance):
        return (Z10_KelaVarhaiskasvatussuhde
                .objects
                .filter(varhaiskasvatussuhde_id=instance.varhaiskasvatussuhde_id, history_type='-')
                .last()
                .history_date
                )
...
class KelaEtuusmaksatusAloittaneetV2Serializer(KelaBaseV2Serializer, serializers.ModelSerializer):
    ...
    class Meta:
        model = Z10_KelaVarhaiskasvatussuhde
        fields = ('vakasuhde_id', 'kotikunta_koodi', 'tallentajakunta_koodi', 'henkilotunnus', 'tietue',
                  'vakasuhde_alkamis_pvm', 'vakasuhde_muutos_pvm')
...
class KelaEtuusmaksatusMaaraaikaisetV2Serializer(KelaBaseV2Serializer, serializers.ModelSerializer):
    ...
    class Meta:
        model = Z10_KelaVarhaiskasvatussuhde
        fields = ('vakasuhde_id', 'kotikunta_koodi', 'tallentajakunta_koodi', 'henkilotunnus', 'tietue',
                  'vakasuhde_alkamis_pvm', 'vakasuhde_paattymis_pvm', 'vakasuhde_muutos_pvm')
...
class KelaEtuusmaksatusLopettaneetV2Serializer(serializers.Serializer):
    ...
    class Meta:
        model = Varhaiskasvatussuhde
        fields = ('vakasuhde_id', 'henkilotunnus', 'kotikunta_koodi', 'tallentajakunta_koodi', 'tietue',
                  'vakasuhde_paattymis_pvm', 'vakasuhde_muutos_pvm')
...
class KelaEtuusmaksatusKorjaustiedotV2Serializer(KelaBaseV2Serializer, serializers.ModelSerializer):
    ...
    class Meta:
        model = Z10_KelaVarhaiskasvatussuhde
        fields = ('vakasuhde_id', 'kotikunta_koodi', 'tallentajakunta_koodi', 'henkilotunnus', 'tietue',
                  'vakasuhde_alkamis_pvm', 'vakasuhde_paattymis_pvm', 'vakasuhde_alkuperainen_alkamis_pvm',
                  'vakasuhde_alkuperainen_paattymis_pvm', 'vakasuhde_muutos_pvm')
...
class TkToimipaikkaSerializer(HistoricalBaseSerializer, serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('id', 'action', 'nimi', 'organisaatio_oid', 'kunta_koodi', 'sahkopostiosoite', 'kayntiosoite',
                  ...
                  'toimintakieli_koodi', 'jarjestamismuoto_koodi', 'varhaiskasvatuspaikat', 'alkamis_pvm',
...
class TkOrganisaatiotSerializer(HistoricalBaseSerializer, serializers.ModelSerializer):
    ...
    def get_tilapainen_henkilosto(self, instance):
        ...
        vuokrattu_henkilosto_qs = (VuokrattuHenkilosto.history
...
class TkVakatiedotSerializer(HistoricalBaseSerializer, serializers.ModelSerializer):
    ...
    def get_huoltajat(self, instance):
        id_list = list(get_related_object_changed_id_qs(
...
class ValssiOrganisaatioSerializer(HistoricalBaseSerializer, serializers.ModelSerializer):
    ...
    class Meta:
        model = Organisaatio
        fields = ('id', 'action', 'action_timestamp', 'nimi', 'y_tunnus', 'organisaatio_oid', 'parent_oid',
                  'organisaatiotyyppi', 'kunta_koodi', 'sahkopostiosoite', 'kayntiosoite', 'kayntiosoite_postinumero',
                  'kayntiosoite_postitoimipaikka', 'postiosoite', 'postinumero', 'postitoimipaikka', 'puhelinnumero',
                  'alkamis_pvm', 'paattymis_pvm', 'yritysmuoto',)
...
class ValssiToimipaikkaSerializer(HistoricalBaseSerializer, serializers.ModelSerializer):
    ...
    class Meta:
        model = Toimipaikka
        fields = ('id', 'action', 'action_timestamp', 'nimi', 'organisaatio_oid', 'parent_oid', 'organisaatiotyyppi',
                  'postinumero', 'jarjestamismuoto_koodi', 'toimintamuoto_koodi',
                  'toimintakieli_koodi', 'alkamis_pvm', 'paattymis_pvm',)
...
class VipunenOrganisaatioSerializer(HistoricalBaseSerializer, serializers.ModelSerializer):
    class Meta:
        model = Organisaatio
        fields = ('id', 'action', 'nimi', 'y_tunnus', 'organisaatio_oid', 'kunta_koodi', 'ytjkieli',
                  'yritysmuoto', 'alkamis_pvm', 'paattymis_pvm',)


class VipunenVuokrattuHenkilostoSerializer(HistoricalBaseSerializer, serializers.ModelSerializer):
    class Meta:
        model = VuokrattuHenkilosto
        fields = ('id', 'action', 'vakajarjestaja_id', 'kuukausi', 'tuntimaara', 'tyontekijamaara',)


class VipunenToimipaikkaSerializer(HistoricalBaseSerializer, serializers.ModelSerializer):
    class Meta:
        model = Toimipaikka
        fields = ('id', 'action', 'organisaatio_oid', 'vakajarjestaja_id', 'nimi', 'kunta_koodi',
                  'kasvatusopillinen_jarjestelma_koodi', 'toimintamuoto_koodi', 'jarjestamismuoto_koodi',
                  'toimintakieli_koodi', 'varhaiskasvatuspaikat', 'toiminnallinenpainotus_kytkin',
                  'kielipainotus_kytkin', 'alkamis_pvm', 'paattymis_pvm',)


class VipunenToiminnallinenPainotusSerializer(HistoricalBaseSerializer, serializers.ModelSerializer):
    class Meta:
        model = ToiminnallinenPainotus
        fields = ('id', 'action', 'toimipaikka_id', 'toimintapainotus_koodi', 'alkamis_pvm', 'paattymis_pvm',)


class VipunenKielipainotusSerializer(HistoricalBaseSerializer, serializers.ModelSerializer):
    class Meta:
        model = KieliPainotus
        fields = ('id', 'action', 'toimipaikka_id',
                  'kielipainotus_koodi', 'kielipainotustyyppi_koodi',
...
class VipunenLapsiSerializer(HistoricalBaseSerializer, serializers.ModelSerializer):
    ...
    class Meta:
        model = Lapsi
        fields = ('id', 'action', 'henkilo_id', 'vakatoimija_id', 'oma_organisaatio_id', 'paos_organisaatio_id',
                  'paos_kytkin', 'henkilo_oid', 'syntyma_pvm', 'aidinkieli_koodi', 'kotikunta_koodi',
                  'sukupuoli_koodi',)
...
class VipunenVarhaiskasvatuspaatosSerializer(HistoricalBaseSerializer, serializers.ModelSerializer):
    class Meta:
        model = Varhaiskasvatuspaatos
        fields = ('id', 'action', 'lapsi_id', 'vuorohoito_kytkin', 'pikakasittely_kytkin', 'paivittainen_vaka_kytkin',
                  'kokopaivainen_vaka_kytkin', 'tilapainen_vaka_kytkin', 'tuntimaara_viikossa',
                  'jarjestamismuoto_koodi', 'hakemus_pvm', 'alkamis_pvm', 'paattymis_pvm',)


class VipunenVarhaiskasvatussuhdeSerializer(HistoricalBaseSerializer, serializers.ModelSerializer):
    class Meta:
        model = Varhaiskasvatussuhde
        fields = ('id', 'action', 'varhaiskasvatuspaatos_id', 'toimipaikka_id', 'alkamis_pvm', 'paattymis_pvm',)
...
class VipunenMaksutietoSerializer(HistoricalBaseSerializer, serializers.ModelSerializer):
    ...
    class Meta:
        model = Maksutieto
        fields = ('id', 'action', 'yksityinen_jarjestaja', 'maksun_peruste_koodi', 'palveluseteli_arvo', 'asiakasmaksu',
                  'perheen_koko', 'alkamis_pvm', 'paattymis_pvm', 'lapsi_id',)
...
class VipunenTyontekijaSerializer(HistoricalBaseSerializer, serializers.ModelSerializer):
    ...
    class Meta:
        model = Tyontekija
        fields = ('id', 'action', 'henkilo_id', 'vakajarjestaja_id', 'henkilo_oid', 'syntyma_pvm', 'aidinkieli_koodi',
                  'kotikunta_koodi', 'sukupuoli_koodi',)
...
class VipunenTutkintoSerializer(HistoricalBaseSerializer, serializers.ModelSerializer):
    class Meta:
        model = Tutkinto
        fields = ('id', 'action', 'henkilo_id', 'vakajarjestaja_id', 'tutkinto_koodi',)


class VipunenPalvelussuhdeSerializer(HistoricalBaseSerializer, serializers.ModelSerializer):
    class Meta:
        model = Palvelussuhde
        fields = ('id', 'action', 'tyontekija_id', 'tyosuhde_koodi', 'tyoaika_koodi', 'tutkinto_koodi',
                  'tyoaika_viikossa', 'alkamis_pvm', 'paattymis_pvm',)


class VipunenTyoskentelypaikkaSerializer(HistoricalBaseSerializer, serializers.ModelSerializer):
    class Meta:
        model = Tyoskentelypaikka
        fields = ('id', 'action', 'palvelussuhde_id', 'toimipaikka_id', 'tehtavanimike_koodi', 'kelpoisuus_kytkin',
                  'kiertava_tyontekija_kytkin', 'alkamis_pvm', 'paattymis_pvm',)
...
class VipunenPidempiPoissaoloSerializer(HistoricalBaseSerializer, serializers.ModelSerializer):
    class Meta:
        model = PidempiPoissaolo
        fields = ('id', 'action', 'palvelussuhde_id', 'alkamis_pvm', 'paattymis_pvm',)
...
class VipunenTaydennyskoulutusTyontekijaSerializer(HistoricalBaseSerializer, serializers.ModelSerializer):
    class Meta:
        ...
        fields = ('id', 'action', 'taydennyskoulutus_id', 'tyontekija_id',)


class VipunenTaydennyskoulutusSerializer(HistoricalBaseSerializer, serializers.ModelSerializer):
    class Meta:
        model = Taydennyskoulutus
        fields = ('id', 'action', 'nimi', 'suoritus_pvm', 'koulutuspaivia',)
