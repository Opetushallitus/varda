...
from varda.excel_export import ExcelReportSubtype, ExcelReportType, ReportStatus, get_s3_object_name
...
from varda.serializers import OrganisaatioPermissionCheckedHLField, ToimipaikkaHLField
...
class ExcelReportSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        if not data.get("organisaatio", None) and data["report_type"] not in [
            ExcelReportType.VAKATIEDOT_VOIMASSA_8Y_OLDER.value,
...
class MassahakuExcelReportRequestSerializer(serializers.Serializer):
    report_type = serializers.ChoiceField(
        choices=[ExcelReportType.TYONTEKIJATIEDOT_VOIMASSA.value],
        help_text="Excel report type to generate. Must be one of the supported report type values listed below.",
    )
    target_date = serializers.DateField(
        help_text="Date for which the report is generated. Use ISO date format YYYY-MM-DD, for example 2026-01-15."
    )
    organisaatio_oid = serializers.CharField(
        required=False,
        validators=[validators.validate_organisaatio_oid],
        help_text=(
            "Organisaatio OID for the report scope. Required for Admin/OPH users. "
            "Palvelukayttaja must omit this field; their organisaatio_oid is derived from the VARDA_MASSAHAKU permission."
        ),
    )
    toimipaikka_oid = serializers.CharField(
        required=False,
        allow_null=True,
        validators=[validators.validate_organisaatio_oid],
        help_text=("Optional toimipaikka_oid. When provided, the toimipaikka must belong to the report's organisation."),
    )
    language = serializers.CharField(
        default=SupportedLanguage.FI.value,
        help_text="Language of the generated Excel report. Accepted values are FI and SV. Defaults to FI.",
    )

    def validate_language(self, value):
        normalized_value = value.upper()
        if normalized_value not in SupportedLanguage.list():
            raise serializers.ValidationError("Unsupported language.")
        return normalized_value


class MassahakuExcelReportResponseSerializer(serializers.Serializer):
    id = serializers.IntegerField(read_only=True)
    status = serializers.SerializerMethodField()
    report_type = serializers.CharField(read_only=True)
    target_date = serializers.DateField(read_only=True)
    toimipaikka_oid = serializers.CharField(source="toimipaikka.organisaatio_oid", read_only=True, allow_null=True)
    ...
    STATUS_NAMES = {
        ...
        ReportStatus.CREATING.value: "running",
        ReportStatus.FINISHED.value: "complete",
        ReportStatus.FAILED.value: "failed",
    }

    def get_status(self, instance):
        return self.STATUS_NAMES.get(instance.status, instance.status.lower())
    ...
    def get_file_url(self, instance):
        if instance.status != ReportStatus.FINISHED.value:
            return None
        return reverse("massahaut-file", kwargs={"pk": instance.id}, request=self.context.get("request"))


class MassahakuExcelReportListSerializer(serializers.Serializer):
    id = serializers.IntegerField(read_only=True)
    status = serializers.SerializerMethodField()
    report_type = serializers.CharField(read_only=True)
    target_date = serializers.DateField(read_only=True)
    toimipaikka_oid = serializers.CharField(source="toimipaikka.organisaatio_oid", read_only=True, allow_null=True)
    language = serializers.CharField(read_only=True)
    organisaatio_oid = serializers.CharField(source="organisaatio.organisaatio_oid", read_only=True, allow_null=True)

    STATUS_NAMES = MassahakuExcelReportResponseSerializer.STATUS_NAMES
