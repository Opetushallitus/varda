...
def send_request_to_external_service(request_type, service_name, http_url_suffix, expected_status_code, auth,
    ...
    while number_of_attempt < MAX_NO_OF_ATTEMPS:
        number_of_attempt += 1
        headers = (get_authentication_header(service_name, external_request=False, force_new_tgt=force_new_tgt)
                   if auth else get_contenttype_header())
...
def is_date_within_timeframe(today, start_date, end_date):
    start_date, end_date = adjust_dates_for_current_year(today, start_date, end_date)
    return start_date <= today <= end_date


def adjust_dates_for_current_year(today, start_date, end_date):
    """
    Adjust the dates according to the current year, the given year of start and end date is not taken into account
    """
    start_date = start_date.replace(year=today.year)
    end_date = end_date.replace(year=today.year)

    # Check if the timeframe spans across two years
    if end_date < start_date:
        if today < start_date.replace(year=today.year):
...
def find_timeframe_for_current_date(today, timeframes):
    for timeframe in timeframes:
        is_within_timeframe = is_date_within_timeframe(
            today,
            timeframe.alkamis_pvm,
            timeframe.paattymis_pvm
        )
...
def calculate_tilastointi_pvm(today, timeframe, next_timeframe=False):
    current_year = today.year
    start_date = timeframe.alkamis_pvm.replace(year=current_year)
    end_date = timeframe.paattymis_pvm.replace(year=current_year)
    statistical_date = timeframe.tilastointi_pvm.replace(year=current_year)

    if start_date <= today <= end_date:
        return statistical_date
    elif today > end_date:
        next_year = current_year + 1
        statistical_date_next_year = timeframe.tilastointi_pvm.replace(year=next_year)

        # Check if statistical date is before start date of next year
        if statistical_date_next_year < timeframe.alkamis_pvm.replace(year=next_year):
            return statistical_date_next_year
        else:
            if next_timeframe and not timeframe.alkamis_pvm.month > timeframe.paattymis_pvm.month:
                return statistical_date_next_year
            return statistical_date
    elif today < start_date and statistical_date > end_date:
        previous_year = current_year - 1
        return timeframe.tilastointi_pvm.replace(year=previous_year)

    return statistical_date


def calculate_next_start_end_date(today, start_date, end_date):
    # Check if the timeframe spans across two years
    start_date, end_date = adjust_dates_for_current_year(today, start_date, end_date)

    # Check if the date is within the timeframe
    if start_date <= today <= end_date:
        return start_date, end_date

    # Handle cases where the timeframe starts next year
    if today < start_date.replace(year=today.year):
        start_date = start_date.replace(year=today.year)
        end_date = end_date.replace(year=today.year)
    else:
    ...
    # Handle cases where the current date is outside the timeframe and the next timeframe crosses the year boundary
    if end_date < start_date:
        end_date = end_date.replace(year=today.year + 1)
...
def find_next_timeframe(today, timeframes):
    ...
    for timeframe in timeframes.order_by(ExtractMonth('alkamis_pvm')):
        if (today.month < timeframe.alkamis_pvm.month or
                (today.month == timeframe.alkamis_pvm.month and today.day < timeframe.alkamis_pvm.day)):
            return timeframe

    return timeframes[0]  # If there are no upcoming timeframes this year, the next one is the first one


def get_tilastointi_pvm(today, timeframes):
    timeframe = find_next_timeframe(today, timeframes)
    tilastointi_pvm = calculate_tilastointi_pvm(today, timeframe)
    return tilastointi_pvm
...
def get_next_timeframe(today, timeframes):
    ...
    next_tilastointi_pvm = calculate_tilastointi_pvm(today, next_timeframe, next_timeframe=True)
    alkamis_pvm, paattymis_pvm = calculate_next_start_end_date(
        today, next_timeframe.alkamis_pvm, next_timeframe.paattymis_pvm)

    return alkamis_pvm, paattymis_pvm, next_tilastointi_pvm


def get_latest_tilastointi_pvm_before_date(organisaatio, before_date):
    latest_tukipaatos = Tukipaatos.objects.filter(
        vakajarjestaja=organisaatio,
        tilastointi_pvm__lte=before_date
    ).order_by('tilastointi_pvm').last()

    return latest_tukipaatos.tilastointi_pvm if latest_tukipaatos else None


def get_latest_tilastointi_pvm(vakajarjestaja):
    latest_tukipaatos = Tukipaatos.objects.filter(vakajarjestaja=vakajarjestaja) \
        .order_by('tilastointi_pvm').last()
...
def create_muistutus_report_csv(msg_logs):
    data = [
        [msg_log.organisaatio.nimi, msg_log.message_type]
...
def get_person_count_per_kunta_dict(henkilo_oid_list, code_language):
    ...
    kunta_dict = dict(
        total=dict(
            amount=len(henkilo_oid_list),
            amount_age_lte_3=henkilo_age_lte_3y_count,
            amount_age_gt_3=henkilo_age_gt_3y_count)
    )
    for henkilo_oid in henkilo_oid_list:
        henkilo_age = henkilo_ages_dict.get(henkilo_oid, None)
        kunta_code_list = (Toimipaikka.objects.filter(
            varhaiskasvatussuhteet__varhaiskasvatuspaatos__lapsi__henkilo__henkilo_oid=henkilo_oid)
            ...
            if kunta_code not in kunta_dict:
                kunta_translation = getattr(
                    Z2_CodeTranslation.objects.filter(
                        language__iexact=code_language, code__code_value=kunta_code,
                        code__koodisto__name=Koodistot.kunta_koodit.value).first(),
                    'name', None)
                kunta_dict[kunta_code] = dict(
                    name=kunta_translation,
                    amount=0,
                    amount_age_lte_3=0,
                    amount_age_gt_3=0)
