...
from varda.enums.yhteystieto import YhteystietoAlkupera, Yhteystietoryhmatyyppi, YhteystietoTyyppi
...
from varda.models import Aikaleima, BatchError, Henkilo, Huoltaja, Huoltajuussuhde, Lapsi
...
def create_henkilo_in_varda_if_not_found(henkilo_oid: str, hetu: str | None) -> int | None:
    """
    :param hetu:
    :param henkilo_oid:
    :return: Henkilo id or None in case of error
    """
    hetu_hash = hash_string(hetu) if hetu else None
    query = Q(henkilo_oid=henkilo_oid)
    if hetu_hash:
        query |= Q(henkilotunnus_unique_hash=hetu_hash)
    henkilo = Henkilo.objects.filter(query).first()

    if henkilo:
        return henkilo.id

    henkilo_data = get_henkilo_data_by_oid(henkilo_oid)
    new_henkilo = Henkilo.objects.create(
        henkilo_oid=henkilo_oid,
        henkilotunnus=encrypt_string(hetu) if hetu else None,
        henkilotunnus_unique_hash=hetu_hash,
    )
    new_henkilo_id = new_henkilo.id
    try:
        save_henkilo_to_db(new_henkilo_id, henkilo_data)
    except Exception as error:
        logger.warning(error)
        return None

    return new_henkilo_id
