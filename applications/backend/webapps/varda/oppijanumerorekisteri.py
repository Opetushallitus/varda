...
def save_henkilo_to_db(henkilo_id, henkilo_json, reason=None, return_not_throw_error=False):  # noqa: C901
    ...
    for key, field_name in henkilo_fields.items():
        if henkilo_json.get(key, None) is not None:
            new_value = henkilo_json[key]
            current_value = getattr(henkilo, field_name)
            if current_value != new_value:
                setattr(henkilo, field_name, new_value)
                has_changes = True

    if "hetu" in henkilo_json and henkilo_json["hetu"]:
        new_hash = hash_string(henkilo_json["hetu"])

        # Check if this hash already exists for a different Henkilo object
        existing_henkilo = Henkilo.objects.filter(henkilotunnus_unique_hash=new_hash).exclude(id=henkilo_id).first()

        if existing_henkilo and return_not_throw_error:
            # TODO: Refactor together with duplicate henkilo fixes
            logger.info(f"Duplicate henkilo found when updating henkilo-data. Henkilo-id: {henkilo_id}.")
            return None

        encrypted_hetu = encrypt_string(henkilo_json["hetu"])
        if henkilo.henkilotunnus != encrypted_hetu or henkilo.henkilotunnus_unique_hash != new_hash:
            henkilo.henkilotunnus = encrypted_hetu
            henkilo.henkilotunnus_unique_hash = new_hash
            has_changes = True
    else:
        if henkilo.henkilotunnus or henkilo.henkilotunnus_unique_hash:
