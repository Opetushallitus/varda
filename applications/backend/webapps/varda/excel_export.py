...
from django.db.models import Count, Exists, F, Model, OuterRef, Q
...
from varda.constants import TYOSUHDE_KOODI_MAARAAIKAINEN, YRITYSMUOTO_KUNTA
...
class ExcelReportType(enum.Enum):
    ...
    VAKATIEDOT_VOIMASSA_8Y_OLDER = "VAKATIEDOT_VOIMASSA_8Y_OLDER"
    ...
    TYONTEKIJATIEDOT_MAARAAIKAISET_VOIMASSA = "TYONTEKIJATIEDOT_MAARAAIKAISET_VOIMASSA"
...
VAKASUHDE_HEADERS_8Y_OLDER = "VAKASUHDE_HEADERS_8Y_OLDER"
...
TRANSLATIONS = {
    ...
    ExcelReportType.VAKATIEDOT_VOIMASSA_8Y_OLDER.value: T.EXCEL_FILENAME_VAKATIEDOT_VOIMASSA_8Y_OLDER,
    ...
    ExcelReportType.TYONTEKIJATIEDOT_MAARAAIKAISET_VOIMASSA.value: T.EXCEL_FILENAME_TYONTEKIJATIEDOT_MAARAAIKAISET_VOIMASSA,
...
@custom_shared_task(single_instance=True)
def create_mass_excel_report_task(report_id):
    report = Z8_ExcelReport.objects.using(DEFAULT_DB_ALIAS).filter(id=report_id).first()
    if not report or report.status != ReportStatus.PENDING.value:
        logger.error(f"Error starting Excel report creation with id {report_id}")
        return

    logger.info(
        "Starting mass Excel report generation: report_id=%s report_type=%s data_db_alias=%s",
        report.id,
        report.report_type,
        settings.READER_DB,
    )
    report_generator = ExcelReportGenerator(report, data_db_alias=settings.READER_DB)
    try:
        report_generator.generate()
        logger.info(
            "Mass Excel report generation completed: report_id=%s report_type=%s data_db_alias=%s status=%s",
            report.id,
            report.report_type,
            settings.READER_DB,
            report.status,
        ...
        logger.error("Mass Excel report generation failed due to database error for report %s", report.id)
        report.status = ReportStatus.FAILED.value
        report.save(using=DEFAULT_DB_ALIAS)


class ExcelReportGenerator:
    def __init__(self, report, data_db_alias=DEFAULT_DB_ALIAS):
        self.report = report
        self.data_db_alias = data_db_alias
    ...
    def _create_excel_report(self):
        match self.report.report_type:
            ...
            case ExcelReportType.VAKATIEDOT_VOIMASSA_8Y_OLDER.value:
                self._create_vakatiedot_voimassa_8y_older_report()
            case (
            ...
            case ExcelReportType.TYONTEKIJATIEDOT_MAARAAIKAISET_VOIMASSA.value:
                self._create_tyontekijatiedot_maaraaikaiset_voimassa_report()
    ...
    def _create_vakatiedot_voimassa_8y_older_report(self):
        target_date = self.report.target_date
        if not target_date:
            target_date = datetime.date.today()

        self._create_vakatiedot_8y_older_report(target_date=target_date)

    def _create_vakatiedot_8y_older_report(self, target_date):
        active_filter = get_active_filter(target_date)
        inactive_filter = get_inactive_filter(target_date)
        vakasuhde_active_vakapaatos_active_filter = active_filter & get_active_filter(target_date, prefix="varhaiskasvatuspaatos")
        vakasuhde_active_vakapaatos_inactive_filter = active_filter & get_inactive_filter(
            target_date, prefix="varhaiskasvatuspaatos"
        )
        vakasuhde_inactive_vakapaatos_active_filter = inactive_filter & get_active_filter(
            target_date, prefix="varhaiskasvatuspaatos"
        )
        ...
        vakajarjestaja_filter = (
            ...
            if self.organisaatio_id
            else Q()
        )

        syntyma_pvm_cutoff = target_date - datetime.timedelta(days=365 * 8 + 2)  # 2 leap years
        age_filter = Q(varhaiskasvatuspaatos__lapsi__henkilo__syntyma_pvm__lte=syntyma_pvm_cutoff)
        ...
        order_by = [
            "toimipaikka__vakajarjestaja__nimi",
        ...
        for sheet in [
            dict(name_key=T.EXCEL_VAKASUHDE_ACTIVE_VAKAPAATOS_ACTIVE, vakasuhde_filter=vakasuhde_active_vakapaatos_active_filter),
            dict(
                name_key=T.EXCEL_VAKASUHDE_ACTIVE_VAKAPAATOS_INACTIVE,
                vakasuhde_filter=vakasuhde_active_vakapaatos_inactive_filter,
            ),
            dict(
                name_key=T.EXCEL_VAKASUHDE_INACTIVE_VAKAPAATOS_ACTIVE,
                vakasuhde_filter=vakasuhde_inactive_vakapaatos_active_filter,
            ),
        ]:
            vakasuhde_sheet = self._add_worksheet(sheet["name_key"])

            self._write_headers(vakasuhde_sheet, VAKASUHDE_HEADERS_8Y_OLDER)

            vakasuhde_qs = (
                Varhaiskasvatussuhde.objects.filter(vakajarjestaja_filter & sheet["vakasuhde_filter"] & age_filter)
                .select_related(*select_related)
                .distinct()
                .order_by(*order_by)
            )

            self._write_vakatiedot_8y_older_rows(
                vakasuhde_sheet,
                vakasuhde_qs,
                jarjestamismuoto_codes,
                lahdejarjestelma_codes,
            )

    def _write_vakatiedot_8y_older_rows(
        self,
        vakasuhde_sheet,
        vakasuhde_qs,
        jarjestamismuoto_codes,
        ...
        for index, vakasuhde in enumerate(vakasuhde_qs.iterator(), 1):
            ...
            if lapsi.paos_kytkin:
                paos_organisaatio = (
                    lapsi.paos_organisaatio
                    if vakasuhde.toimipaikka.vakajarjestaja != lapsi.paos_organisaatio
                    else lapsi.oma_organisaatio
                )
                vakasuhde_values.extend([paos_organisaatio.nimi, paos_organisaatio.organisaatio_oid])
            else:
                vakasuhde_values.extend([None, None])
            ...
            vakasuhde_values.extend(
                [
                    ...
                    vakasuhde.alkamis_pvm,
                    vakasuhde.paattymis_pvm,
                    vakasuhde.toimipaikka.vakajarjestaja.nimi,
                    vakasuhde.toimipaikka.vakajarjestaja.organisaatio_oid,
                    vakasuhde.toimipaikka.nimi,
                    vakasuhde.toimipaikka.organisaatio_oid,
                    vakasuhde.toimipaikka.id,
                ]
    ...
    def _create_tyontekijatiedot_report(self, palvelussuhde_filter=Q()):
        ...
        palvelussuhde_qs = (
            Palvelussuhde.objects.using(self.data_db_alias)
            .filter(vakajarjestaja_filter & palvelussuhde_filter)
            .distinct()
    ...
    def _create_tyontekijatiedot_maaraaikaiset_voimassa_report(self):  #
        target_date = self.report.target_date
        if not target_date:
            target_date = datetime.date.today()

        target_date_secondary = self.report.target_date_secondary
        if not target_date_secondary:
            target_date_secondary = target_date

        palvelussuhde_filter = (
            get_active_on_any_date_filter(target_date, target_date_secondary)
            & get_active_on_any_date_filter(target_date, target_date_secondary, prefix="tyoskentelypaikat")
            & Q(**{"tyosuhde_koodi": TYOSUHDE_KOODI_MAARAAIKAINEN})
        )
    ...
    def _create_vuosiraportti_henkilosto(self, tilasto_date, poiminta_datetime):
        ...
        # Form 13 rows for full year and monthly numbers
        term_count = get_yearly_report_fixed_term_data(poiminta_datetime, tilasto_date.year, self.organisaatio_id)

        self._write_row(sheet, index, [self._get_translation(T.EXCEL_WHOLE_YEAR), term_count[0][1]])
        index += 1
        for month_index in range(1, 13):
            self._write_row(sheet, index, [month_translation_list[month_index - 1], term_count[month_index][1]])
            index += 1
    ...
    def _add_worksheet(self, translation_key):
        ...
        translation = self._get_translation(translation_key)
        return self.workbook.add_worksheet(translation[:MAX_SHEET_NAME_LENGTH])
    ...
    def _get_koodisto_with_translations(self, koodisto):
        return list(
            Z2_Code.objects.using(self.data_db_alias)
            .filter(Q(koodisto__name=koodisto) & Q(translations__language__iexact=self.language))
