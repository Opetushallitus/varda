...
class ExcelReportGenerator:
    ...
    def generate(self):
        ...
        if not self.admin_user_all_organizations:
            ...
            save_data_access_log(self.report.user, DataAccessType.EXCEL.value,
                                 henkilo_id_oid_list=self.henkilo_id_oid_set)
        ...
        for index, vakasuhde in enumerate(vakasuhde_qs.iterator(), 1):
            ...
            if self.admin_user_all_organizations:
                ...
                vakasuhde_values.extend([vakapaatos.id, vakapaatos.hakemus_pvm, vakapaatos.alkamis_pvm,
                                         ...
                                         _get_code_translation(jarjestamismuoto_codes,
    ...
    def _create_toimipaikat_report(self, toimipaikka_filter=Q()):
        ...
        kasvatusjarjestelma_codes = self._get_koodisto_with_translations(
            Koodistot.kasvatusopillinen_jarjestelma_koodit.value)
        toiminnallinen_painotus_codes = self._get_koodisto_with_translations(
            Koodistot.toiminnallinen_painotus_koodit.value)
    ...
    if isinstance(model(), Model):
        if model_instance := model.objects.filter(id=model_id).first():
            if hasattr(model_instance, 'alkamis_pvm') and hasattr(model_instance, 'paattymis_pvm'):
                ...
                paattymis_pvm = model_instance.paattymis_pvm.strftime('%d.%m.%Y') \
                    if model_instance.paattymis_pvm else ''
...
def _encrypt_excel_file(file_path, password):
    ...
    with (
        ...
        requests.post(
            ...
            f'{settings.JAVA_URL}/upload', files={'excel': file, 'password': (None, password,)},
