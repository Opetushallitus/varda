...
def set_paattymis_pvm_for_vakajarjestaja_data(vakajarjestaja, paattymis_pvm):
    ...
    painotus_filter = Q(toimipaikka__vakajarjestaja=vakajarjestaja) & Q(
        toimipaikka__hallinnointijarjestelma=Hallinnointijarjestelma.VARDA.value
    )

    with transaction.atomic():
        qs_list = [
            Toimipaikka.objects.filter(
                Q(vakajarjestaja=vakajarjestaja)
            ...
            Varhaiskasvatussuhde.objects.filter(
                Q(varhaiskasvatuspaatos__lapsi__vakatoimija=vakajarjestaja) & paattymis_pvm_filter
            ),
            Maksutieto.objects.filter(Q(huoltajuussuhteet__lapsi__vakatoimija=vakajarjestaja) & paattymis_pvm_filter).distinct(
                "id"
            ),
            Palvelussuhde.objects.filter(Q(tyontekija__vakajarjestaja=vakajarjestaja) & paattymis_pvm_filter),
            Tyoskentelypaikka.objects.filter(Q(palvelussuhde__tyontekija__vakajarjestaja=vakajarjestaja) & paattymis_pvm_filter),
        ]
