...
from varda.serializers_oppija import (
    HuoltajatiedotSerializer,
    TyontekijatiedotSerializer,
    HenkilotiedotSerializer,
...
class AbstractOppijaViewSet(GenericViewSet, RetrieveModelMixin):
    ...
    def get_object(self):
        ...
        if (
            hasattr(user, "additional_cas_user_fields")
            and (
                (huollettava_oid_list := getattr(user.additional_cas_user_fields, "huollettava_oid_list", None))
                and henkilo_oid in huollettava_oid_list
            )
            and henkilo.turvakielto
        ):
