...
from varda.constants import JARJESTAMISMUODOT_KUNTA, JARJESTAMISMUODOT_PAOS, JARJESTAMISMUODOT_YKSITYINEN
...
from varda.related_object_validations import (
    check_if_immutable_object_is_changed,
    check_overlapping_kielipainotus,
    check_overlapping_maksutieto,
    check_overlapping_toiminnallinen_painotus,
    check_overlapping_varhaiskasvatuspaatos,
    check_overlapping_varhaiskasvatussuhde,
    check_toimipaikka_and_vakajarjestaja_have_oids,
)
from varda.serializers_common import (
    HenkiloHLField,
    HenkiloPermissionCheckedHLField,
    OidRelatedField,
    OptionalToimipaikkaMixin,
    OrganisaatioHLField,
    OrganisaatioPermissionCheckedHLField,
    PermissionCheckedHLFieldMixin,
    ToimipaikkaHLField,
    ToimipaikkaPermissionCheckedHLField,
    TunnisteRelatedField,
)
from varda.validators import (
    fill_missing_fields_for_validations,
    validate_henkilo_oid,
    validate_henkilotunnus_or_oid_needed,
...
class ActiveUserSerializer(serializers.ModelSerializer):
    toimintakieli_koodi = serializers.ReadOnlyField()
    ...
    def to_representation(self, instance):
        ...
        if not additional_cas_user_fields:
            data["toimintakieli_koodi"] = "fi"
            data["henkilo_oid"] = ""
            return data

        data["toimintakieli_koodi"] = additional_cas_user_fields.toimintakieli_koodi
        henkilo_oid = additional_cas_user_fields.henkilo_oid
        data["henkilo_oid"] = henkilo_oid

        if additional_cas_user_fields.kayttajatyyppi == Kayttajatyyppi.OPPIJA_CAS.value:
...
class LapsiOptionalToimipaikkaMixin(OptionalToimipaikkaMixin):
    toimipaikka = ToimipaikkaPermissionCheckedHLField(
        view_name="toimipaikka-detail",
        required=False,
        write_only=True,
        permission_groups=[Z4_CasKayttoOikeudet.TALLENTAJA, Z4_CasKayttoOikeudet.PALVELUKAYTTAJA],
        check_paos=True,
        default=None,
    )
...
class RequiredLahdejarjestelmaMixin(metaclass=serializers.SerializerMetaclass):
    lahdejarjestelma = serializers.CharField(required=True, max_length=2, validators=[validators.validate_lahdejarjestelma_koodi])
...
class OrganisaatioSerializer(serializers.HyperlinkedModelSerializer):
    ...
    def to_representation(self, instance):
        return super(OrganisaatioSerializer, self).to_representation(instance)
...
class ToimipaikkaSerializer(RequiredLahdejarjestelmaMixin, serializers.HyperlinkedModelSerializer):
    id = serializers.IntegerField(read_only=True)
    vakajarjestaja = OrganisaatioPermissionCheckedHLField(
        view_name="organisaatio-detail",
        required=False,
        permission_groups=[Z4_CasKayttoOikeudet.TALLENTAJA, Z4_CasKayttoOikeudet.PALVELUKAYTTAJA],
    )
    vakajarjestaja_oid = OidRelatedField(
    ...
    def validate(self, data):
        ...
        if self.instance:
            ...
            if (
                data["toimintamuoto_koodi"].lower() != self.instance.toimintamuoto_koodi.lower()
                and self.instance.organisaatio_oid is None
            ):
        ...
        validators.validate_kasvatusopillinen_jarjestelma_koodi(
            data["kasvatusopillinen_jarjestelma_koodi"], alkamis_pvm, paattymis_pvm
        )
    ...
    def _validate_jarjestamismuoto_codes(self, data):
        ...
        with ViewSetValidator() as validator:
            ...
            if vakajarjestaja.kunnallinen_kytkin and not jarjestamismuoto_codes_set.issubset(jarjestamismuoto_codes_kunta):
            ...
            elif not vakajarjestaja.kunnallinen_kytkin and not jarjestamismuoto_codes_set.issubset(
                jarjestamismuoto_codes_yksityinen
            ):
            ...
            with validator.wrap():
                for jarjestamismuoto_code in jarjestamismuoto_codes_set:
                    validators.validate_jarjestamismuoto_koodi(
                        jarjestamismuoto_code, data["alkamis_pvm"], data.get("paattymis_pvm")
                    )
    ...
    def _validate_dates_within_vakajarjestaja(self, data):
        ...
        if vakajarjestaja.paattymis_pvm:
            if validators.validate_paivamaara1_after_paivamaara2(alkamis_pvm, vakajarjestaja.paattymis_pvm, can_be_same=True):
            ...
            elif paattymis_pvm and validators.validate_paivamaara1_after_paivamaara2(
                paattymis_pvm, vakajarjestaja.paattymis_pvm, can_be_same=True
            ):
    ...
    def get_toiminnallisetpainotukset_top(self, instance):
        ...
        return get_top_results(self.context["request"], instance.toiminnallisetpainotukset.all(), permission_group_list, oid_list)
    ...
    def get_kielipainotukset_top(self, instance):
        ...
        oid_list = [instance.organisaatio_oid, instance.vakajarjestaja.organisaatio_oid]
        return get_top_results(self.context["request"], instance.kielipainotukset.all(), permission_group_list, oid_list)
    ...
    def get_varhaiskasvatussuhteet_top(self, instance):
        ...
        oid_list = [instance.organisaatio_oid, instance.vakajarjestaja.organisaatio_oid]
        return get_top_results(self.context["request"], instance.varhaiskasvatussuhteet.all(), permission_group_list, oid_list)


class ToimipaikkaV2Serializer(ToimipaikkaSerializer):
    class Meta(ToimipaikkaSerializer.Meta):
        exclude = (
            "nimi_sv",
            "luonti_pvm",
        )
...
class ToimipaikkaUpdateSerializer(ToimipaikkaSerializer):
    ...
    (no OrganisaatioPermissionCheckedHLField)
    """

    vakajarjestaja = OrganisaatioHLField(view_name="organisaatio-detail", required=False, read_only=True)
    vakajarjestaja_oid = OidRelatedField(
        object_type=Organisaatio,
        parent_field="vakajarjestaja",
        parent_attribute="organisaatio_oid",
...
class ToimipaikkaUpdateV2Serializer(ToimipaikkaV2Serializer):
    """
    Different PUT/PATCH serializer so that Toimipaikka can be updated with only Toimipaikka permissions
    (no OrganisaatioPermissionCheckedHLField)
    """
...
class ToiminnallinenPainotusSerializer(RequiredLahdejarjestelmaMixin, serializers.HyperlinkedModelSerializer):
    ...
    toimipaikka_tunniste = TunnisteRelatedField(
        object_type=Toimipaikka,
        parent_field="toimipaikka",
    ...
    def validate(self, data):
        ...
        check_toimipaikka_and_vakajarjestaja_have_oids(
            toimipaikka, toimipaikka.vakajarjestaja.organisaatio_oid, toimipaikka.organisaatio_oid
        )
        validators.validate_alkamis_pvm_before_paattymis_pvm(data)
        validators.validate_toimintapainotus_koodi(data["toimintapainotus_koodi"], data["alkamis_pvm"], data.get("paattymis_pvm"))
...
class KieliPainotusSerializer(RequiredLahdejarjestelmaMixin, serializers.HyperlinkedModelSerializer):
    ...
    kielipainotustyyppi_koodi = serializers.CharField(required=True)
...
class HenkiloSerializer(serializers.HyperlinkedModelSerializer):
    ...
    kutsumanimi = serializers.CharField(required=False, allow_blank=True)
    ...
    def validate_henkilo_oid(self, value):
        if value:
            validate_henkilo_oid(value)
        return value
...
class NestedMaksutietoHuoltajaSerializer(serializers.Serializer):
    ...
    def create(self, data):
        ...
        return MaksutietoHuoltajuussuhde.objects.create(huoltajuussuhde=huoltajuussuhde_obj, maksutieto=data["maksutieto"])
...
def _validate_maksutieto_dates(data, lapsi_obj=None):
    ...
    if vakapaatokset.count() > 0:
        ...
        if all((v.paattymis_pvm is not None) for v in vakapaatokset):
            if not validators.validate_paivamaara1_after_paivamaara2(latest_paattymis_pvm, alkamis_pvm, can_be_same=True):
        ...
        if paattymis_pvm is not None:
            if all((v.paattymis_pvm is not None) for v in vakapaatokset):
                if not validators.validate_paivamaara1_after_paivamaara2(latest_paattymis_pvm, paattymis_pvm, can_be_same=True):
...
def _get_lapsi_for_maksutieto(maksutieto):
    ...
    lapsi_set = {
        maksutieto_huoltajuussuhde.huoltajuussuhde.lapsi
...
class MaksutietoSerializer(RequiredLahdejarjestelmaMixin, serializers.HyperlinkedModelSerializer):
    id = serializers.IntegerField(read_only=True)
    huoltajat = NestedMaksutietoHuoltajaSerializer(
        required=True, allow_empty=False, many=True, source="maksutiedot_huoltajuussuhteet"
    )
    lapsi = LapsiHLField(required=False, view_name="lapsi-detail")
    lapsi_tunniste = TunnisteRelatedField(
        object_type=Lapsi,
        parent_field="lapsi",
        prevalidator=validators.validate_tunniste,
        either_required=True,
...
class MaksutietoPostSerializer(MaksutietoSerializer):
    ...
    def _validate_maksun_peruste(self, data):
        ...
        validators.validate_maksun_peruste_koodi(data["maksun_peruste_koodi"], data["alkamis_pvm"], data.get("paattymis_pvm"))
...
class MaksutietoUpdateSerializer(RequiredLahdejarjestelmaMixin, serializers.HyperlinkedModelSerializer):
    id = serializers.IntegerField(read_only=True)
    huoltajat = NestedMaksutietoHuoltajaSerializer(
        required=False, allow_empty=False, many=True, source="maksutiedot_huoltajuussuhteet", read_only=True
    )
...
class LapsiSerializer(RequiredLahdejarjestelmaMixin, LapsiOptionalToimipaikkaMixin, serializers.HyperlinkedModelSerializer):
    id = serializers.IntegerField(read_only=True)
    henkilo = HenkiloPermissionCheckedHLField(
        view_name="henkilo-detail",
        required=False,
        allow_null=True,
        default=None,
    )
    henkilo_oid = OidRelatedField(
        object_type=Henkilo,
    ...
    vakatoimija = OrganisaatioPermissionCheckedHLField(
        view_name="organisaatio-detail",
        required=False,
        allow_null=True,
        default=None,
    ...
    vakatoimija_oid = OidRelatedField(
        object_type=Organisaatio,
    ...
    oma_organisaatio = OrganisaatioHLField(
        required=False,
        allow_null=True,
        default=None,
        view_name="organisaatio-detail",
    )
    oma_organisaatio_oid = OidRelatedField(
        object_type=Organisaatio,
    ...
    paos_organisaatio = OrganisaatioHLField(
        required=False,
        allow_null=True,
        default=None,
        view_name="organisaatio-detail",
    )
    paos_organisaatio_oid = OidRelatedField(
        object_type=Organisaatio,
    ...
    def get_varhaiskasvatuspaatokset_top(self, instance):
        ...
        oid_list = (
            [instance.vakatoimija.organisaatio_oid]
            if instance.vakatoimija
            else [instance.oma_organisaatio.organisaatio_oid, instance.paos_organisaatio.organisaatio_oid]
        )
        return get_top_results(self.context["request"], instance.varhaiskasvatuspaatokset.all(), permission_group_list, oid_list)
...
class LapsiSerializerAdmin(RequiredLahdejarjestelmaMixin, serializers.HyperlinkedModelSerializer):
    ...
    varhaiskasvatuspaatokset_top = serializers.HyperlinkedRelatedField(
        many=True, read_only=True, view_name="varhaiskasvatuspaatos-detail"
    )
    huoltajuussuhteet = serializers.HyperlinkedRelatedField(many=True, read_only=True, view_name="huoltajuussuhde-detail")
    vakatoimija = OrganisaatioHLField(
        required=False,
        allow_null=True,
        default=None,
        view_name="organisaatio-detail",
...
class VarhaiskasvatuspaatosSerializer(
    RequiredLahdejarjestelmaMixin, LapsiOptionalToimipaikkaMixin, serializers.HyperlinkedModelSerializer
):
    id = serializers.IntegerField(read_only=True)
    lapsi = LapsiPermissionCheckedHLField(required=False, view_name="lapsi-detail")
    lapsi_tunniste = TunnisteRelatedField(
        object_type=Lapsi, parent_field="lapsi", prevalidator=validators.validate_tunniste, either_required=True
    )
    varhaiskasvatussuhteet_top = serializers.SerializerMethodField(read_only=True)
    ...
    def validate(self, data):
        ...
        validators.validate_jarjestamismuoto_koodi(jarjestamismuoto_koodi, data["alkamis_pvm"], data.get("paattymis_pvm"))
    ...
    def _validate_paos_specific_data(self, lapsi_obj, jarjestamismuoto_koodi):
        ...
        if lapsi_obj.paos_organisaatio is not None:
            check_if_oma_organisaatio_and_paos_organisaatio_have_paos_agreement(
                lapsi_obj.oma_organisaatio, lapsi_obj.paos_organisaatio
            )
    ...
    def _validate_jarjestamismuoto(self, lapsi_obj, jarjestamismuoto_koodi):
        ...
        if lapsi_obj.vakatoimija is not None:
            if lapsi_obj.vakatoimija.kunnallinen_kytkin and jarjestamismuoto_koodi not in jarjestamismuoto_koodit_kunta:
                ...
                not lapsi_obj.vakatoimija.kunnallinen_kytkin and jarjestamismuoto_koodi not in jarjestamismuoto_koodit_yksityinen
            ):
    ...
    def get_varhaiskasvatussuhteet_top(self, instance):
        ...
        oid_list = (
            [lapsi.vakatoimija.organisaatio_oid]
...
def _validate_toimipaikka_with_vakajarjestaja_of_lapsi(toimipaikka, lapsi):
    """
    Validate that given toimipaikka belongs to Organisaatio that Lapsi is linked to
...
class VarhaiskasvatussuhdeSerializer(RequiredLahdejarjestelmaMixin, serializers.HyperlinkedModelSerializer):
    id = serializers.IntegerField(read_only=True)
    varhaiskasvatuspaatos = VarhaiskasvatuspaatosPermissionCheckedHLField(
        required=False, view_name="varhaiskasvatuspaatos-detail"
    )
    varhaiskasvatuspaatos_tunniste = TunnisteRelatedField(
        object_type=Varhaiskasvatuspaatos,
    ...
    def _validate_paivamaarat_varhaiskasvatussuhde_toimipaikka(self, data):
        ...
        if toimipaikka_paattymis_pvm is not None:
            if not validators.validate_paivamaara1_before_paivamaara2(
                data["alkamis_pvm"], toimipaikka_paattymis_pvm, can_be_same=True
            ):
                raise ValidationError({"errors": [ErrorMessages.VS009.value]})
            if vakasuhde_paattymis_pvm is not None and not validators.validate_paivamaara1_before_paivamaara2(
                vakasuhde_paattymis_pvm, toimipaikka_paattymis_pvm, can_be_same=True
            ):
...
class PaosToimintaSerializer(serializers.HyperlinkedModelSerializer):
    ...
    paos_toimipaikka = ToimipaikkaHLField(
        view_name="toimipaikka-detail",
        required=False,
        allow_null=True,
        default=None,
    )
    paos_toimipaikka_oid = OidRelatedField(
        object_type=Toimipaikka,
    ...
    def validate(self, data):
        oma_org = data.get("oma_organisaatio")
        paos_org = data.get("paos_organisaatio")
        paos_tp = data.get("paos_toimipaikka")

        if paos_org and paos_tp:
            raise serializers.ValidationError({"errors": [ErrorMessages.PT006.value]}, code="invalid")

        if not paos_org and not paos_tp:
            raise serializers.ValidationError({"errors": [ErrorMessages.PT007.value]}, code="invalid")

        if not oma_org:
            raise serializers.ValidationError({"oma_organisaatio": [ErrorMessages.GE001.value]}, code="invalid")

        if not self.context["request"].user.has_perm("view_organisaatio", oma_org):
            raise serializers.ValidationError({"oma_organisaatio": [ErrorMessages.GE008.value]}, code="invalid")

        if paos_org and not paos_org.kunnallinen_kytkin:
...
class PaosOikeusSerializer(serializers.HyperlinkedModelSerializer):
    id = serializers.IntegerField(read_only=True)
    jarjestaja_kunta_organisaatio = OrganisaatioHLField(view_name="organisaatio-detail", read_only=True)
    tuottaja_organisaatio = OrganisaatioHLField(view_name="organisaatio-detail", read_only=True)
    voimassa_kytkin = serializers.BooleanField(read_only=True)
    tallentaja_organisaatio = OrganisaatioHLField(view_name="organisaatio-detail", required=False)
    tallentaja_organisaatio_oid = OidRelatedField(
        object_type=Organisaatio,
...
class TukipaatosSerializer(serializers.HyperlinkedModelSerializer):
    id = serializers.IntegerField(read_only=True)
    vakajarjestaja = OrganisaatioPermissionCheckedHLField(
        view_name="organisaatio-detail", required=False, permission_groups=[Z4_CasKayttoOikeudet.TUEN_TIEDOT_TALLENTAJA]
    )
    vakajarjestaja_oid = OidRelatedField(
        object_type=Organisaatio,
        parent_field="vakajarjestaja",
    ...
    class Meta:
        ...
        read_only_fields = ["tilastointi_pvm", "luonti_pvm"]

    @caching_to_representation("tukipaatos")
    def to_representation(self, instance):
        # Include read-only fields in the response
        data = super().to_representation(instance)
        data["tilastointi_pvm"] = instance.tilastointi_pvm
        return data

    def create(self, validated_data):
        today = datetime.date.today()
        validated_data["tilastointi_pvm"] = get_tilastointi_pvm(today, TukipaatosAikavali.objects.all())
        return super().create(validated_data)

    def validate(self, data):
        user = self.context["request"].user
        is_oph_user = user.is_superuser or is_oph_staff(user)
        today = datetime.date.today()
        timeframes = TukipaatosAikavali.objects.all()
        with ViewSetValidator() as validator:
            # Skip date validation id admin/OPH user
            if not is_oph_user:
                self.validate_current_date_in_allowed_timeframe(validator, today, timeframes)
            self.validate_uniqueness(data, validator, today, timeframes)
            return data

    def validate_current_date_in_allowed_timeframe(self, validator, today, timeframes):
        """Check if current date is within allowed timeframe for Tukipaatos creation"""

        errors, _ = find_active_timeframe(today, timeframes)
        if errors:
            raise ValidationError({"errors": errors})

        if self._is_updating_request():
            _, _, next_tilastointi_pvm = get_next_timeframe(today, timeframes)
            if self.instance.tilastointi_pvm < next_tilastointi_pvm:
                # Tukipaatos is being updated, but it has already been reported
                validator.error("errors", ErrorMessages.TT003.value)

    def validate_uniqueness(self, data, validator, today, timeframes):
        """
        Check that there is no existing Tukipaatos with the same vakajarjestaja, ikaryhma_koodi,
        tilastointi_pvm, and tuentaso_koodi.
        """
        vakajarjestaja = data.get("vakajarjestaja", getattr(self.instance, "vakajarjestaja", None))
        yksityinen_jarjestaja = data.get("yksityinen_jarjestaja", getattr(self.instance, "yksityinen_jarjestaja", None))
        ikaryhma_koodi = data.get("ikaryhma_koodi", getattr(self.instance, "ikaryhma_koodi", None)).upper()
        tuentaso_koodi = data.get("tuentaso_koodi", getattr(self.instance, "tuentaso_koodi", None)).upper()
        tilastointi_pvm = self.instance.tilastointi_pvm if self.instance else get_tilastointi_pvm(today, timeframes)

        # Query the database to find existing Tukipaatos objects
        existing_tukipaatokset = Tukipaatos.objects.filter(
        ...
        # If there are existing Tukipaatos objects, raise a validation error
        if self.instance:
            existing_tukipaatokset = existing_tukipaatokset.exclude(pk=self.instance.pk)

        if existing_tukipaatokset.exists():
            validator.error("errors", ErrorMessages.TT004.value)

    def _is_updating_request(self):
...
class OrganisaatioYhteenvetoSerializer(serializers.Serializer):
    ...
    vuokrattu_henkilosto_maara_kuluva_vuosi = serializers.IntegerField(read_only=True, allow_null=True)
    vuokrattu_henkilosto_tunnit_kuluva_vuosi = serializers.FloatField(read_only=True, allow_null=True)
    tuen_tiedot_kunnallinen_tt_01 = serializers.IntegerField(read_only=True)
    tuen_tiedot_kunnallinen_tt_02 = serializers.IntegerField(read_only=True)
    tuen_tiedot_kunnallinen_tt_03 = serializers.IntegerField(read_only=True)
    tuen_tiedot_yksityinen_tt_01 = serializers.IntegerField(read_only=True)
    tuen_tiedot_yksityinen_tt_02 = serializers.IntegerField(read_only=True)
    tuen_tiedot_yksityinen_tt_03 = serializers.IntegerField(read_only=True)
    tuen_tiedot_kunnallinen_tt_01_date = serializers.CharField(read_only=True)
    tuen_tiedot_kunnallinen_tt_02_date = serializers.CharField(read_only=True)
    tuen_tiedot_kunnallinen_tt_03_date = serializers.CharField(read_only=True)
    tuen_tiedot_yksityinen_tt_01_date = serializers.CharField(read_only=True)
    tuen_tiedot_yksityinen_tt_02_date = serializers.CharField(read_only=True)
    tuen_tiedot_yksityinen_tt_03_date = serializers.CharField(read_only=True)
    is_kunta = serializers.BooleanField(read_only=True)
...
class LapsiKoosteSerializer(serializers.Serializer):
    ...
    muutos_pvm = serializers.DateTimeField(read_only=True)
    luonti_pvm = serializers.DateTimeField(read_only=True)
...
class PaosToimijatSerializer(serializers.HyperlinkedModelSerializer):
    ...
    vakajarjestaja_url = serializers.HyperlinkedRelatedField(
        view_name="organisaatio-detail", source="paos_organisaatio.id", read_only=True
    )
    ...
    def get_paos_oikeus(self, instance):
        paos_oikeus = (
            PaosOikeus.objects.filter(
                jarjestaja_kunta_organisaatio=instance.paos_organisaatio, tuottaja_organisaatio=instance.oma_organisaatio
            )
    ...
    class Meta:
        model = PaosToiminta
        exclude = ("luonti_pvm", "muutos_pvm", "oma_organisaatio", "paos_organisaatio", "paos_toimipaikka")


class PaosToimipaikatSerializer(serializers.HyperlinkedModelSerializer):
    paos_toiminta_url = serializers.HyperlinkedRelatedField(view_name="paostoiminta-detail", source="id", read_only=True)
    paos_toiminta_id = serializers.IntegerField(source="id", read_only=True)
    toimija_url = serializers.HyperlinkedRelatedField(
        view_name="organisaatio-detail", source="paos_toimipaikka.vakajarjestaja.id", read_only=True
    )
    ...
    toimipaikka_url = serializers.HyperlinkedRelatedField(
        view_name="toimipaikka-detail", source="paos_toimipaikka.id", read_only=True
    )
...
class PaosOrganisaatioSerializer(serializers.HyperlinkedModelSerializer):
    ...
    def to_representation(self, instance):
        return super(PaosOrganisaatioSerializer, self).to_representation(instance)
...
class ToimipaikkaKoosteSerializer(serializers.ModelSerializer):
    ...
    toiminnalliset_painotukset = ToimipaikkaKoosteToiminnallinenPainotusSerializer(
        source="toiminnallisetpainotukset", many=True, read_only=True
    )

    class Meta:
        model = Toimipaikka
        exclude = ("vakajarjestaja",)


class ToimipaikkaKoosteV2Serializer(ToimipaikkaKoosteSerializer):
    class Meta(ToimipaikkaKoosteSerializer.Meta):
        exclude = ("vakajarjestaja",)
