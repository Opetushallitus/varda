...
from varda.models import (Henkilo, Organisaatio, Taydennyskoulutus, Toimipaikka, Lapsi, Tutkinto, Varhaiskasvatussuhde,
                          PaosToiminta, PaosOikeus, Z3_AdditionalCasUserFields, Z4_CasKayttoOikeudet, LoginCertificate,
                          Maksutieto, Tyontekija, Tyoskentelypaikka, TaydennyskoulutusTyontekija)
...
VAKA_GROUPS = (Z4_CasKayttoOikeudet.PAAKAYTTAJA, Z4_CasKayttoOikeudet.PALVELUKAYTTAJA, Z4_CasKayttoOikeudet.KATSELIJA,
               ...
               Z4_CasKayttoOikeudet.HUOLTAJATIEDOT_TALLENTAJA, Z4_CasKayttoOikeudet.TUEN_TIEDOT_TALLENTAJA,
...
class ClientCertPermissions(permissions.BasePermission):
    ...
    def has_permission(self, request, view):
        ...
        return (user.is_superuser or
                is_oph_staff(user) or
                user.groups.filter(name__startswith=Z4_CasKayttoOikeudet.LUOVUTUSPALVELU).exists())
...
def _user_has_certificate_access(user, request):
    ...
    return is_cert_auth and user_permission_groups_in_organization(user, organisaatio_oid,
...
def assign_vuokrattu_henkilosto_permissions(vuokrattu_henkilosto, reassign=False):
    ...
    assign_henkilosto_object_permissions(vuokrattu_henkilosto,
                                         (vuokrattu_henkilosto.vakajarjestaja.organisaatio_oid,))
...
def get_related_organisaatio_for_user(user, groups=None):
    ...
    return (Organisaatio.objects
            .filter(Q(organisaatio_oid__in=organisaatio_oid_set) |
                    Q(toimipaikat__organisaatio_oid__in=organisaatio_oid_set))
            .distinct()
            .order_by('id'))
...
def assign_tukipaatos_permissions(tukipaatos, reassign=False):
    ...
    assign_vaka_object_permissions(tukipaatos,
                                   (tukipaatos.vakajarjestaja.organisaatio_oid,))
