...
from django.db.models import F, IntegerField, Model, Q, QuerySet, Value
...
from varda.misc_queries import (
    get_lapsi_for_maksutieto,
    get_organisaatio_oid_for_taydennyskoulutus,
...
class RaportitPermissions(permissions.BasePermission):
    ...
    def has_permission(self, request, view):
        ...
        return (
            user.is_superuser
            or is_oph_staff(user)
            or (
                not Z3_AdditionalCasUserFields.objects.filter(user=user, kayttajatyyppi="PALVELU").exists()
                and user.groups.filter(name__startswith=Z4_CasKayttoOikeudet.RAPORTTIEN_KATSELIJA).exists()
            )
        )


class MassahakuPermissions(permissions.BasePermission):
    def has_permission(self, request, view):
        user = request.user
        if not user or not user.is_authenticated:
            return False
        if AdminOrOPHUser().has_permission(request, view):
            return True
        if not Z3_AdditionalCasUserFields.objects.filter(user=user, kayttajatyyppi="PALVELU").exists():
            return False
        organization_oids = get_organisaatio_oids_from_groups(user, f"{Z4_CasKayttoOikeudet.RAPORTTIEN_KATSELIJA}_")
        return len(organization_oids) == 1


class ClientCertPermissions(permissions.BasePermission):
    """
    Allows access to admin, OPH users, and users who belong to a VARDA_LUOVUTUSPALVELU group
    """
    ...
    def has_permission(self, request, view):
        ...
        return (
            user.is_superuser
            or is_oph_staff(user)
            or user.groups.filter(name__startswith=Z4_CasKayttoOikeudet.LUOVUTUSPALVELU).exists()
        )


class FrontendAccessOnly(permissions.BasePermission):
    """
    Allow access only from frontend nginx proxy
    """

    def has_permission(self, request, view):
        return request.headers.get("X-Proxied-From", None) == settings.VARDA_FRONTEND_SECRET
...
def _user_has_certificate_access(user, request):
    ...
    login_certificate = (
        LoginCertificate.objects.alias(request_path=Value(request.path))
    ...
    # in the correct organization specified in LoginCertificate object
    organisaatio_oid = getattr(login_certificate.organisaatio, "organisaatio_oid", None)
    return (
        is_cert_auth
        and user_permission_groups_in_organization(user, organisaatio_oid, [Z4_CasKayttoOikeudet.LUOVUTUSPALVELU]).exists()
    )
...
def _parse_nginx_subject(subject_dn_string):
    ...
    return {
        subject_data_pair[0]: subject_data_pair[1]
    ...
    if not oma_organisaatio:
        ...
        if toimipaikka_obj:
            paos_toiminta = PaosToiminta.objects.filter(
                Q(voimassa_kytkin=True) & Q(oma_organisaatio=oma_organisaatio, paos_toimipaikka=toimipaikka_obj)
        ...
        if paos_oikeus:
            ...
            if paos_voimassa_kytkin and user_has_tallentaja_permission_in_organization(
                paos_tallentaja_organisaatio.organisaatio_oid, user
            ):
                """
...
def user_has_huoltajatieto_tallennus_permissions_to_correct_organization(
    user, vakajarjestaja_organisaatio_oid, toimipaikka_qs=None
):
    """
...
def object_ids_organization_has_permissions_to(organisaatio_oid, model):
    ...
    return set(
        GroupObjectPermission.objects.filter(group__name__endswith=organisaatio_oid, content_type=content_type.id)
...
def get_tyontekija_and_toimipaikka_lists_for_taydennyskoulutus(taydennyskoulutus_tyontekija_list):
    ...
    if taydennyskoulutus_tyontekija_list and isinstance(taydennyskoulutus_tyontekija_list[0], TaydennyskoulutusTyontekija):
        taydennyskoulutus_tyontekija_list = [
            model_to_dict(taydennyskoulutus_tyontekija, fields=["tehtavanimike_koodi", "tyontekija"])
    ...
    tyontekija_id_list = [
        taydennyskoulutus_tyontekija["tyontekija"] for taydennyskoulutus_tyontekija in taydennyskoulutus_tyontekija_list
    ]

    # List of lists of toimipaikka oids
    # Toimipaikka level user must have permissions to at least one toimipaikka in a list
    toimipaikka_oid_list_list = [
        Tyoskentelypaikka.objects.filter(
            palvelussuhde__tyontekija=taydennyskoulutus_tyontekija["tyontekija"],
            tehtavanimike_koodi=taydennyskoulutus_tyontekija["tehtavanimike_koodi"],
...
def is_correct_taydennyskoulutus_tyontekija_permission(user, taydennyskoulutus_tyontekija_list, throws=True):
    ...
    if not is_user_permission(user, permission_format.format(vakajarjestaja_oid)):
        ...
        for toimipaikka_oid_list in toimipaikka_oid_list_list:
            if not any(
                is_user_permission(user, permission_format.format(toimipaikka_oid)) for toimipaikka_oid in toimipaikka_oid_list
            ):
...
def get_available_tehtavanimike_codes_for_user(user, tyontekija, has_permissions=False, organisaatio_oid_list=()):
    permission_qs = user_permission_groups_in_organization(user, tyontekija.vakajarjestaja.organisaatio_oid, TYONTEKIJA_GROUPS)
    if user.is_superuser or is_oph_staff(user) or has_permissions or permission_qs.exists():
        ...
        tehtavanimike_set = set(tyontekija.palvelussuhteet.all().values_list("tyoskentelypaikat__tehtavanimike_koodi", flat=True))
    else:
        if not organisaatio_oid_list:
            organisaatio_oid_list = get_taydennyskoulutus_tyontekija_group_organisaatio_oids(user)
        tehtavanimike_set = set(
            Tyoskentelypaikka.objects.filter(
                (
                    Q(toimipaikka__organisaatio_oid__in=organisaatio_oid_list)
...
def get_tyontekija_filters_for_taydennyskoulutus_groups(user, prefix=""):
    ...
    return (
        Q(**{prefix + "vakajarjestaja__organisaatio_oid__in": organisaatio_oids})
        | Q(**{prefix + "palvelussuhteet__tyoskentelypaikat__toimipaikka__organisaatio_oid__in": organisaatio_oids})
    ), organisaatio_oids
...
def toimipaikka_tallentaja_pidempipoissaolo_has_perm_to_add(user, vakajarjestaja_oid, validated_data):
    ...
    if user.is_superuser or user.groups.filter(name="HENKILOSTO_TYONTEKIJA_TALLENTAJA_{}".format(vakajarjestaja_oid)):
        ...
        return Tyoskentelypaikka.objects.filter(
            Q(palvelussuhde=validated_data["palvelussuhde"], toimipaikka__organisaatio_oid__in=user_perm_oids)
        ).exists()


def user_belongs_to_correct_groups(user, instance, permission_groups=(), accept_toimipaikka_permission=False, check_paos=False):
    """
    ...
    if not user.is_superuser and not is_oph_staff(user) and permission_groups:
        ...
        if isinstance(instance, Toimipaikka):
            ...
            if check_paos:
                ...
                paos_organisaatio_oid_list = PaosToiminta.objects.filter(
                    paos_toimipaikka=instance, voimassa_kytkin=True
...
def get_vakajarjestajat_filter_for_raportit(request):
    ...
    for vakajarjestaja_id in vakajarjestaja_ids_splitted:
        ...
        permission_group_qs = user_permission_groups_in_organization(
            user, vakajarjestaja_obj.organisaatio_oid, [Z4_CasKayttoOikeudet.RAPORTTIEN_KATSELIJA]
        )
...
def parse_toimipaikka_id_list(user, toimipaikka_ids_string, required_permission_groups, include_paos=False):
    ...
    for toimipaikka_id in toimipaikka_ids_splitted:
        ...
        if include_paos:
            paos_oid_list = set(
                PaosToiminta.objects.filter(Q(paos_toimipaikka=toimipaikka)).values_list(
                    "oma_organisaatio__organisaatio_oid", flat=True
                )
            )
        ...
        if (
            user.is_superuser
            or is_oph_staff(user)
            or user_permission_groups_in_organizations(user, oid_list, required_permission_groups).exists()
        ):
...
def delete_permissions_from_object_instance_by_oid(instance, organisaatio_oid):
    ...
    GroupObjectPermission.objects.filter(
        content_type=content_type, object_pk=instance.id, group__name__endswith=organisaatio_oid
    ).delete()
...
def delete_all_user_permissions(user, delete_henkilo_permissions=True):
    ...
    user.groups.clear()
    if cas_fields := user.additional_cas_user_fields:
        cas_fields.all_groups.clear()
...
def assign_general_object_permissions(instance, oid_list, view_only=False):
    assign_or_remove_object_permissions(
        instance, oid_list, VAKA_GROUPS + HENKILOSTO_GROUPS + GENERAL_GROUPS, view_only=view_only, assign=True
    )
...
def assign_toimipaikka_permissions(toimipaikka, reassign=False):
    ...
    assign_general_object_permissions(
        toimipaikka,
        (
    ...
    paos_toiminta_qs = (
        PaosToiminta.objects.filter(paos_toimipaikka=toimipaikka)
        .values_list("oma_organisaatio_id", flat=True)
        .distinct("oma_organisaatio_id")
        .order_by("oma_organisaatio_id")
    )
    for jarjestaja_organisaatio_id in paos_toiminta_qs:
        paos_oikeus = PaosOikeus.objects.filter(
            jarjestaja_kunta_organisaatio_id=jarjestaja_organisaatio_id,
            tuottaja_organisaatio=toimipaikka.vakajarjestaja,
            voimassa_kytkin=True,
        )
        varhaiskasvatussuhde_qs = Varhaiskasvatussuhde.objects.filter(
            toimipaikka=toimipaikka, varhaiskasvatuspaatos__lapsi__oma_organisaatio_id=jarjestaja_organisaatio_id
        )
    ...
    assign_all_taydennyskoulutus_toimipaikka_permissions.delay(
        toimipaikka.vakajarjestaja.organisaatio_oid, toimipaikka.organisaatio_oid
    )
...
def assign_all_taydennyskoulutus_toimipaikka_permissions(organisaatio_oid, toimipaikka_oid):
    taydennyskoulutus_qs = (
        Taydennyskoulutus.objects.filter(tyontekijat__vakajarjestaja__organisaatio_oid=organisaatio_oid)
        .distinct("id")
        .order_by("id")
    )
...
def assign_toiminnallinen_painotus_permissions(toiminnallinen_painotus, reassign=False):
    ...
    assign_vaka_object_permissions(
...
def assign_kielipainotus_permissions(kielipainotus, reassign=False):
    ...
    assign_vaka_object_permissions(
        kielipainotus,
        (
            kielipainotus.toimipaikka.organisaatio_oid,
...
def assign_lapsi_permissions(lapsi, toimipaikka_oid=None, user=None, reassign=False):
    ...
    if lapsi.paos_kytkin:
        ...
        assign_vaka_object_permissions(
            ...
            view_only=is_not_tuottaja_tallentaja,
            paos_tuottaja=True,
        )
...
def assign_varhaiskasvatuspaatos_permissions(vakapaatos, toimipaikka_oid=None, reassign=False):
    ...
    if lapsi.paos_kytkin:
        ...
        assign_vaka_object_permissions(
            ...
            view_only=tuottaja_oid != tallentaja_oid,
            paos_tuottaja=True,
        )
...
def assign_varhaiskasvatussuhde_permissions(vakasuhde, reassign=False):
    ...
    if lapsi.paos_kytkin:
        ...
        assign_vaka_object_permissions(
            vakasuhde.varhaiskasvatuspaatos, (toimipaikka_oid,), view_only=is_not_tuottaja_tallentaja, paos_tuottaja=True
        )
        assign_vaka_object_permissions(lapsi, (toimipaikka_oid,), view_only=is_not_tuottaja_tallentaja, paos_tuottaja=True)
        assign_vaka_object_permissions(
            lapsi.henkilo, (toimipaikka_oid,), view_only=is_not_tuottaja_tallentaja, paos_tuottaja=True
        )
    else:
        # Organisaatio and Toimipaikka level permissions for Varhaiskasvatussuhde object
        assign_vaka_object_permissions(
...
def assign_maksutieto_permissions(maksutieto, reassign=False):
    ...
    if lapsi.paos_kytkin:
        ...
        toimipaikka_oid_qs = (
            Toimipaikka.objects.filter(varhaiskasvatussuhteet__varhaiskasvatuspaatos__lapsi=lapsi)
            .values_list("organisaatio_oid", flat=True)
            .distinct("organisaatio_oid")
            .order_by("organisaatio_oid")
        )
...
def assign_paos_toiminta_permissions(paos_toiminta, reassign=False):
    ...
    jarjestaja_organisaatio = (
        paos_toiminta.oma_organisaatio if paos_toiminta.paos_toimipaikka else paos_toiminta.paos_organisaatio
    )
    tuottaja_organisaatio = (
        paos_toiminta.paos_toimipaikka.vakajarjestaja if paos_toiminta.paos_toimipaikka else paos_toiminta.oma_organisaatio
    )
    paos_oikeus = PaosOikeus.objects.filter(
    ...
    if paos_oikeus and paos_oikeus.voimassa_kytkin:
        ...
        paos_toiminta_qs = (
            PaosToiminta.objects.filter(
                voimassa_kytkin=True,
                oma_organisaatio=jarjestaja_organisaatio,
                paos_toimipaikka__vakajarjestaja=tuottaja_organisaatio,
            )
            .values_list("paos_toimipaikka_id", flat=True)
            .distinct("paos_toimipaikka_id")
            .order_by("paos_toimipaikka_id")
        )
...
def remove_paos_toiminta_permissions(paos_toiminta):
    ...
    if (
        paos_toiminta.paos_toimipaikka
        and not Varhaiskasvatussuhde.objects.filter(
            toimipaikka=paos_toiminta.paos_toimipaikka, varhaiskasvatuspaatos__lapsi__oma_organisaatio=jarjestaja_organisaatio
        ).exists()
    ):
        # paos_toimipaikka is known and there are no Lapsi objects related to that Toimipaikka,
        # permissions can be removed
        remove_vaka_object_permissions(paos_toiminta.paos_toimipaikka, (jarjestaja_organisaatio.organisaatio_oid,))
        ...
        paos_toiminta_qs = (
            ...
            .exclude(
                **{
                    "paos_toimipaikka__varhaiskasvatussuhteet__varhaiskasvatuspaatos__"
...
def reassign_paos_permissions(jarjestaja_organisaatio_id, tuottaja_organisaatio_id):
    ...
    """
    lapsi_qs = Lapsi.objects.filter(oma_organisaatio_id=jarjestaja_organisaatio_id, paos_organisaatio_id=tuottaja_organisaatio_id)
...
def assign_tyontekija_permissions(tyontekija, toimipaikka_oid=None, user=None, reassign=False):
    ...
    assign_henkilosto_object_permissions(
        tyontekija,
        (
            tyontekija.vakajarjestaja.organisaatio_oid,
            toimipaikka_oid,
        ),
    )
    # Organisaatio and Toimipaikka level permissions for Henkilo object
...
def assign_tutkinto_permissions(tutkinto, toimipaikka_oid=None, reassign=False):
    ...
    assign_henkilosto_object_permissions(
        tutkinto,
        (
            tutkinto.vakajarjestaja.organisaatio_oid,
            toimipaikka_oid,
        ),
    )
    ...
    toimipaikka_oid_qs = (
        Toimipaikka.objects.filter(
            tyoskentelypaikat__palvelussuhde__tyontekija__vakajarjestaja=tutkinto.vakajarjestaja,
...
def assign_palvelussuhde_permissions(palvelussuhde, toimipaikka_oid=None, reassign=False):
    ...
    assign_henkilosto_object_permissions(
        palvelussuhde,
        (
            palvelussuhde.tyontekija.vakajarjestaja.organisaatio_oid,
            toimipaikka_oid,
        ),
    )
...
def assign_pidempi_poissaolo_permissions(pidempi_poissaolo, reassign=False):
    ...
    toimipaikka_oid_qs = (
        palvelussuhde.tyoskentelypaikat.values_list("toimipaikka__organisaatio_oid", flat=True)
        .distinct("toimipaikka__organisaatio_oid")
        .order_by("toimipaikka__organisaatio_oid")
    )
...
def assign_taydennyskoulutus_permissions(taydennyskoulutus, reassign=False):
    ...
    toimipaikka_oid_qs = (
        taydennyskoulutus.tyontekijat.values_list("palvelussuhteet__tyoskentelypaikat__toimipaikka__organisaatio_oid", flat=True)
...
def assign_taydennyskoulutus_all_toimipaikka_permissions(taydennyskoulutus_id, organisaatio_oid):
    ...
    toimipaikka_oid_qs = Toimipaikka.objects.filter(vakajarjestaja__organisaatio_oid=organisaatio_oid).values_list(
        "organisaatio_oid", flat=True
    )
    assign_henkilosto_object_permissions(taydennyskoulutus, toimipaikka_oid_qs)
...
@transaction.atomic
def assign_vuokrattu_henkilosto_permissions(vuokrattu_henkilosto, reassign=False):
    """
    Assign object permissions for VuokrattuHenkilosto instance
    :param vuokrattu_henkilosto: VuokrattuHenkilosto instance
    :param reassign: delete old permissions before assignment
    """
    if reassign:
        delete_object_permissions(vuokrattu_henkilosto)

    # Organisaatio level permissions for VuokrattuHenkilosto object
    assign_henkilosto_object_permissions(vuokrattu_henkilosto, (vuokrattu_henkilosto.vakajarjestaja.organisaatio_oid,))


def get_related_organisaatio_for_user(user, groups=None):
    """
    Determine which Organisaatio objects user has permissions in
    :param user: User object
    :param groups: Group QuerySet (defaults to user.groups)
    :return: Organisaatio QuerySet
    """
    if user.is_superuser or is_oph_staff(user):
        return Organisaatio.objects.filter(organisaatio_oid=settings.OPETUSHALLITUS_ORGANISAATIO_OID)

    if not groups:
        groups = user.groups

    user_group_names = groups.values_list("name", flat=True)

    # Matches permission group names, <NAME>_<OID> e.g. VARDA-TALLENTAJA_1.2.246.562.10.93957375488
    regex_pattern = re.compile(r".*_([\d\.]*)")
    organisaatio_oid_set = {
        match.group(1) for group_name in user_group_names if (match := regex_pattern.fullmatch(group_name)) and match.group(1)
    }

    # organisaatio_oid might be that of Vakajarjestaja or Toimipaikka
    return (
        Organisaatio.objects.filter(
            Q(organisaatio_oid__in=organisaatio_oid_set) | Q(toimipaikat__organisaatio_oid__in=organisaatio_oid_set)
        )
        .distinct()
        .order_by("id")
    )
...
def activate_all_permissions_decorator(function):
    """
    Decorate ViewSet action so that User is temporarily added to all permission groups available to it
    :return: decorated function
    """

    @wraps(function)
    @transaction.atomic
    def _activate_all_permissions_wrapper(*args, **kwargs):
        request = args[1]

        user = request.user

        cas_fields = getattr(user, "additional_cas_user_fields", None)
        if user.is_anonymous or user.is_superuser or is_oph_staff(user) or not cas_fields:
            # User is anonymous, admin or OPH user, or cas_fields are missing, do not modify groups
            return function(*args, **kwargs)

        original_groups = list(user.groups.all())
        # Add User to all permission groups available to it
        user.groups.set(list(cas_fields.all_groups.all()) + original_groups)
        # Handle request in try block, exceptions are raised after permissions are rolled back
        try:
            # Delete object id cache for user before running the function
            delete_object_ids_user_has_permissions(user.id)
            response = function(*args, **kwargs)
        finally:
            # Reset permission groups to original ones
            user.groups.set(original_groups)

        return response

    return _activate_all_permissions_wrapper


@transaction.atomic
def assign_tukipaatos_permissions(tukipaatos, reassign=False):
    """
    Assign object permissions for Tukipaatos instance
    :param tukipaatos: Tukipaatos instance
    :param reassign: delete old permissions before assignment
    """
    if reassign:
        delete_object_permissions(tukipaatos)

    # Organisaatio level permissions for Tukipaatos object
    assign_vaka_object_permissions(tukipaatos, (tukipaatos.vakajarjestaja.organisaatio_oid,))
