...
from varda.models import (Organisaatio, Lapsi, PaosOikeus, PaosToiminta, Toimipaikka, Z2_Code, Z4_CasKayttoOikeudet,
...
def _execute_yearly_report_query(cursor, query, params, format_args=(), return_dict=False, organisaatio_id=None,
                                 organisaatio_field_list=(), group_by_list=()):
    if organisaatio_id:
    ...
    if group_by_list or return_dict:
...
def get_yearly_report_toimipaikka_data(poiminta_pvm, tilasto_pvm, organisaatio_id):
    ...
    with connections[settings.READER_DB].cursor() as cursor:
        ...
        return _execute_yearly_report_query(cursor, query, params, format_args=format_args, return_dict=True,
    ...
    with connections[settings.READER_DB].cursor() as cursor:
        query = '''
            ...
            JOIN (
                SELECT DISTINCT ON (id) * FROM varda_historicaltoimipaikka
                WHERE history_date <= %s ORDER BY id, history_date DESC
            ) tp ON tp.id = kp.toimipaikka_id AND tp.history_type != '-' AND tp.alkamis_pvm <= %s AND
...
def get_yearly_report_lapsi_ids(poiminta_pvm, tilasto_pvm, organisaatio_id):
    with connections[settings.READER_DB].cursor() as cursor:
        ...
        return _execute_yearly_report_query(cursor, query, params, return_dict=True, organisaatio_id=organisaatio_id,
                                            organisaatio_field_list=organisaatio_field_list)
...
def get_yearly_report_taydennyskoulutus_data(poiminta_pvm, tilasto_pvm, tyoskentelypaikka_id_list):
    with connections[settings.READER_DB].cursor() as cursor:
        ...
        '''
        params = [poiminta_pvm, tyoskentelypaikka_id_list, poiminta_pvm, poiminta_pvm, poiminta_pvm, poiminta_pvm,
                  tilasto_pvm.year]
        group_by_list = ['tehtavanimike']
...
def get_yearly_report_fixed_term_data(poiminta_pvm, year, month, organisaatio_id):
    ...
    with connections[settings.READER_DB].cursor() as cursor:
        ...
        '''
        params = [poiminta_pvm, poiminta_pvm, poiminta_pvm, alkamis_pvm_lt, paattymis_pvm_gte, alkamis_pvm_lt,
                  paattymis_pvm_gte]
        organisaatio_field_list = ['ty.vakajarjestaja_id']
