...
ORG_FILTER_PLACEHOLDER = "ORGANISAATIO_FILTER_PLACEHOLDER"
...
def _execute_yearly_report_query(
    ...
    organisaatio_sql = ""
    if organisaatio_id:
        # TODO: move to its own function and execute in the query function, because what if other filters will be made?
        if isinstance(params, dict):
            organisaatio_filter_list = [f"{field} = {organisaatio_id}" for field in organisaatio_field_list]
            organisaatio_sql = " AND (" + " OR ".join(organisaatio_filter_list) + ")"
        else:
    ...
    query = query.replace(ORG_FILTER_PLACEHOLDER, organisaatio_sql)
...
def get_yearly_report_tyontekija_ids(poiminta_pvm, tilasto_pvm, organisaatio_id):
    with connections[settings.READER_DB].cursor() as cursor:
        query = f"""
            with pa AS (
                SELECT DISTINCT ON (p.id) p.id, p.tyontekija_id, p.history_type, p.alkamis_pvm, p.paattymis_pvm
                FROM varda_historicalpalvelussuhde p
                WHERE p.history_date <= %(history_cutoff)s::timestamptz
                ORDER BY p.id, p.history_date DESC
            ),
            ty AS (
                SELECT DISTINCT ON (t.id) t.id, t.vakajarjestaja_id, t.history_type
                FROM varda_historicaltyontekija t
                WHERE t.history_date <= %(history_cutoff)s::timestamptz
                ORDER BY t.id, t.history_date DESC
            ),
            -- join and filter pa and ty to limit computation in following operations
             eligible AS (
                 SELECT pa.id AS palvelussuhde_id, pa.alkamis_pvm, pa.paattymis_pvm, ty.vakajarjestaja_id
                 FROM pa JOIN ty ON ty.id = pa.tyontekija_id
                 WHERE pa.history_type <> '-'
                   AND ty.history_type <> '-'
                   AND pa.alkamis_pvm <= DATE %(statistics_date)s
                   AND (pa.paattymis_pvm >= DATE %(statistics_date)s OR pa.paattymis_pvm IS NULL)
                   {ORG_FILTER_PLACEHOLDER}
             ),
             pp_latest AS (
                 SELECT DISTINCT ON (pp.id) pp.id, pp.palvelussuhde_id, pp.history_type, pp.alkamis_pvm, pp.paattymis_pvm
                 FROM varda_historicalpidempipoissaolo pp
                 JOIN eligible e ON e.palvelussuhde_id = pp.palvelussuhde_id
                 WHERE pp.history_date <= %(history_cutoff)s::timestamptz
                 ORDER BY pp.id, pp.history_date DESC
             ),
            pp AS (SELECT DISTINCT palvelussuhde_id
                   FROM pp_latest
                   WHERE history_type <> '-'
                   AND alkamis_pvm <= DATE %(statistics_date)s
                   AND paattymis_pvm >= DATE %(statistics_date)s
                   ),
            tyo AS (
                SELECT DISTINCT ON (t.id) t.id, t.palvelussuhde_id, t.history_type, t.alkamis_pvm, t.paattymis_pvm
                FROM varda_historicaltyoskentelypaikka t
                    JOIN eligible e
                    ON e.palvelussuhde_id = t.palvelussuhde_id
                WHERE t.history_date <= %(history_cutoff)s::timestamptz
                ORDER BY t.id, t.history_date DESC
            )
        SELECT
            array_agg(DISTINCT eligible.palvelussuhde_id) palvelussuhde_id_array,
            array_agg(DISTINCT tyo.id) tyoskentelypaikka_id_array
        FROM eligible JOIN tyo ON tyo.palvelussuhde_id = eligible.palvelussuhde_id
                WHERE tyo.history_type <> '-'
                AND tyo.alkamis_pvm <= DATE %(statistics_date)s
                AND (tyo.paattymis_pvm >= DATE %(statistics_date)s OR tyo.paattymis_pvm IS NULL)
                AND eligible.alkamis_pvm <= DATE %(statistics_date)s
                AND (eligible.paattymis_pvm >= DATE %(statistics_date)s OR eligible.paattymis_pvm IS NULL)
                AND NOT EXISTS (SELECT 1 FROM pp WHERE pp.palvelussuhde_id = eligible.palvelussuhde_id)
        """

        params = {
            "history_cutoff": poiminta_pvm,
            "statistics_date": tilasto_pvm,
        }
    ...
    with connections[settings.READER_DB].cursor() as cursor:
        query = """
            ...
            FROM (
                ...
                FROM (
                    SELECT DISTINCT ON (id) palvelussuhde_id, tehtavanimike_koodi FROM varda_historicaltyoskentelypaikka
                    WHERE history_date <= %s AND id = ANY(%s) ORDER BY id, history_date DESC
                ) tyo
                JOIN LATERAL (
                    SELECT DISTINCT ON (id) tyontekija_id, history_type FROM varda_historicalpalvelussuhde
                    WHERE history_date <= %s AND id = tyo.palvelussuhde_id ORDER BY id, history_date DESC
                ) pa ON pa.history_type != '-'
                JOIN LATERAL (
                    SELECT DISTINCT ON (id) henkilo_id, history_type FROM varda_historicaltyontekija
                    WHERE history_date <= %s AND id = pa.tyontekija_id ORDER BY id, history_date DESC
                ) ty ON ty.history_type != '-'
...
def get_yearly_report_fixed_term_data(poiminta_pvm, year, organisaatio_id):
    # Same idea with get_yearly_report_tyontekija_ids, but without PidempiPoissaolo
    # and querying a whole year and 12 separate months

    with connections[settings.READER_DB].cursor() as cursor:
        # bounds: used to limit results within a year
        # periods: 0 = whole year, 1-12 = each month of the year
        # eligible: join and filter pa and ty to limit the computation in following operations
        query = f"""
                WITH bounds AS (
                    SELECT
                        make_date(%(year)s, 1, 1) AS year_start,
                        make_date(%(year)s + 1, 1, 1) AS year_end
                ),
                     periods AS (
                         SELECT
                             0 AS ord,
                             NULL::int AS month_no,
                             b.year_start AS start_date,
                             b.year_end AS end_date
                         FROM bounds b
                         UNION ALL
                         SELECT
                             m AS ord,
                             m AS month_no,
                             make_date(%(year)s, m, 1) AS start_date,
                             (make_date(%(year)s, m, 1) + INTERVAL '1 month')::date AS end_date
                    ...
                    ty AS (
                        SELECT DISTINCT ON (t.id) t.id, t.henkilo_id, t.vakajarjestaja_id, t.history_type
                        FROM varda_historicaltyontekija t
                        WHERE t.history_date <= %(history_cutoff)s::timestamptz
                        ORDER BY t.id, t.history_date DESC
                    ),
                    eligible AS (
                        SELECT pa.id AS palvelussuhde_id, pa.alkamis_pvm, pa.paattymis_pvm, ty.henkilo_id
                        FROM pa
                            JOIN ty
                        ON ty.id = pa.tyontekija_id
                            CROSS JOIN bounds b
                        WHERE pa.history_type != '-'
                          AND ty.history_type != '-'
                          AND pa.tyosuhde_koodi = '2'
                          AND pa.alkamis_pvm < b.year_end
                          AND (pa.paattymis_pvm >= b.year_start OR pa.paattymis_pvm IS NULL)
                          {ORG_FILTER_PLACEHOLDER}
                            ),
                    tyo AS (
                        SELECT DISTINCT ON (t.id) t.id, t.palvelussuhde_id, t.alkamis_pvm, t.paattymis_pvm, t.history_type
                        FROM varda_historicaltyoskentelypaikka t
                            JOIN eligible e
                        ON e.palvelussuhde_id = t.palvelussuhde_id
                        WHERE t.history_date <= %(history_cutoff)s::timestamptz
                    ...
                    counts AS (
                        SELECT
                            p.ord,
                            p.month_no,
                            COUNT(DISTINCT e.henkilo_id) AS henkilo_count
                        FROM periods p
                            JOIN eligible e
                        ON e.alkamis_pvm < p.end_date
                            AND COALESCE(e.paattymis_pvm, 'infinity'::date) >= p.start_date
                        WHERE EXISTS (
                            SELECT 1
                            FROM tyo t
                            WHERE t.palvelussuhde_id = e.palvelussuhde_id
                          AND t.history_type != '-'
                          AND t.alkamis_pvm < p.end_date
                          AND COALESCE(t.paattymis_pvm, 'infinity'::date) >= p.start_date
                            )
                        GROUP BY p.ord, p.month_no
                    )
                SELECT
                    CASE
                        WHEN p.ord = 0 THEN 'year'
                        ELSE to_char(make_date(%(year)s, p.month_no, 1), 'YYYY-MM')
                        END AS bucket,
                    COALESCE(c.henkilo_count, 0) AS henkilo_count
                FROM periods p
                         LEFT JOIN counts c
                                   ON c.ord = p.ord
                                       AND c.month_no IS NOT DISTINCT FROM p.month_no
                ORDER BY p.ord
                """
...
def get_active_on_any_date_filter(target_date, target_date_secondary=None, prefix=""):
    """
    Get a Q filter for objects that are active for at least one day within the given date range.
    :param target_date: Start date of the requested date range
    :param target_date_secondary: End date of the requested date range; defaults to target_date
    :param prefix: Related field prefix, e.g. "tyoskentelypaikat" (__ added automatically)
    :return: Q object filter
    """
    if prefix and not prefix.endswith("__"):
        prefix += "__"

    end_date = target_date_secondary or target_date

    return Q(**{f"{prefix}alkamis_pvm__lte": end_date}) & (
        Q(**{f"{prefix}paattymis_pvm__gte": target_date}) | Q(**{f"{prefix}paattymis_pvm__isnull": True})
    )


def get_inactive_filter(target_date, target_date_secondary=None, prefix=""):
    """
    Get Q filter for getting only objects that are inactive. Optionally use prefix to filter on related data.
    ...
    return Q(**{f"{prefix}alkamis_pvm__gt": target_date}) | Q(**{f"{prefix}paattymis_pvm__lt": end_date})
