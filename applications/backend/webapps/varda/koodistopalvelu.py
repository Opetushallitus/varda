...
KOODISTOPALVELU_DICT = {
    ...
    Koodistot.kielipainotustyyppi_koodit: "vardakielipainotuksentyyppi",
    Koodistot.ikaryhma_koodit: "vardaikaryhma",
    Koodistot.tuentaso_koodit: "vardatuentasot",
}
...
def update_koodistot():
    for key, value in KOODISTOPALVELU_DICT.items():
        ...
        koodisto_obj = Z2_Koodisto.objects.update_or_create(
            name=key.value,
            defaults={"name_koodistopalvelu": value, "version": koodisto_result["versio"], "update_datetime": update_datetime},
        )[0]
        ...
        for code in code_list:
            ...
            code_obj, created = Z2_Code.objects.update_or_create(
                koodisto=koodisto_obj,
                code_value=code["koodiArvo"],
                defaults={"alkamis_pvm": code["voimassaAlkuPvm"], "paattymis_pvm": code["voimassaLoppuPvm"]},
            )
            ...
            for translation in code["metadata"]:
                ...
                Z2_CodeTranslation.objects.update_or_create(
                    code=code_obj,
                    language=translation["kieli"],
                    defaults={"name": translation_name, "description": translation_description},
                )
