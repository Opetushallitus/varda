...
from django.db.models import Q, CharField, ManyToManyField
...
def validate_email(email):
    ...
    email_regex = re.compile(
        r"^[_A-Za-z0-9-+!#$%&\'*/=?^`{|}~]+(\.[_A-Za-z0-9-+!#$%&\'*/=?^`{|}~]+)*@[A-Za-z0-9][A-Za-z0-9-]+(\.[A-Za-z0-9-]+)*(\.[A-Za-z]{2,})$"
    )
...
def validate_henkilotunnus_or_oid_needed(validated_data):
    if (
        "henkilotunnus" not in validated_data
        and "henkilo_oid" not in validated_data
        or "henkilotunnus" in validated_data
        and "henkilo_oid" in validated_data
    ):
...
def validate_henkilotunnus(henkilotunnus):
    """
    Validate the date, identification number and control character.
    """
    ...
    intermediate_character = henkilotunnus[6]

    match intermediate_character:
        case "+":
            century = "18"
        case "-" | "Y" | "X" | "W" | "V" | "U":
            century = "19"
    ...
    try:
        birthday = datetime.date(int(century + year), int(month), int(day))
    ...
    identification_number = henkilotunnus[:6] + henkilotunnus[7:10]
    control_character = henkilotunnus[10]
    control_character_option_str = "0123456789ABCDEFHJKLMNPRSTUVWXY"
    try:
        remainder = int(identification_number) % 31
        if control_character != control_character_option_str[remainder]:
            raise Exception()
    except Exception:
...
def validate_postinumero_toimipaikka(postinumero):
    """
    Validate the postal code: numeric, length 5 string
    - Cannot have '0' as the 3rd digit
    - Last digit must be '0' or '5'
    - '00099' is allowed
    """
    validate_postinumero(postinumero)
    if postinumero == "00099":
        return
    elif postinumero[2] != "0" and postinumero[-1] in ("0", "5"):
        return
    else:
        raise ValidationError([ErrorMessages.MI022.value])
...
def validate_z2_koodi(code_value, koodisto_name, alkamis_pvm=None, paattymis_pvm=None, field_name=None, paattymis_pvm_only=True):
...
def validate_maksun_peruste_koodi(maksun_peruste_koodi, alkamis_pvm=None, paattymis_pvm=None, field_name="maksun_peruste_koodi"):
    validate_z2_koodi(maksun_peruste_koodi, Koodistot.maksun_peruste_koodit.value, alkamis_pvm, paattymis_pvm, field_name)
...
def validate_jarjestamismuoto_koodi(
    jarjestamismuoto_koodi, alkamis_pvm=None, paattymis_pvm=None, field_name="jarjestamismuoto_koodi"
):
    validate_z2_koodi(jarjestamismuoto_koodi, Koodistot.jarjestamismuoto_koodit.value, alkamis_pvm, paattymis_pvm, field_name)


def validate_toimintamuoto_koodi(toimintamuoto_koodi, alkamis_pvm=None, paattymis_pvm=None, field_name="toimintamuoto_koodi"):
    validate_z2_koodi(toimintamuoto_koodi, Koodistot.toimintamuoto_koodit.value, alkamis_pvm, paattymis_pvm, field_name)


def validate_kasvatusopillinen_jarjestelma_koodi(
    kasvatusopillinen_jarjestelma_koodi, alkamis_pvm=None, paattymis_pvm=None, field_name="kasvatusopillinen_jarjestelma_koodi"
):
    validate_z2_koodi(
...
def validate_toimintapainotus_koodi(
    toimintapainotus_koodi, alkamis_pvm=None, paattymis_pvm=None, field_name="toimintapainotus_koodi"
):
    validate_z2_koodi(
        toimintapainotus_koodi, Koodistot.toiminnallinen_painotus_koodit.value, alkamis_pvm, paattymis_pvm, field_name
    )
...
def validate_tehtavanimike_koodi(tehtavanimike_koodi, alkamis_pvm=None, paattymis_pvm=None, field_name="tehtavanimike_koodi"):
...
def validate_lahdejarjestelma_koodi(lahdejarjestelma_koodi, alkamis_pvm=None, paattymis_pvm=None, field_name="lahdejarjestelma"):
    validate_z2_koodi(lahdejarjestelma_koodi, Koodistot.lahdejarjestelma_koodit.value, alkamis_pvm, paattymis_pvm, field_name)
...
def validate_kielipainotustyyppi_koodi(
    kielipainotustyyppi_koodi, alkamis_pvm=None, paattymis_pvm=None, field_name="kielipainotustyyppi_koodi"
):
    validate_z2_koodi(
        kielipainotustyyppi_koodi, Koodistot.kielipainotustyyppi_koodit.value, alkamis_pvm, paattymis_pvm, field_name
    )


def validate_tuentaso_koodi(tuentaso_koodi, alkamis_pvm=None, paattymis_pvm=None, field_name="tuentaso_koodi"):
    validate_z2_koodi(tuentaso_koodi, Koodistot.tuentaso_koodit.value, alkamis_pvm, paattymis_pvm, field_name)


def validate_ikaryhma_koodi(ikaryhma_koodi, alkamis_pvm=None, paattymis_pvm=None, field_name="ikaryhma_koodi"):
    validate_z2_koodi(ikaryhma_koodi, Koodistot.ikaryhma_koodit.value, alkamis_pvm, paattymis_pvm, field_name)
...
def validate_paattymispvm_same_or_after_alkamispvm(validated_data):
    ...
    if "paattymis_pvm" in validated_data and validated_data["paattymis_pvm"] is not None:
        if not validate_paivamaara1_before_paivamaara2(
            validated_data["alkamis_pvm"], validated_data["paattymis_pvm"], can_be_same=True
        ):
...
def validate_dates_within_toimipaikka(validated_data, toimipaikka_obj):
    ...
    if "alkamis_pvm" in validated_data:
        ...
        if not validate_paivamaara1_after_paivamaara2(
            validated_data["alkamis_pvm"], toimipaikka_obj.alkamis_pvm, can_be_same=True
        ):
            raise ValidationError({"alkamis_pvm": [ErrorMessages.TP011.value]})
    if "paattymis_pvm" in validated_data:
        if not validate_paivamaara1_before_paivamaara2(
            validated_data["paattymis_pvm"], toimipaikka_obj.paattymis_pvm, can_be_same=True
        ):
...
def validate_organisaatio_oid(oid, custom_raise_500=False):
    field_name = "organisaatio_oid"
    validate_oid(oid, field_name)
    oid_sections = oid.split(".")

    raise_error = False
    # Allow only '10' (section 5) in production
    if settings.PRODUCTION_ENV and oid_sections[4] != "10":
        raise_error = True
    elif oid_sections[4] not in ["10", "99", "199", "299"]:
        raise_error = True

    if raise_error and custom_raise_500:
        raise APIException({field_name: [ErrorMessages.MI010.value]})
    elif raise_error:
        raise ValidationError({field_name: [ErrorMessages.MI010.value]})


def validate_henkilo_oid(oid, custom_raise_500=False):
    field_name = "henkilo_oid"
    validate_oid(oid, field_name)
    oid_sections = oid.split(".")

    raise_error = False
    # Allow only '24' (section 5) in production
    if settings.PRODUCTION_ENV and oid_sections[4] != "24":
        raise_error = True
    elif oid_sections[4] not in ["24", "98", "198", "298"]:
        raise_error = True

    if raise_error and custom_raise_500:
        raise APIException({field_name: [ErrorMessages.MI011.value]})
    elif raise_error:
...
def validate_toimipaikan_nimi(toimipaikan_nimi):
    special_characters_partial = [
        ...
        ",",
        "(",
    ]  # Allow toimipaikan nimi to end with '.' and ')'
    ...
    if (
        toimipaikan_nimi.startswith(tuple(special_characters_full))
        or toimipaikan_nimi.endswith(tuple(special_characters_partial))
        or "  " in toimipaikan_nimi
    ):
...
def validate_taydennyskoulutus_suoritus_pvm(date):
    date_limit_after = datetime.date(2020, 9, 1)
    date_limit_before = datetime.date(2024, 12, 31)
    if not validate_paivamaara1_after_paivamaara2(date, date_limit_after, can_be_same=True):
        raise ValidationError([ErrorMessages.TK015.value])
    if not validate_paivamaara1_before_paivamaara2(date, date_limit_before, can_be_same=True):
        raise ValidationError([ErrorMessages.TK018.value])


def validate_palvelussuhde_alkamis_pvm(date):
    date_limit = datetime.date(1950, 1, 1)
    if not validate_paivamaara1_after_paivamaara2(date, date_limit, can_be_same=True):
        raise ValidationError([ErrorMessages.PS011.value])
...
def validate_instance_uniqueness(model, data, error, instance_id=None, ignore_fields=(), instance_id_m2m=None):
    """
    ...
    :param instance_id_m2m: Existing instance IDs to consider in the validation (for ManyToManyFields)
    """
    ignore_fields_list = (
        "id",
        "lahdejarjestelma",
    ...
    for model_field in model._meta.get_fields():
        ...
        elif model_field.name not in ignore_fields_list and isinstance(model_field, ManyToManyField) and instance_id_m2m:
            qs_filter &= Q(**{f"{model_field.name}__id": instance_id_m2m})

    if instance_id:
        model_objs = model.objects.filter(qs_filter).exclude(id=instance_id)
    else:
        model_objs = model.objects.filter(qs_filter)
...
def validate_datetime_string(datetime_str, field_name, datetime_format="%Y-%m-%d"):
    try:
        datetime_obj = datetime.datetime.strptime(datetime_str, datetime_format)
    except Exception:
        raise ValidationError({field_name: [ErrorMessages.GE026.value]})
