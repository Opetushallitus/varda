...
def mock_get_koodisto(koodisto_name):
    return {"versio": "1", "paivitysPvm": "2020-05-25"}
...
class KoodistopalveluTests(TestCase):
    ...
    def test_simple_update(self):
        ...
        koodisto_qs = Z2_Koodisto.objects.filter(name__in=Koodistot.list())
        code_qs = Z2_Code.objects.filter(koodisto__name__in=Koodistot.list())
        translation_qs = Z2_CodeTranslation.objects.filter(code__koodisto__name__in=Koodistot.list())

        code_id_qs = code_qs.values_list("id", flat=True)
        code_id_list_old = list(code_id_qs)

        koodistopalvelu.update_koodistot()

        self.assertEqual(koodisto_qs.count(), koodistot_count)
        self.assertEqual(code_qs.count(), koodistot_count * 2)
        self.assertEqual(translation_qs.count(), koodistot_count * 2 * 2)
    ...
    def test_empty_codes_result(self):
        code_id_qs = Z2_Code.objects.filter(koodisto__name__in=Koodistot.list()).values_list("id", flat=True)
        code_id_list_old = list(code_id_qs)
