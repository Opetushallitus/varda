...
from varda.models import Henkilo, Organisaatio, Z4_CasKayttoOikeudet
from varda.unit_tests.test_utils import assert_status_code, assert_validation_error, base64_encoding
...
class TestPalvelukayttajaKayttooikeus(TestCase):
    ...
    def test_palvelukayttaja_luovutuspalvelu(self):
        ...
        vipunen_api_response = client.get("/api/reporting/v1/vipunen/organisaatiot/", **headers)
        self.assertEqual(vipunen_api_response.status_code, status.HTTP_200_OK)
    ...
    def test_palvelukayttaja_henkilosto_only_permissions(self):
        ...
        organisaatiot = [
            {
                ...
                "kayttooikeudet": [
                    {
                        "palvelu": "VARDA",
                        "oikeus": Z4_CasKayttoOikeudet.HENKILOSTO_VUOKRATTU_TALLENTAJA,
                    },
                    {
                        "palvelu": "VARDA",
        ...
        expected_group_names = [
            ...
            "HENKILOSTO_VUOKRATTU_TALLENTAJA_1.2.246.562.10.27580498759",
        ...
        expected_integraatio = [
            TietosisaltoRyhma.VUOKRATTUHENKILOSTOTIEDOT.value,
    ...
    def test_multiple_palvelukayttaja_different_permissions(self):
        ...
        self.assertCountEqual(
            vakajarjestaja_qs.first().integraatio_organisaatio,
            [TietosisaltoRyhma.VAKATIEDOT.value, TietosisaltoRyhma.TYONTEKIJATIEDOT.value],
        )

    def test_report_viewer_permission_is_valid_for_service_user(self):
        permissions = [
            Z4_CasKayttoOikeudet.RAPORTTIEN_KATSELIJA,
            Z4_CasKayttoOikeudet.KATSELIJA,
        ]

        filtered_permissions = kayttooikeuspalvelu._exclude_non_valid_permissions(permissions)

        self.assertIn(Z4_CasKayttoOikeudet.RAPORTTIEN_KATSELIJA, filtered_permissions)
        self.assertNotIn(Z4_CasKayttoOikeudet.KATSELIJA, filtered_permissions)

    @responses.activate
    def test_palvelukayttaja_report_viewer_permission(self):
        username = "palvelukayttaja"
        organisaatio_oid = "1.2.246.562.10.27580498759"
        organisaatiot = [
            {
                ...
                "kayttooikeudet": [
                    {
                        "palvelu": "VARDA",
                        "oikeus": Z4_CasKayttoOikeudet.RAPORTTIEN_KATSELIJA,
                    },
                ],
            }
        ]
        ...
        expected_group_names = [f"{Z4_CasKayttoOikeudet.RAPORTTIEN_KATSELIJA}_{organisaatio_oid}"]
        self._assert_user_permissiongroups(expected_group_names, username)

        user = User.objects.get(username=username)
        self.assertTrue(
            Z4_CasKayttoOikeudet.objects.filter(
                user=user,
                kayttooikeus=Z4_CasKayttoOikeudet.RAPORTTIEN_KATSELIJA,
                organisaatio_oid=organisaatio_oid,
            ).exists()
        )

        vakajarjestaja = Organisaatio.objects.get(organisaatio_oid=organisaatio_oid)
        self.assertEqual(vakajarjestaja.integraatio_organisaatio, [])
    ...
    def test_service_user_cas_login(self):
        ...
        organisaatiot = [
            {
                ...
                "kayttooikeudet": [
                    ...
                    {"palvelu": "VARDA", "oikeus": Z4_CasKayttoOikeudet.HENKILOSTO_TYONTEKIJA_TALLENTAJA},
                    {"palvelu": "VARDA", "oikeus": Z4_CasKayttoOikeudet.PALVELUKAYTTAJA},
                ],
            }
        ]
        mock_cas_palvelukayttaja_responses(organisaatiot, username)
        ...
        expected_group_names = (
            f"{Z4_CasKayttoOikeudet.PALVELUKAYTTAJA}_{organisaatio_oid}",
    ...
    def test_service_user_login_clear_permissions(self):
        ...
        def _successful_login():
            ...
            self._assert_user_permissiongroups(expected_group_names, username)
            self.assertCountEqual(vakajarjestaja_qs.first().integraatio_organisaatio, [TietosisaltoRyhma.TYONTEKIJATIEDOT.value])
        ...
        expected_group_names = ["HENKILOSTO_VUOKRATTU_TALLENTAJA_1.2.246.562.10.27580498759"]
        ...
        self.assertCountEqual(
            vakajarjestaja_qs.first().integraatio_organisaatio,
            [TietosisaltoRyhma.TYONTEKIJATIEDOT.value, TietosisaltoRyhma.VUOKRATTUHENKILOSTOTIEDOT.value],
        )
...
def mock_cas_palvelukayttaja_responses(organisaatiot, username, henkilo_oid="1.2.246.562.24.10000000001"):
    ...
    responses.add(
        ...
        json=[
            {"oid": organisaatio["organisaatioOid"], "tyypit": ["organisaatiotyyppi_07"], "status": "AKTIIVINEN"}
