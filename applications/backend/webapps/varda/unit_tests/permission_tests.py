...
import varda.permissions as varda_permissions
...
from varda.permissions import RaportitPermissions
...
class RaportitPermissionsServiceUserTests(TestCase):
    organization_oid = "1.2.246.562.10.27580498759"

    def test_raportit_permissions_allows_virkailija_report_viewer(self):
        user = User.objects.create_user(username="virkailija-report-viewer")
        Z3_AdditionalCasUserFields.objects.create(user=user, kayttajatyyppi="VIRKAILIJA")
        group = Group.objects.create(name=f"{Z4_CasKayttoOikeudet.RAPORTTIEN_KATSELIJA}_{self.organization_oid}")
        user.groups.add(group)
        request = SimpleNamespace(user=user)

        result = RaportitPermissions().has_permission(request, view=None)

        self.assertTrue(result)

    def test_raportit_permissions_rejects_service_user_report_viewer(self):
        user = User.objects.create_user(username="service-report-viewer")
        Z3_AdditionalCasUserFields.objects.create(user=user, kayttajatyyppi="PALVELU")
        group = Group.objects.create(name=f"{Z4_CasKayttoOikeudet.RAPORTTIEN_KATSELIJA}_{self.organization_oid}")
        user.groups.add(group)
        request = SimpleNamespace(user=user)
...
class MassahakuPermissionsTests(TestCase):
    organization_oid = "1.2.246.562.10.27580498759"

    def _get_permission(self):
        permission_class = getattr(varda_permissions, "MassahakuPermissions", None)
        self.assertIsNotNone(permission_class)
        return permission_class()

    def test_massahaku_permissions_allows_superuser(self):
        user = User.objects.create_user(username="massahaku-superuser", is_superuser=True)
        request = SimpleNamespace(user=user)

        result = self._get_permission().has_permission(request, view=None)

        self.assertTrue(result)

    def test_massahaku_permissions_rejects_staff_user_without_oph_group(self):
        user = User.objects.create_user(username="massahaku-staff-only", is_staff=True, is_superuser=False)
        request = SimpleNamespace(user=user)
    ...
    def test_massahaku_permissions_allows_oph_administrator_group_user(self):
        user = User.objects.create_user(username="massahaku-oph-administrator", is_superuser=False)
        group, _ = Group.objects.get_or_create(name=get_oph_yllapitaja_group_name())
        user.groups.add(group)
        request = SimpleNamespace(user=user)
    ...
    def test_massahaku_permissions_allows_service_user_report_viewer(self):
        user = User.objects.create_user(username="massahaku-service-report-viewer")
        Z3_AdditionalCasUserFields.objects.create(user=user, kayttajatyyppi="PALVELU")
        group = Group.objects.create(name=f"{Z4_CasKayttoOikeudet.RAPORTTIEN_KATSELIJA}_{self.organization_oid}")
        user.groups.add(group)
        request = SimpleNamespace(user=user)
    ...
    def test_massahaku_permissions_rejects_virkailija_report_viewer(self):
        user = User.objects.create_user(username="massahaku-virkailija-report-viewer")
        Z3_AdditionalCasUserFields.objects.create(user=user, kayttajatyyppi="VIRKAILIJA")
        group = Group.objects.create(name=f"{Z4_CasKayttoOikeudet.RAPORTTIEN_KATSELIJA}_{self.organization_oid}")
        user.groups.add(group)
        request = SimpleNamespace(user=user)
    ...
    def test_massahaku_permissions_rejects_service_user_without_report_viewer_group(self):
        user = User.objects.create_user(username="massahaku-service-user")
        Z3_AdditionalCasUserFields.objects.create(user=user, kayttajatyyppi="PALVELU")
        request = SimpleNamespace(user=user)
    ...
    def test_massahaku_permissions_rejects_user_without_additional_fields(self):
        user = User.objects.create_user(username="massahaku-without-additional-fields")
        group = Group.objects.create(name=f"{Z4_CasKayttoOikeudet.RAPORTTIEN_KATSELIJA}_{self.organization_oid}")
        user.groups.add(group)
        request = SimpleNamespace(user=user)
    ...
    def test_massahaku_permissions_rejects_anonymous_user_without_orm_queries(self):
        request = SimpleNamespace(user=varda_permissions.AnonymousUser())

        with patch.object(
            Z3_AdditionalCasUserFields.objects,
            "filter",
            side_effect=AssertionError("AnonymousUser must not reach ORM filters"),
        ):
            result = self._get_permission().has_permission(request, view=None)
...
class VardaPermissionsTests(TestCase):
    ...
    def test_permissions_henkilo(self):
        ...
        henkilo = {"henkilotunnus": decrypt_henkilotunnus(henkilo_obj.henkilotunnus)}
    ...
    def test_get_lapset_by_parent_id_2(self):
        ...
        new_henkilo_url = json.loads(resp.content)["url"]
        lapsi = {"henkilo": new_henkilo_url, "vakatoimija": "/api/v1/vakajarjestajat/1/", "lahdejarjestelma": "1"}
    ...
    def test_create_non_unique_lapsi_within_vakajarjestaja(self):
        ...
        lapsi = {"henkilo": henkilo_url, "vakatoimija": vakatoimija_url, "lahdejarjestelma": "1"}
        resp = client.post("/api/v1/lapset/", lapsi)
    ...
    def test_request_log_collected_when_fetching_henkilo(self):
        data = {"henkilo_oid": "1.2.246.562.24.47279942650"}
        client = SetUpTestClient("tester").client()
        client.post("/api/v1/hae-henkilo/", data)
        request_log = Z6_RequestLog.objects.all()[0]
        self.assertEqual(request_log.user.username, "tester")
        self.assertEqual(request_log.request_url, "/api/v1/hae-henkilo/")
    ...
    def test_toimijatiedot_update(self):
        ...
        vakajarjestaja_patch = {"sahkopostiosoite": new_email}
    ...
    def test_henkilo_lapsi_permissions(self):
        ...
        henkilo = {"henkilotunnus": "280616A148T", "etunimet": "Testi Tero", "kutsumanimi": "Testi", "sukunimi": "Testinen"}
        ...
        self._assert_permissions_to_henkilo(
            henkilo_id,
            (vaka_vakajarjestaja_client,),
            (
                vaka_vakajarjestaja_client_2,
                vaka_toimipaikka_client,
                henkilosto_vakajarjestaja_client,
    ...
    def test_henkilo_tyontekija_permissions(self):
        ...
        vuokrattu_client = SetUpTestClient("vuokrattu_tallentaja").client()
        vaka_vakajarjestaja_client = SetUpTestClient("vakatietojen_tallentaja").client()
        vaka_toimipaikka_client = SetUpTestClient("vakatietojen_toimipaikka_tallentaja_9395737548815").client()
        ...
        self._assert_permissions_to_henkilo(
            ...
            (henkilosto_toimipaikka_client, vaka_vakajarjestaja_client, vaka_toimipaikka_client),
        )
        ...
        resp_palvelussuhde_post = henkilosto_vakajarjestaja_client.post("/api/henkilosto/v1/palvelussuhteet/", palvelussuhde)
        ...
        resp_tyoskentelypaikka_post = henkilosto_vakajarjestaja_client.post(
            "/api/henkilosto/v1/tyoskentelypaikat/", tyoskentelypaikka
        )
