...
from varda.cas.cas_components import OppijaCASBackend
...
from varda.models import Henkilo, Organisaatio, Z3_AdditionalCasUserFields, Z5_AuditLog
from varda.unit_tests.test_utils import SetUpTestClient, assert_status_code, assert_validation_error


class VardaOppijaViewsTests(TestCase):
    fixtures = ["fixture_basics"]
    TEST_HUOLTAJA_OID = "1.2.246.562.24.10000000999"
    TEST_HUOLTAJA_HETU = "010180-905H"

    def _oppija_cas_username(self, cas_attrs):
        return OppijaCASBackend().sanitize_username("oppija-cas-test-user", cas_attrs)

    def test_api_varhaiskasvatustiedot_get_data(self):
        lapsi_oid = "1.2.246.562.24.86012997950"
        henkilo = Henkilo.objects.get(henkilo_oid="1.2.246.562.24.86012997950")
        hetu = decrypt_henkilotunnus(henkilo.henkilotunnus)
        client_suomifi_tester = self._create_oppija_cas_user_and_assert_huoltaja_oid(self.TEST_HUOLTAJA_OID, lapsi_oid)
        ...
        henkilotiedot_response = {
            ...
            "aidinkieli_koodi": "FI",
            "sukupuoli_koodi": "1",
            "syntyma_pvm": henkilo.syntyma_pvm.strftime("%Y-%m-%d"),
            "kotikunta_koodi": "915",
        ...
        resp_henkilotiedot = client_suomifi_tester.get(
            f"/api/oppija/v1/henkilotiedot/{lapsi_oid}/", content_type="application/json"
        )
                            ...
                            "varhaiskasvatussuhteet": [
                                {
                                    ...
                                    "toimipaikka": {"toimipaikka_nimi": "Espoo", "toimipaikka_kunta_koodi": "091"},
                                    "yhteysosoite": "test1@espoo.fi",
                                },
                                {
                                    "alkamis_pvm": "2018-09-05",
                                    "paattymis_pvm": "2020-01-20",
                                    "toimipaikka": {"toimipaikka_nimi": "Espoo_2", "toimipaikka_kunta_koodi": "091"},
                                    "yhteysosoite": "organization@domain.com",
                                },
                            ],
                        }
                            ...
                            "varhaiskasvatussuhteet": [
                                {
                                    "alkamis_pvm": "2019-11-11",
                                    "paattymis_pvm": None,
                                    "toimipaikka": {"toimipaikka_nimi": "Espoo_3", "toimipaikka_kunta_koodi": "091"},
                                    "yhteysosoite": "organization@domain.com",
                                }
                            ],
                        }
                            ...
                            "varhaiskasvatussuhteet": [
                                {
                                    ...
                                    "toimipaikka": {"toimipaikka_nimi": "Tester2 toimipaikka", "toimipaikka_kunta_koodi": "091"},
        ...
        resp_varhaiskasvatustiedot = client_suomifi_tester.get(
            f"/api/oppija/v1/varhaiskasvatustiedot/{lapsi_oid}/", content_type="application/json"
        )
        ...
        resp_varhaiskasvatustiedot_json["lapset"] = sorted(
            resp_varhaiskasvatustiedot_json["lapset"],
            key=lambda key: (key["varhaiskasvatuksen_jarjestaja"], key["varhaiskasvatuspaatokset"][0]["alkamis_pvm"]),
        )
        for lapsi in resp_varhaiskasvatustiedot_json["lapset"]:
            ...
            for vakapaatos in lapsi["varhaiskasvatuspaatokset"]:
                del vakapaatos["id"]
                vakapaatos["varhaiskasvatussuhteet"] = sorted(
                    vakapaatos["varhaiskasvatussuhteet"], key=lambda key: key["alkamis_pvm"]
                )
        ...
        request_count = Z5_AuditLog.objects.filter(
            user__username=self._oppija_cas_username({"personOid": self.TEST_HUOLTAJA_OID})
        ).count()
        self.assertEqual(request_count, 2)

    def test_get_varhaiskasvatustiedot_data_no_permission(self):
        client = SetUpTestClient("tester2").client()
        resp_varhaiskasvatustiedot_api = client.get(
            "/api/oppija/v1/varhaiskasvatustiedot/1.2.246.562.24.86012997950/", content_type="application/json"
        )
    ...
    def test_api_varhaiskasvatustiedot_cant_access_wrong_lapsi(self):
        ...
        huoltaja_oid = self.TEST_HUOLTAJA_OID
        client_suomifi_tester = self._create_oppija_cas_user_and_assert_huoltaja_oid(huoltaja_oid, lapsi_oid)
        resp_varhaiskasvatustiedot_api = client_suomifi_tester.get(
            "/api/oppija/v1/varhaiskasvatustiedot/{}/".format(other_lapsi_oid), content_type="application/json"
        )
        assert_status_code(resp_varhaiskasvatustiedot_api, status.HTTP_403_FORBIDDEN)

    def test_api_varhaiskasvatustiedot_huollettavat_from_vtj(self):
        lapsi_oid = "1.2.246.562.24.86012997950"
        huoltaja_oid = self.TEST_HUOLTAJA_OID
        client_suomifi_tester = self._create_oppija_cas_user_and_assert_huoltaja_oid(huoltaja_oid, lapsi_oid)
        resp_varhaiskasvatustiedot_api = client_suomifi_tester.get(
            "/api/oppija/v1/varhaiskasvatustiedot/{}/".format(lapsi_oid), content_type="application/json"
        )
    ...
    def test_api_turvakielto(self):
        def _assert_all_resp_status(client, correct_status):
            url_list = [
                f"/api/oppija/v1/henkilotiedot/{lapsi_oid}/",
        ...
        client_huoltaja = self._create_oppija_cas_user_and_assert_huoltaja_oid(huoltaja_oid, lapsi_oid)
        _assert_all_resp_status(client_huoltaja, status.HTTP_200_OK)

        Henkilo.objects.filter(henkilo_oid=lapsi_oid).update(turvakielto=True)
        _assert_all_resp_status(client_huoltaja, status.HTTP_404_NOT_FOUND)
    ...
    def _mock_vtj_lapset(self, *lapsi_oids):
        service = MagicMock()
        service.get_henkilo.return_value = {
            "lapset": [
                {"hetu": decrypt_henkilotunnus(Henkilo.objects.get(henkilo_oid=lapsi_oid).henkilotunnus)}
                for lapsi_oid in lapsi_oids
            ]
        }
        return patch("varda.custom_auth.get_vtj_service", return_value=service)

    def _create_test_huoltaja_henkilo(self, henkilo_oid):
        return Henkilo.objects.get_or_create(
            henkilo_oid=henkilo_oid,
            defaults={
                "henkilotunnus": encrypt_string(self.TEST_HUOLTAJA_HETU),
                "henkilotunnus_unique_hash": hash_string(self.TEST_HUOLTAJA_HETU),
                "etunimet": "Testi",
                "kutsumanimi": "Testi",
                "sukunimi": "Huoltaja",
            },
    ...
    def _create_oppija_cas_user_and_assert_huoltaja_oid(self, huoltaja_oid, lapsi_oid):
        henkilo = self._create_test_huoltaja_henkilo(huoltaja_oid)
        hetu = decrypt_henkilotunnus(henkilo.henkilotunnus)
        cas_attrs = {"personOid": huoltaja_oid, "nationalIdentificationNumber": hetu}
        user_suomifi, _ = User.objects.get_or_create(
            username=self._oppija_cas_username(cas_attrs), is_staff=False, is_active=True
        )
        with self._mock_vtj_lapset(lapsi_oid):
            oppija_post_login_handler(user_suomifi, cas_attrs)
        ...
        self.assertIn(lapsi_oid, additional_cas_user_fields.huollettava_oid_list)
    ...
    def test_api_tyontekijatiedot(self):
        ...
        tyontekija_json = next(
            tyontekija for tyontekija in resp_tyontekijatiedot_json["tyontekijat"] if tyontekija["id"] == tyontekija_obj.id
        )
        self.assertEqual(
            len(tyontekija_json["tutkinnot"]),
        ...
        palvelussuhde_json = next(
            palvelussuhde for palvelussuhde in tyontekija_json["palvelussuhteet"] if palvelussuhde["id"] == palvelussuhde_obj.id
        )
        ...
        self.assertEqual(
            len(tyontekija_json["taydennyskoulutukset"]),
        ...
        taydennyskoulutus_json = next(
            taydennyskoulutus
            for taydennyskoulutus in tyontekija_json["taydennyskoulutukset"]
            if taydennyskoulutus["id"] == taydennyskoulutus_obj.id
        )
        self.assertEqual(
            len(taydennyskoulutus_json["tehtavanimike_koodi_list"]),
    ...
    def test_api_huoltajatiedot(self):
        ...
        huoltajuussuhde_json = next(
            huoltajuussuhde
            for huoltajuussuhde in resp_huoltajatiedot_json["huoltajuussuhteet"]
        ...
        maksutieto_json = next(
            maksutieto for maksutieto in huoltajuussuhde_json["maksutiedot"] if maksutieto["id"] == maksutieto_obj.id
        )
    ...
    def test_api_tyontekijatiedot_not_available_for_unrelated_huoltaja(self):
        henkilo_oid = "1.2.246.562.24.2431884920041"
        lapsi_oid = "1.2.246.562.24.86012997950"

        client = self._create_oppija_cas_user_and_assert_huoltaja_oid(self.TEST_HUOLTAJA_OID, lapsi_oid)
        resp_tyontekijatiedot = client.get(f"/api/oppija/v1/tyontekijatiedot/{henkilo_oid}/")
        assert_status_code(resp_tyontekijatiedot, status.HTTP_403_FORBIDDEN)
    ...
    def _create_oppija_cas_user_and_assert_henkilo_oid(self, henkilo_oid, assert_oid=True):
        ...
        if henkilo_oid:
            ...
            hetu = decrypt_henkilotunnus(henkilo.henkilotunnus)
        cas_attrs = {"personOid": henkilo_oid, "nationalIdentificationNumber": hetu}
        username = self._oppija_cas_username(cas_attrs) if henkilo_oid else "oppija-cas-test-user-no-oid"
        user_suomifi = User.objects.create(username=username, is_staff=False, is_active=True)
        with self._mock_vtj_lapset():
            oppija_post_login_handler(user_suomifi, cas_attrs)
