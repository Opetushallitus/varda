...
class VardaOppijaViewsTests(TestCase):
    ...
    def test_api_varhaiskasvatustiedot_get_data(self):
        ...
        # Mock Huoltaja login
        user_suomifi = User.objects.create(username=f"suomi.fi#010280-952L#{hetu}", is_staff=False, is_active=True)
        cas_attrs = {"impersonatorPersonOid": "[1.2.246.562.24.1234567890]", "personOid": "1.2.246.562.24.86012997950"}
        oppija_post_login_handler(user_suomifi, cas_attrs)

        client_suomifi_tester = SetUpTestClient(f"suomi.fi#010280-952L#{hetu}").client()
        ...
        resp_henkilotiedot = client_suomifi_tester.get(
            "/api/oppija/v1/henkilotiedot/1.2.246.562.24.86012997950/", content_type="application/json"
        )
        ...
        request_count = Z5_AuditLog.objects.filter(user__username=f"suomi.fi#010280-952L#{hetu}").count()
    ...
    def _create_oppija_cas_user_and_assert_huoltaja_oid(self, huoltaja_oid, lapsi_oid):
        henkilo = Henkilo.objects.get(henkilo_oid="1.2.246.562.24.47279942650")
        hetu = decrypt_henkilotunnus(henkilo.henkilotunnus)
        user_suomifi, _ = User.objects.get_or_create(username=f"suomi.fi#010280-952L#{hetu}", is_staff=False, is_active=True)
        impersonator_person_oid = None if huoltaja_oid is None else f"[{huoltaja_oid}]"
        cas_attrs = {"impersonatorPersonOid": impersonator_person_oid, "personOid": lapsi_oid}
        oppija_post_login_handler(user_suomifi, cas_attrs)
    ...
    def _create_oppija_cas_user_and_assert_henkilo_oid(self, henkilo_oid, assert_oid=True):
        ...
        cas_attrs = {"personOid": henkilo_oid}
        oppija_post_login_handler(user_suomifi, cas_attrs)
