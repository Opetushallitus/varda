...
class VardaViewsLuovutuspalveluTests(TestCase):
    ...
    def test_csr_post_no_data(self):
        ...
        self.assertEqual(json.loads(resp.content), {
            "csr":
                [{"error_code": "LU002", "description": "No file uploaded.", "translations": []}]
        })
        ...
        with open('manage.py', 'rb') as f:
            ...
            self.assertEqual(json.loads(resp.content), {
                "csr":
                    [{
                        "error_code": "LU003",
                        "description": "Uploaded file is not PEM encoded CSR.",
                        "translations": []
                    }]
            })
    ...
    def test_invalid_csr_encoding(self):
        ...
        ])).sign(private_key, hashes.SHA256()).public_bytes(encoding=Encoding.DER)
    ...
    def test_invalid_csr_private_key(self):
        ...
        csr = x509.CertificateSigningRequestBuilder().subject_name(x509.Name([
            ...
            x509.NameAttribute(NameOID.ORGANIZATION_NAME, 'My Organization'),
            x509.NameAttribute(NameOID.COMMON_NAME, 'example.com'),
        ])).sign(private_key, hashes.SHA256()).public_bytes(encoding=Encoding.PEM)
        ...
        self.assertEqual(json.loads(resp.content), {
            "csr":
                [{
                    "error_code": "LU004",
                    "description": "CSR public key is not of type RSAPublicKey.",
                    "translations": []
                }]
        })
    ...
    def test_invalid_csr_country(self):
        ...
        self.assertEqual(json.loads(resp.content), {
            "csr":
                [{
                    "error_code": "LU007",
                    "description": "CSR Subject C (Country) must be FI.",
                    "translations": []
                }]
        })
    ...
    def test_invalid_csr_cn_missing(self):
        ...
        self.assertEqual(json.loads(resp.content), {
            "csr":
                [{
                    "error_code": "LU008",
                    "description": "CSR Subject CN is missing.",
                    "translations": []
                }]
        })
    ...
    def test_invalid_change_private_key(self):
        ...
        client = SetUpTestClient('kela_luovutuspalvelu').client()
        resp = client.post('/api/client/v1/csr/', {
            'file_uploaded': io.BytesIO(csr),
            'change_private_key': 123
        })
        self.assertEqual(resp.status_code, 400)
        self.assertEqual(json.loads(resp.content), {
            "csr":
                [{
                    "error_code": "LU012",
                    "description": "Invalid value in change_private_key.",
                    "translations": []
                }]
        })
