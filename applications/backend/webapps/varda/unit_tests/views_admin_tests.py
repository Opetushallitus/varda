...
from varda.unit_tests.test_utils import SetUpTestClient, assert_status_code, assert_validation_error, mock_admin_user

REAL_CACHE_SETTINGS = {"default": {"BACKEND": "django.core.cache.backends.locmem.LocMemCache"}}
...
class VardaAdminViewSetTests(TestCase):
    ...
    def test_set_paattymis_pvm_not_admin(self):
        ...
        paattymis_pvm = {"vakajarjestaja": f"/api/v1/vakajarjestajat/{vakajarjestaja.id}/", "paattymis_pvm": "2022-02-01"}
    ...
    def test_set_paattymis_pvm_incorrect(self):
        ...
        paattymis_pvm = {"vakajarjestaja": f"/api/v1/vakajarjestajat/{vakajarjestaja.id}/", "paattymis_pvm": "2018-02-01"}
    ...
    def test_set_paattymis_pvm_not_set(self):
        ...
        self.assertEqual(
            Varhaiskasvatuspaatos.objects.get(tunniste="testing-varhaiskasvatuspaatos3").paattymis_pvm, paattymis_pvm_date
        )
        self.assertEqual(
            Varhaiskasvatuspaatos.objects.get(tunniste="testing-varhaiskasvatuspaatos6").paattymis_pvm,
            datetime.date(year=2019, month=10, day=24),
        )
        self.assertEqual(
            Varhaiskasvatuspaatos.objects.get(tunniste="testing-varhaiskasvatuspaatos_kela_tilapainen").paattymis_pvm,
            datetime.date(year=2021, month=10, day=2),
        )
        self.assertEqual(
            Varhaiskasvatussuhde.objects.get(tunniste="testing-varhaiskasvatussuhde3").paattymis_pvm, paattymis_pvm_date
        )
        self.assertEqual(
            Varhaiskasvatussuhde.objects.get(tunniste="testing-varhaiskasvatussuhde6").paattymis_pvm,
            datetime.date(year=2019, month=2, day=24),
        )
        self.assertEqual(
            Varhaiskasvatussuhde.objects.get(tunniste="kela_testing_public_tilapainen").paattymis_pvm,
            datetime.date(year=2021, month=10, day=2),
        )
        ...
        self.assertEqual(
            Tyoskentelypaikka.objects.get(tunniste="testing-tyoskentelypaikka1").paattymis_pvm,
            datetime.date(year=2020, month=9, day=10),
        )
        self.assertEqual(
            Tyoskentelypaikka.objects.get(tunniste="testing-tyoskentelypaikka1-1").paattymis_pvm,
            datetime.date(year=2020, month=10, day=2),
        )
        self.assertEqual(
            Tyoskentelypaikka.objects.get(tunniste="testing-tyoskentelypaikka2").paattymis_pvm,
            datetime.date(year=2021, month=5, day=2),
        )
        self.assertEqual(Tyoskentelypaikka.objects.get(tunniste="testing-tyoskentelypaikka2_1").paattymis_pvm, paattymis_pvm_date)
        self.assertEqual(
            Tyoskentelypaikka.objects.get(tunniste="testing-tyoskentelypaikka4").paattymis_pvm,
            datetime.date(year=2020, month=10, day=2),
        )
        self.assertEqual(
            Tyoskentelypaikka.objects.get(tunniste="testing-tyoskentelypaikka-kiertava").paattymis_pvm,
            datetime.date(year=2020, month=10, day=2),
        )
        # PAOS not set
        ...
        self.assertEqual(
            Varhaiskasvatussuhde.objects.get(tunniste="kela_testing_jm03").paattymis_pvm, datetime.date(year=2022, month=1, day=3)
        )
