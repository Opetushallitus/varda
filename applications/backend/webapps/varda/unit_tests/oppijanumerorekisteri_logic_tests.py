...
from varda.models import BatchError, Henkilo, Huoltaja, Lapsi
...
class TestOppijanumerorekisteriLogic(TestCase):
    ...
    def test_update_huoltajuussuhteet_uusi_huoltaja(self):
        responses.add(
            responses.GET,
            re.compile(
                "https://virkailija.testiopintopolku.fi/oppijanumerorekisteri-service/henkilo/huoltajasuhdemuutokset/alkaen/"
                + self.date_time_regex
            ),
        ...
        responses.add(
            responses.GET,
            "https://virkailija.testiopintopolku.fi/oppijanumerorekisteri-service/henkilo/1.2.246.562.24.6815981182311/huoltajat",
            json=[
                {
                    "etunimet": "Arpa",
                    "sukunimi": "Kuutio",
        ...
        responses.add(
            responses.GET,
            "https://virkailija.testiopintopolku.fi/oppijanumerorekisteri-service/henkilo/1.2.3.4.5/master",
            json={
                "etunimet": "Arpa",
                "sukunimi": "Kuutio",
                "kutsumanimi": "Arpa",
        ...
        actuals = list(
            lapsi.huoltajuussuhteet.all().values(
                "voimassa_kytkin", "huoltaja__henkilo__henkilo_oid", "lapsi__henkilo__henkilo_oid"
            )
        )
    ...
    def test_update_huoltajuussuhde_does_not_create_multiple_with_oid_and_hetu(self):
        ...
        responses.add(
            responses.GET,
            "https://virkailija.testiopintopolku.fi/oppijanumerorekisteri-service/henkilo/1.2.987654321/master",
            json={
                "etunimet": "Pauliina",
                "sukunimi": "Virtanen",
                "kutsumanimi": "Pauliina",
    ...
    def test_update_huoltajuussuhde_does_not_create_multiple_with_siblings_existing_huoltaja(self):
        ...
        responses.add(
            responses.GET,
            "https://virkailija.testiopintopolku.fi/oppijanumerorekisteri-service/henkilo/1.2.246.562.24.49084901393/huoltajat",
            json=[{"etunimet": "Pauliina", "sukunimi": "Virtanen", "kutsumanimi": "Pauliina", "oidHenkilo": "1.2.987654321"}],
            status=status.HTTP_200_OK,
        )
        responses.add(
    ...
    def test_update_henkilotieto_fails_and_batcherror_is_created(self):
        ...
        responses.add(
            responses.GET,
            re.compile(
                "https://virkailija.testiopintopolku.fi/oppijanumerorekisteri-service/s2s/changedSince/{}".format(
                    self.date_time_regex
                )
            ),
            json=[henkilo_oid],
        ...
        oppijanumerorekisteri.fetch_and_update_modified_henkilot()
        batch_error_after_retry_count = BatchError.objects.filter(
            type=BatchErrorType.HENKILOTIETO_UPDATE.name, henkilo=henkilo
        ).count()
    ...
    def test_batcherror_henkilo_duplicate(self):
        ...
        responses.add(
            responses.GET,
            re.compile(
                f"https://virkailija.testiopintopolku.fi/oppijanumerorekisteri-service/s2s/changedSince/{self.date_time_regex}"
            ),
            json=[henkilo_oid],
            status=status.HTTP_200_OK,
        )
