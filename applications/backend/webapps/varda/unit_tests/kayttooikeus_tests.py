...
class TestKayttooikeus(TestCase):
    ...
    def test_permissions_in_multiple_organizations(self):
        ...
        all_group_names = [f'{permission["oikeus"]}_{organisaatio["organisaatioOid"]}'
        ...
        self.assertListEqual(all_group_names,
                             list(user.additional_cas_user_fields.all_groups.values_list('name', flat=True)))
        # Actual groups should only be the default ones (ordered by ID -> 1.2.246.562.10.34683023489)
        self.assertListEqual([name for name in all_group_names if name.endswith('1.2.246.562.10.34683023489')],
        ...
        assert_status_code(
            client.post('/api/ui/active-organisaatio/', {'organisaatio_oid': '1.2.246.562.10.93957375488'}),
            status.HTTP_200_OK
        )
        self.assertListEqual([name for name in all_group_names if name.endswith('1.2.246.562.10.93957375488')],
                             list(user.groups.values_list('name', flat=True)))
