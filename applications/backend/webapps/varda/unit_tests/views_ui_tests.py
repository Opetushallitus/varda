...
from varda.models import Henkilo, Lapsi, Organisaatio, Palvelussuhde, Toimipaikka, Tyontekija
from varda.unit_tests.test_utils import SetUpTestClient, assert_status_code, post_henkilo_to_get_permissions
...
class VardaHenkilostoViewSetTests(TestCase):
    ...
    def test_tyontekija_list_toimipaikka_tyontekija_user(self):
        ...
        expected_henkilo_count = (
            Tyontekija.objects.filter(palvelussuhteet__tyoskentelypaikat__toimipaikka__organisaatio_oid=toimipaikka_oid)
            .distinct()
            .count()
        )
        vakajarjestaja_id = Organisaatio.objects.filter(organisaatio_oid=vakajarjestaja_oid).values_list("id", flat=True).first()
        ...
        group_tyontekija_tallentaja_9395737548815 = Group.objects.get(
    ...
    def test_tyontekija_list_filters(self):
        ...
        query_result_list = [
            ...
            ("search=daniella", result_qs.filter(etunimet__contains="Daniella")),
            (
                "search={}".format(hash_string("020400A928E")),
                result_qs.filter(henkilotunnus_unique_hash__iexact=hash_string("020400A928E")),
            ),
            ("search=1.2.246.562.24.2431884920044", result_qs.filter(henkilo_oid__iexact="1.2.246.562.24.2431884920044")),
            (
                "voimassa_pvm=2020-03-01",
                result_qs.filter(
                    Q(tyontekijat__palvelussuhteet__alkamis_pvm__lte="2020-03-01")
                    & (
    ...
    def test_lapsi_list_vakajarjestaja_huoltajatieto_user(self):
        ...
        vakajarjestaja_oid = "1.2.246.562.10.34683023489"
        vakajarjestaja_condition = (
            Q(lapsi__vakatoimija__organisaatio_oid=vakajarjestaja_oid)
            | Q(lapsi__oma_organisaatio__organisaatio_oid=vakajarjestaja_oid)
            | Q(lapsi__paos_organisaatio__organisaatio_oid=vakajarjestaja_oid)
        )
    ...
    def test_lapsi_list_toimipaikka_tyontekija_user(self):
        ...
        expected_henkilo_count = (
            Lapsi.objects.filter(varhaiskasvatuspaatokset__varhaiskasvatussuhteet__toimipaikka__organisaatio_oid=toimipaikka_oid)
            .distinct()
            .count()
        )
        vakajarjestaja_id = Organisaatio.objects.filter(organisaatio_oid=vakajarjestaja_oid).values_list("id", flat=True).first()
    ...
    def test_lapsi_list_filters(self):
        ...
        query_result_list = [
            ...
            (
                "toimipaikka_oid={}".format(toimipaikka_oid),
                result_qs.filter(
                    lapsi__varhaiskasvatuspaatokset__varhaiskasvatussuhteet__toimipaikka__organisaatio_oid=toimipaikka_oid
                ),
            ),
            (
                "toimipaikka_id={}".format(toimipaikka_id),
            ...
            ("search=aamu", result_qs.filter(etunimet__contains="Aamu")),
            (
                "search={}".format(hash_string("120699-985W")),
                result_qs.filter(henkilotunnus_unique_hash__iexact=hash_string("120699-985W")),
            ),
            ("search=1.2.246.562.24.6815981182311", result_qs.filter(henkilo_oid__iexact="1.2.246.562.24.6815981182311")),
            (
                "voimassa_pvm=2020-01-01",
                result_qs.filter(
                    Q(lapsi__varhaiskasvatuspaatokset__varhaiskasvatussuhteet__alkamis_pvm__lte="2020-01-01")
                    & (
    ...
    def test_api_get_toimipaikan_lapset_json(self):
        henkilo2 = Henkilo.objects.get(henkilo_oid="1.2.246.562.24.86012997950")
        toimipaikan_lapset_json = {
            ...
            "results": [
                ...
                },
                {
                    ...
                    "syntyma_pvm": henkilo2.syntyma_pvm.strftime("%Y-%m-%d"),
                    "oma_organisaatio_nimi": "Tester2 organisaatio",
                    "paos_organisaatio_nimi": "Tester organisaatio",
                    "lapsi_id": 8,
        ...
        self.assertEqual(resp.json(), toimipaikan_lapset_json)

    def test_api_toimipaikan_lapset_henkilo_nimi_filter(self):
        henkilo = Henkilo.objects.get(henkilo_oid="1.2.246.562.24.58672764848")
    ...
    def test_toimipaikan_lapset_huoltajatieto_tallentaja(self):
        ...
        henkilo1 = Henkilo.objects.get(henkilo_oid="1.2.246.562.24.86012997950")
        henkilo2 = Henkilo.objects.get(henkilo_oid="1.2.246.562.24.2395579779541")
            ...
            "results": [
                {
                    ...
                    "sukunimi": "Testilä",
                    "henkilo_oid": "1.2.246.562.24.86012997950",
                    "syntyma_pvm": henkilo1.syntyma_pvm.strftime("%Y-%m-%d"),
                    "lapsi_id": 6,
        ...
        self.assertEqual(resp.json(), accepted_response_json)

    def test_api_lapset_paos_toimipaikka_filter(self):
        # Tested lapsi (henkilo 16 1.2.246.562.24.86012997950) is paos-lapsi between
        # vakajarjestaja 1.2.246.562.10.34683023489 and
        ...
        toimipaikka_id = Toimipaikka.objects.get(organisaatio_oid=toimipaikka_oid).id
        resp_ui_list_lapset = client.get(
            f"/api/ui/vakajarjestajat/{vakajarjestaja_id}/lapsi-list/?toimipaikka_id={toimipaikka_id}"
        )
        assert_status_code(resp_ui_list_lapset, status.HTTP_200_OK)
        expected_henkilo_oid = "1.2.246.562.24.86012997950"
        henkilo_result = next(
            henkilo for henkilo in resp_ui_list_lapset.data["results"] if henkilo["henkilo_oid"] == expected_henkilo_oid
        )
    ...
    def test_ui_tyontekija_filter_tehtavanimike(self):
        ...
        resp_ui_2 = client.get(
            f"/api/ui/vakajarjestajat/1/tyontekijat/?tehtavanimike=39407&search={palvelussuhde.tyontekija.henkilo.etunimet}"
        )
    ...
    def test_ui_toimipaikat(self):
        ...
        actual_oid_set = {"1.2.246.562.10.9395737548815", "", "1.2.246.562.10.9395737548817", "1.2.246.562.10.9395737548812"}
