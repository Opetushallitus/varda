...
from varda.excel_export import ExcelReportType
from varda.misc import decrypt_excel_report_password, decrypt_henkilotunnus, encrypt_string
...
from varda.viewsets_reporting import MassahakuExcelReportViewSet
...
class MassahakuExcelReportCreateTests(TestCase):
    fixtures = ["fixture_basics"]

    def setUp(self):
        self.factory = APIRequestFactory()
        self.organization = Organisaatio.objects.first()
        self.user = User.objects.create_user(username="massahaku-service-user")
        Z3_AdditionalCasUserFields.objects.create(user=self.user, kayttajatyyppi="PALVELU")
        self.group_prefix = f"{Z4_CasKayttoOikeudet.RAPORTTIEN_KATSELIJA}_"
        self.group, _ = Group.objects.get_or_create(name=f"{self.group_prefix}{self.organization.organisaatio_oid}")
        self.user.groups.add(self.group)

    def _post(self, data):
        request = self.factory.post("/massahaku/", data, format="json")
        force_authenticate(request, user=self.user)
        return MassahakuExcelReportViewSet.as_view({"post": "create"})(request)

    def test_massahaku_post_creates_pending_report_and_dispatches_after_commit(self):
        with (
            mock.patch("varda.viewsets_reporting.make_random_password", return_value="DeterministicPass1"),
            mock.patch("varda.viewsets_reporting.create_mass_excel_report_task.delay") as delay_mock,
        ):
            with self.captureOnCommitCallbacks(execute=False) as callbacks:
                response = self._post(
                    {
                        "report_type": ExcelReportType.TYONTEKIJATIEDOT_VOIMASSA.value,
                        "target_date": "2026-08-10",
                        "language": SupportedLanguage.FI.value,
                    }
                )
            report = Z8_ExcelReport.objects.get()
            self.assertEqual(response.status_code, status.HTTP_202_ACCEPTED)
            self.assertEqual(report.user, self.user)
            self.assertEqual(report.organisaatio, self.organization)
            self.assertIsNone(report.toimipaikka)
            self.assertEqual(report.status, ReportStatus.PENDING.value)
            self.assertNotEqual(report.password, "DeterministicPass1")
            self.assertEqual(response.data["status"], "pending")
            self.assertEqual(
                set(response.data),
                {"id", "status", "report_type", "target_date", "toimipaikka_oid", "language", "password", "file_url"},
            )
            self.assertIsNone(response.data["password"])
            self.assertIsNone(response.data["file_url"])
            self.assertFalse(delay_mock.called)
            self.assertEqual(len(callbacks), 1)
            callbacks[0]()
            delay_mock.assert_called_once_with(report.id)

    def test_massahaku_post_is_request_logged(self):
        with (
            mock.patch("varda.viewsets_reporting.create_mass_excel_report_task.delay"),
            mock.patch("varda.viewsets_reporting.make_random_password", return_value="PlaintextPassword"),
        ):
            response = (
                SetUpTestClient(self.user.username)
                .client()
                .post(
                    "/api/reporting/v1/massahaut/",
                    {
                        "report_type": ExcelReportType.TYONTEKIJATIEDOT_VOIMASSA.value,
                        "target_date": "2026-08-10",
                        "language": SupportedLanguage.FI.value,
        ...
        self.assertEqual(response.status_code, status.HTTP_202_ACCEPTED)
        request_log = Z6_RequestLog.objects.get(request_url="/api/reporting/v1/massahaut/")
        self.assertEqual(request_log.request_method, "POST")
        self.assertEqual(request_log.response_code, status.HTTP_202_ACCEPTED)
        self.assertEqual(request_log.user, self.user)
        self.assertEqual(request_log.vakajarjestaja, self.organization)
        self.assertNotIn("PlaintextPassword", request_log.request_body)
        self.assertNotIn(Z8_ExcelReport.objects.get().password, request_log.request_body)

    def test_massahaku_post_accepts_toimipaikka_of_service_user_organization(self):
        toimipaikka = self.organization.toimipaikat.first()
        with mock.patch("varda.viewsets_reporting.create_mass_excel_report_task.delay"):
            response = self._post(
                {
                    "report_type": ExcelReportType.TYONTEKIJATIEDOT_VOIMASSA.value,
                    "target_date": "2026-08-10",
        ...
        self.assertEqual(response.status_code, status.HTTP_202_ACCEPTED)
        self.assertEqual(Z8_ExcelReport.objects.get().toimipaikka, toimipaikka)
        self.assertEqual(response.data["toimipaikka_oid"], toimipaikka.organisaatio_oid)

    def test_massahaku_post_rejects_toimipaikka_of_other_organization(self):
        other_organization = Organisaatio.objects.exclude(id=self.organization.id).first()
        toimipaikka = other_organization.toimipaikat.first()
        with mock.patch("varda.viewsets_reporting.create_mass_excel_report_task.delay") as delay_mock:
            response = self._post(
                {
                    "report_type": ExcelReportType.TYONTEKIJATIEDOT_VOIMASSA.value,
                    "target_date": "2026-08-10",
        ...
        self.assertFalse(Z8_ExcelReport.objects.exists())
        self.assertFalse(delay_mock.called)

    def test_massahaku_post_rejects_other_excel_report_type(self):
        with mock.patch("varda.viewsets_reporting.create_mass_excel_report_task.delay") as delay_mock:
            response = self._post({"report_type": ExcelReportType.VAKATIEDOT_VOIMASSA.value, "target_date": "2026-08-10"})
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertFalse(Z8_ExcelReport.objects.exists())
        self.assertFalse(delay_mock.called)

    def test_massahaku_post_requires_target_date(self):
        with mock.patch("varda.viewsets_reporting.create_mass_excel_report_task.delay") as delay_mock:
            response = self._post({"report_type": ExcelReportType.TYONTEKIJATIEDOT_VOIMASSA.value})
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertFalse(Z8_ExcelReport.objects.exists())
        self.assertFalse(delay_mock.called)

    def test_massahaku_post_rejects_ambiguous_service_user_organizations(self):
        second_organization = Organisaatio.objects.exclude(id=self.organization.id).first()
        self.user.groups.add(Group.objects.get_or_create(name=f"{self.group_prefix}{second_organization.organisaatio_oid}")[0])
        with mock.patch("varda.viewsets_reporting.create_mass_excel_report_task.delay") as delay_mock:
            response = self._post({"report_type": ExcelReportType.TYONTEKIJATIEDOT_VOIMASSA.value, "target_date": "2026-08-10"})
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
        self.assertFalse(Z8_ExcelReport.objects.exists())
        self.assertFalse(delay_mock.called)

    def test_massahaku_post_rejects_virkailija(self):
        self.user.additional_cas_user_fields.kayttajatyyppi = "VIRKAILIJA"
        self.user.additional_cas_user_fields.save()
        with mock.patch("varda.viewsets_reporting.create_mass_excel_report_task.delay") as delay_mock:
            response = self._post({"report_type": ExcelReportType.TYONTEKIJATIEDOT_VOIMASSA.value, "target_date": "2026-08-10"})
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
        self.assertFalse(Z8_ExcelReport.objects.exists())
    ...
    def test_massahaku_public_post_route_creates_report(self):
        client = SetUpTestClient(self.user.username).client()
        with (
            mock.patch("varda.viewsets_reporting.create_mass_excel_report_task.delay") as delay_mock,
            mock.patch("varda.viewsets_reporting.make_random_password", return_value="DeterministicPass1"),
        ):
    ...
    def test_massahaku_collection_lists_only_owned_reports_newest_first(self):
        older = self._create_massahaku_report(ReportStatus.PENDING.value)
        newer = self._create_massahaku_report(ReportStatus.FAILED.value)
        other_user = User.objects.create_user(username="massahaku-list-other-user")
        other_report = self._create_massahaku_report(ReportStatus.FINISHED.value, user=other_user)

        response = SetUpTestClient(self.user.username).client().get("/api/reporting/v1/massahaut/")

        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual([item["id"] for item in response.data["results"]], [newer.id, older.id])
        self.assertNotIn(other_report.id, [item["id"] for item in response.data["results"]])
        self.assertNotIn("password", response.data["results"][0])
        self.assertNotIn("file_url", response.data["results"][0])

    def test_massahaku_admin_post_requires_and_uses_organization(self):
        admin = User.objects.create_superuser(username="massahaku-admin", email="admin@example.com", password="pass")
        request = self.factory.post(
            "/massahaku/",
            {
                "report_type": ExcelReportType.TYONTEKIJATIEDOT_VOIMASSA.value,
                "target_date": "2026-08-10",
                "organisaatio_oid": self.organization.organisaatio_oid,
            },
            format="json",
        )
        ...
        with mock.patch("varda.viewsets_reporting.create_mass_excel_report_task.delay"):
            response = MassahakuExcelReportViewSet.as_view({"post": "create"})(request)

        self.assertEqual(response.status_code, status.HTTP_202_ACCEPTED)
        self.assertEqual(Z8_ExcelReport.objects.get().organisaatio, self.organization)

    def test_massahaku_admin_post_requires_organization(self):
        admin = User.objects.create_superuser(username="massahaku-admin-missing-org", email="admin@example.com", password="pass")
        request = self.factory.post(
            "/massahaku/",
            {"report_type": ExcelReportType.TYONTEKIJATIEDOT_VOIMASSA.value, "target_date": "2026-08-10"},
            format="json",
        )
        force_authenticate(request, user=admin)
    ...
    def test_massahaku_anonymous_requests_are_rejected_without_server_errors(self):
        paths_and_methods = (
            ("/api/reporting/v1/massahaut/", "get"),
            ("/api/reporting/v1/massahaut/", "post"),
            ("/api/reporting/v1/massahaut/999999/", "get"),
            ("/api/reporting/v1/massahaut/999999/file/", "get"),
        )
        for path, method in paths_and_methods:
            with self.subTest(path=path, method=method):
                ...
                self.assertNotIn(b"AnonymousUser", response.content)

    def test_massahaku_authenticated_unsupported_methods_are_rejected(self):
        report = self._create_massahaku_report(ReportStatus.PENDING.value)
        paths = (
            "/api/reporting/v1/massahaut/",
            f"/api/reporting/v1/massahaut/{report.id}/",
            f"/api/reporting/v1/massahaut/{report.id}/file/",
        )
    ...
    def test_massahaku_authenticated_missing_report_returns_not_found(self):
        self.client.force_login(self.user)
        for path in (
            "/api/reporting/v1/massahaut/999999/",
            "/api/reporting/v1/massahaut/999999/file/",
        ):
    ...
    def _create_massahaku_report(self, status_value, user=None, **kwargs):
        return Z8_ExcelReport.objects.create(
            password=encrypt_string("MassReportPass1!"),
            status=status_value,
            report_type=ExcelReportType.TYONTEKIJATIEDOT_VOIMASSA.value,
            target_date=datetime.date(2026, 8, 10),
            language=SupportedLanguage.FI.value,
            organisaatio=self.organization,
            user=user or self.user,
            **kwargs,
        )

    def test_massahaku_owner_can_retrieve_pending_report(self):
        report = self._create_massahaku_report(ReportStatus.PENDING.value)
        response = SetUpTestClient(self.user.username).client().get(f"/api/reporting/v1/massahaut/{report.id}/")
        ...
        self.assertEqual(response.data["status"], "pending")
        self.assertEqual(response.data["report_type"], report.report_type)
        self.assertEqual(response.data["target_date"], "2026-08-10")
        self.assertIsNone(response.data["toimipaikka_oid"])
        self.assertEqual(response.data["language"], report.language)
        self.assertIsNone(response.data["password"])
        self.assertIsNone(response.data["file_url"])
        for field in ("filename", "s3_object_path", "user"):
            self.assertNotIn(field, response.data)

    def test_massahaku_retrieve_maps_creating_to_running(self):
        report = self._create_massahaku_report(ReportStatus.CREATING.value)
        response = SetUpTestClient(self.user.username).client().get(f"/api/reporting/v1/massahaut/{report.id}/")
    ...
    def test_massahaku_retrieve_is_audit_logged(self):
        report = self._create_massahaku_report(ReportStatus.PENDING.value)
        path = f"/api/reporting/v1/massahaut/{report.id}/"

        response = SetUpTestClient(self.user.username).client().get(path)

        self.assertEqual(response.status_code, status.HTTP_200_OK)
        audit_logs = Z5_AuditLog.objects.filter(user=self.user, successful_get_request_path=path)
        self.assertEqual(audit_logs.count(), 1)

    def test_massahaku_retrieve_maps_finished_to_complete_without_download_data(self):
        report = self._create_massahaku_report(
            ReportStatus.FINISHED.value,
            filename="report.xlsx",
            s3_object_path="reports/report.xlsx",
        )
        response = SetUpTestClient(self.user.username).client().get(f"/api/reporting/v1/massahaut/{report.id}/")

        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data["status"], "complete")
        self.assertEqual(response.data["password"], "MassReportPass1!")
        self.assertEqual(response.data["file_url"], f"http://testserver/api/reporting/v1/massahaut/{report.id}/file/")
        for field in ("filename", "s3_object_path", "user"):
            self.assertNotIn(field, response.data)

    def test_massahaku_retrieve_maps_failed_to_failed(self):
        report = self._create_massahaku_report(ReportStatus.FAILED.value)
        response = SetUpTestClient(self.user.username).client().get(f"/api/reporting/v1/massahaut/{report.id}/")
    ...
    def test_massahaku_file_rejects_pending_report(self):
        report = self._create_massahaku_report(ReportStatus.PENDING.value)
        response = SetUpTestClient(self.user.username).client().get(f"/api/reporting/v1/massahaut/{report.id}/file/")
        self.assertEqual(response.status_code, status.HTTP_409_CONFLICT)
        self.assertEqual(response.data["detail"], "REPORT_NOT_READY")

    def test_massahaku_file_rejects_creating_report(self):
        report = self._create_massahaku_report(ReportStatus.CREATING.value)
        response = SetUpTestClient(self.user.username).client().get(f"/api/reporting/v1/massahaut/{report.id}/file/")
        self.assertEqual(response.status_code, status.HTTP_409_CONFLICT)
        self.assertEqual(response.data["detail"], "REPORT_NOT_READY")

    def test_massahaku_file_rejects_failed_report(self):
        report = self._create_massahaku_report(ReportStatus.FAILED.value)
        response = SetUpTestClient(self.user.username).client().get(f"/api/reporting/v1/massahaut/{report.id}/file/")
        self.assertEqual(response.status_code, status.HTTP_409_CONFLICT)
        self.assertEqual(response.data["detail"], "REPORT_NOT_READY")

    def test_massahaku_file_rejects_other_service_user(self):
        other_user = User.objects.create_user(username="massahaku-file-other-service-user")
        Z3_AdditionalCasUserFields.objects.create(user=other_user, kayttajatyyppi="PALVELU")
        other_user.groups.add(self.group)
        report = self._create_massahaku_report(ReportStatus.FINISHED.value, filename="report.xlsx", s3_object_path="reports/")
        with mock.patch("varda.viewsets_reporting.S3Client") as s3_client:
            response = SetUpTestClient(other_user.username).client().get(f"/api/reporting/v1/massahaut/{report.id}/file/")
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
        s3_client.assert_not_called()

    @override_settings(PRODUCTION_ENV=False, QA_ENV=False, DEV_ENV=True, S3_EXCEL_REPORTS="excel-reports")
    def test_massahaku_file_redirects_to_presigned_s3_url(self):
        report = self._create_massahaku_report(ReportStatus.FINISHED.value, filename="report.xlsx", s3_object_path="reports/")
        with mock.patch("varda.viewsets_reporting.S3Client") as client_class:
            client_class.return_value.create_presigned_url.return_value = "https://example.invalid/fresh-download"
            response = SetUpTestClient(self.user.username).client().get(f"/api/reporting/v1/massahaut/{report.id}/file/")
        self.assertEqual(response.status_code, status.HTTP_302_FOUND)
        self.assertEqual(response["Location"], "https://example.invalid/fresh-download")
        client_class.assert_called_once_with("dev-excel-reports")
        client_class.return_value.create_presigned_url.assert_called_once_with("reports/report.xlsx", expiration=3600)

    @override_settings(PRODUCTION_ENV=False, QA_ENV=False, DEV_ENV=True, S3_EXCEL_REPORTS="excel-reports")
    def test_massahaku_cloud_file_access_is_audit_logged(self):
        report = self._create_massahaku_report(ReportStatus.FINISHED.value, filename="report.xlsx", s3_object_path="reports/")
        path = f"/api/reporting/v1/massahaut/{report.id}/file/"
        with mock.patch("varda.viewsets_reporting.S3Client") as client_class:
            client_class.return_value.create_presigned_url.return_value = "https://example.invalid/fresh-download"
            response = SetUpTestClient(self.user.username).client().get(path)

        self.assertEqual(response.status_code, status.HTTP_302_FOUND)
        self.assertEqual(Z5_AuditLog.objects.filter(user=self.user, successful_get_request_path=path).count(), 1)

    @override_settings(PRODUCTION_ENV=False, QA_ENV=False, DEV_ENV=True, S3_EXCEL_REPORTS="excel-reports")
    def test_massahaku_file_generates_presigned_url_on_each_request(self):
        report = self._create_massahaku_report(ReportStatus.FINISHED.value, filename="report.xlsx", s3_object_path="reports/")
        with mock.patch("varda.viewsets_reporting.S3Client") as client_class:
            client_class.return_value.create_presigned_url.side_effect = [
                "https://example.invalid/first",
                "https://example.invalid/second",
            ]
            client = SetUpTestClient(self.user.username).client()
            first_response = client.get(f"/api/reporting/v1/massahaut/{report.id}/file/")
            second_response = client.get(f"/api/reporting/v1/massahaut/{report.id}/file/")
        self.assertEqual(first_response["Location"], "https://example.invalid/first")
        self.assertEqual(second_response["Location"], "https://example.invalid/second")
        self.assertEqual(client_class.return_value.create_presigned_url.call_count, 2)

    @override_settings(PRODUCTION_ENV=False, QA_ENV=False, DEV_ENV=True, S3_EXCEL_REPORTS="excel-reports")
    def test_massahaku_file_returns_503_when_presigned_url_fails(self):
        report = self._create_massahaku_report(ReportStatus.FINISHED.value, filename="report.xlsx", s3_object_path="reports/")
        with mock.patch("varda.viewsets_reporting.S3Client") as client_class:
            client_class.return_value.create_presigned_url.side_effect = RuntimeError("S3 unavailable")
            response = SetUpTestClient(self.user.username).client().get(f"/api/reporting/v1/massahaut/{report.id}/file/")
        self.assertEqual(response.status_code, status.HTTP_503_SERVICE_UNAVAILABLE)
        self.assertEqual(response.data["detail"], "REPORT_FILE_UNAVAILABLE")

    def test_massahaku_failed_file_delivery_is_not_audit_logged(self):
        report = self._create_massahaku_report(ReportStatus.PENDING.value)
        path = f"/api/reporting/v1/massahaut/{report.id}/file/"
        ...
        self.assertEqual(response.status_code, status.HTTP_409_CONFLICT)
        self.assertEqual(Z5_AuditLog.objects.filter(user=self.user, successful_get_request_path=path).count(), 0)

    @override_settings(PRODUCTION_ENV=False, QA_ENV=False, DEV_ENV=False, S3_EXCEL_REPORTS="excel-reports")
    def test_massahaku_file_returns_503_when_presigned_url_is_empty(self):
        report = self._create_massahaku_report(ReportStatus.FINISHED.value, filename="report.xlsx", s3_object_path="reports/")
        with mock.patch("varda.viewsets_reporting.S3Client") as client_class:
            client_class.return_value.create_presigned_url.return_value = ""
            response = SetUpTestClient(self.user.username).client().get(f"/api/reporting/v1/massahaut/{report.id}/file/")
        self.assertEqual(response.status_code, status.HTTP_503_SERVICE_UNAVAILABLE)
        self.assertEqual(response.data["detail"], "REPORT_FILE_UNAVAILABLE")

    def test_massahaku_other_service_user_cannot_retrieve_report(self):
        other_user = User.objects.create_user(username="massahaku-other-service-user")
        Z3_AdditionalCasUserFields.objects.create(user=other_user, kayttajatyyppi="PALVELU")
        other_user.groups.add(self.group)
        report = self._create_massahaku_report(ReportStatus.PENDING.value)

        response = SetUpTestClient(other_user.username).client().get(f"/api/reporting/v1/massahaut/{report.id}/")

        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)

    def test_massahaku_retrieve_uses_default_database(self):
        request = self.factory.get("/api/reporting/v1/massahaut/1/")
        force_authenticate(request, user=self.user)
        request.user = self.user
        view = MassahakuExcelReportViewSet()
        view.request = request
        queryset = mock.Mock()
        queryset.filter.return_value = queryset
        with mock.patch("varda.viewsets_reporting.Z8_ExcelReport.objects.using", return_value=queryset) as using_mock:
            self.assertIs(view.get_queryset(), queryset)
        using_mock.assert_called_once_with(DEFAULT_DB_ALIAS)


class ExcelReportCreationTests(TestCase):
    fixtures = ["fixture_basics"]

    def test_excel_report_post_creates_pending_report_and_dispatches_after_commit(self):
        client = SetUpTestClient("tester2").client()
        user = User.objects.get(username="tester2")
        report_viewer_prefix = f"{Z4_CasKayttoOikeudet.RAPORTTIEN_KATSELIJA}_"
        report_viewer_group = user.groups.get(name__startswith=report_viewer_prefix)
        organisaatio_oid = report_viewer_group.name.removeprefix(report_viewer_prefix)
        organisaatio = Organisaatio.objects.get(organisaatio_oid=organisaatio_oid)
        plaintext_password = "MassReportPass1!"
        existing_report_ids = set(Z8_ExcelReport.objects.values_list("id", flat=True))
        request_data = {
            "report_type": ExcelReportType.TYONTEKIJATIEDOT_VOIMASSA.value,
            "target_date": "2026-08-10",
            "language": SupportedLanguage.FI.value,
        ...
        with (
            mock.patch("varda.viewsets_reporting.make_random_password", return_value=plaintext_password),
            mock.patch("varda.viewsets_reporting.create_excel_report_task.delay") as delay_mock,
        ):
            with self.captureOnCommitCallbacks(execute=False) as callbacks:
                response = client.post("/api/reporting/v1/excel-reports/", request_data, format="json")
            ...
            created_reports = Z8_ExcelReport.objects.exclude(id__in=existing_report_ids)
            self.assertEqual(created_reports.count(), 1)
            report = created_reports.get()
            self.assertEqual(report.user, user)
            self.assertEqual(report.organisaatio, organisaatio)
            self.assertEqual(report.report_type, ExcelReportType.TYONTEKIJATIEDOT_VOIMASSA.value)
            self.assertEqual(report.target_date, datetime.date(2026, 8, 10))
            self.assertEqual(report.language, SupportedLanguage.FI.value)
            self.assertEqual(report.status, ReportStatus.PENDING.value)
            self.assertIsNone(report.filename)
            self.assertIsNone(report.s3_object_path)
            self.assertNotEqual(report.password, plaintext_password)
            self.assertTrue(report.password)
            self.assertEqual(decrypt_excel_report_password(report.password, report.id), plaintext_password)

            callbacks[0]()

            delay_mock.assert_called_once_with(report.id)
            response_data = response.json()
            self.assertEqual(response_data["id"], report.id)
            self.assertEqual(response_data["report_type"], ExcelReportType.TYONTEKIJATIEDOT_VOIMASSA.value)
            self.assertEqual(response_data["target_date"], "2026-08-10")
            self.assertEqual(response_data["language"], SupportedLanguage.FI.value)
            self.assertEqual(response_data["status"], ReportStatus.PENDING.value)
            self.assertEqual(response_data["organisaatio_oid"], organisaatio_oid)
            self.assertIsNone(response_data["url"])
            self.assertEqual(response_data["password"], plaintext_password)

    def test_excel_report_owner_can_retrieve_pending_report(self):
        owner = User.objects.get(username="tester2")
        report_viewer_prefix = f"{Z4_CasKayttoOikeudet.RAPORTTIEN_KATSELIJA}_"
        report_viewer_group = owner.groups.get(name__startswith=report_viewer_prefix)
        organisaatio_oid = report_viewer_group.name.removeprefix(report_viewer_prefix)
        organisaatio = Organisaatio.objects.get(organisaatio_oid=organisaatio_oid)
        plaintext_password = "MassReportPass1!"
        target_date = datetime.date(2026, 8, 10)
        report = Z8_ExcelReport.objects.create(
            password=encrypt_string(plaintext_password),
        ...
        response = SetUpTestClient("tester2").client().get(f"/api/reporting/v1/excel-reports/{report.id}/")

        assert_status_code(response, status.HTTP_200_OK)
        response_data = response.json()
        self.assertEqual(response_data["id"], report.id)
        self.assertEqual(response_data["report_type"], ExcelReportType.TYONTEKIJATIEDOT_VOIMASSA.value)
        self.assertEqual(response_data["target_date"], target_date.isoformat())
        self.assertEqual(response_data["language"], SupportedLanguage.FI.value)
        self.assertEqual(response_data["status"], ReportStatus.PENDING.value)
        self.assertEqual(response_data["organisaatio_oid"], organisaatio_oid)
        self.assertEqual(response_data["password"], plaintext_password)
    ...
    def test_excel_report_other_user_cannot_retrieve_report(self):
        owner = User.objects.get(username="tester2")
        report_viewer_prefix = f"{Z4_CasKayttoOikeudet.RAPORTTIEN_KATSELIJA}_"
        report_viewer_group = owner.groups.get(name__startswith=report_viewer_prefix)
        organisaatio_oid = report_viewer_group.name.removeprefix(report_viewer_prefix)
        organisaatio = Organisaatio.objects.get(organisaatio_oid=organisaatio_oid)
        other_user = User.objects.get(username="tester3")
        other_user.groups.add(report_viewer_group)
        report = Z8_ExcelReport.objects.create(
            password=encrypt_string("MassReportPass1!"),
            status=ReportStatus.PENDING.value,
            report_type=ExcelReportType.TYONTEKIJATIEDOT_VOIMASSA.value,
        ...
        response = SetUpTestClient(other_user.username).client().get(f"/api/reporting/v1/excel-reports/{report.id}/")
    ...
    def test_reporting_api(self):
        ...
        self.assertEqual(
            json.loads(resp.content),
            {
                ...
                "massahaut": "http://testserver/api/reporting/v1/massahaut/",
