...
from varda.models import (
    Lapsi, Huoltaja, Maksutieto, Varhaiskasvatussuhde, Varhaiskasvatuspaatos, Henkilo, Organisaatio, Huoltajuussuhde,
    Palvelussuhde, Tutkinto, Z5_AuditLog, Tukipaatos)
from varda.permission_groups import get_oph_yllapitaja_group_name
from varda.tasks import (
    change_lapsi_henkilo_task, delete_huoltajat_without_relations_task, delete_henkilot_without_relations_task,
    general_monitoring_task, reset_superuser_permissions_task, remove_inactive_tutkintos,
    add_missing_tukipaatos_paatosmaaras, remove_tutkintos_003_if_multiple_tutkinto_koodi)
...
class TaskTests(TestCase):
    ...
    def test_delete_henkilot_task(self):
        ...
        henkilo_qs.update(
            luonti_pvm=timezone.now() - datetime.timedelta(days=settings.DELETE_HENKILO_WITHOUT_ROLE_IN_DAYS + 1)
        )
    ...
    def test_remove_inactive_tutkintos(self):
        ...
        self.assertTrue(Palvelussuhde.objects.filter(
            tyontekija__henkilo__henkilotunnus_unique_hash=hetu_hash, tutkinto_koodi=tutkintokoodi_is_found).exists())
        self.assertFalse(Palvelussuhde.objects.filter(
            tyontekija__henkilo__henkilotunnus_unique_hash=hetu_hash, tutkinto_koodi=tutkintokoodi_not_found).exists())

        # check Tutkinto objects are found before removal
        self.assertTrue(Tutkinto.objects.filter(
            henkilo__henkilotunnus_unique_hash=hetu_hash, tutkinto_koodi=tutkintokoodi_is_found).exists())
        self.assertTrue(Tutkinto.objects.filter(
            henkilo__henkilotunnus_unique_hash=hetu_hash, tutkinto_koodi=tutkintokoodi_not_found).exists())
        ...
        Tutkinto.objects.filter(
            henkilo__henkilotunnus_unique_hash=hetu_hash).update(luonti_pvm=datetime_91d_ago)
    ...
    def test_add_missing_tukipaatos_paatosmaaras_add_task(self):
        ...
        Tukipaatos.objects.create(
            ...
            lahdejarjestelma='1')
        self.assertEqual(Tukipaatos.objects.filter(
            vakajarjestaja=vakajarjestaja,
            tilastointi_pvm=datetime.date(2024, 5, 31)).count(), 1)

        # Check added Tukipaatoses after using task-function
        add_missing_tukipaatos_paatosmaaras()
        self.assertEqual(Tukipaatos.objects.filter(
            vakajarjestaja=vakajarjestaja,
            tilastointi_pvm=datetime.date(2024, 5, 31)).count(), 30)
    ...
    def test_add_missing_tukipaatos_paatosmaaras_no_add_task(self):
        ...
        self.assertEqual(Tukipaatos.objects.filter(
            vakajarjestaja=vakajarjestaja,
            tilastointi_pvm=datetime.date(2024, 6, 1)).count(), 30)
