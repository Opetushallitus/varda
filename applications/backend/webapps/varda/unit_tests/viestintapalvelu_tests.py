...
from varda.models import (Aikaleima, Henkilo, Maksutieto, Organisaatio, Toimipaikka, Tyoskentelypaikka, Z11_MessageLog,
                          Z11_MessageTarget, Z6_LastRequest, Z6_RequestLog, Lapsi)
from varda.tasks import update_last_request_table_task
from varda.unit_tests.test_utils import (assert_status_code, mock_admin_user, mock_date_decorator_factory,
                                         SetUpTestClient, RollbackTestCase)
...
class ViestintapalveluTests(RollbackTestCase):
    ...
    def test_update_message_targets_and_paakayttaja_status(self):
        ...
        responses.add(
            method=responses.GET, status=status.HTTP_200_OK, json=kayttaja_response,
        ...
        responses.add(
            method=responses.POST, status=status.HTTP_200_OK, json=yhteystiedot_response,
        ...
        kayttaja_response[0]['organisaatiot'][1]['kayttooikeudet'].append(
            {'palvelu': 'VARDA', 'oikeus': 'VARDA-PAAKAYTTAJA'}
        )
        responses.replace(
            method_or_response=responses.GET, status=status.HTTP_200_OK, json=kayttaja_response,
            url='https://virkailija.testiopintopolku.fi/kayttooikeus-service/kayttooikeus/kayttaja?palvelu=VARDA&offset=0'
        )
    ...
    def test_no_paakayttaja_message(self):
        ...
        message_count = 0
        message_qs = (Z11_MessageLog.objects
                      .filter(message_type=MessageType.NO_PAAKAYTTAJA.value, organisaatio=organisaatio)
                      .order_by('-timestamp'))
    ...
    def test_puutteelliset_tiedot_message(self):
        ...
        for organisaatio in Organisaatio.vakajarjestajat.all():
            ...
            if (json.loads(resp_lapset.content)['results'] or json.loads(resp_tyontekijat.content)['results'] or
                    json.loads(resp_toimipaikat.content)['results']):
        ...
        if organisaatio_puutteelliset_list:
            ...
            Z11_MessageTarget.objects.create(organisaatio=organisaatio, email='test1@email.com',
                                             user_type=Kayttajatyyppi.PAAKAYTTAJA.value)
            Z11_MessageTarget.objects.create(organisaatio=organisaatio, email='test2@email.com',
                                             user_type=Kayttajatyyppi.PAAKAYTTAJA.value)
            Z11_MessageTarget.objects.create(organisaatio=organisaatio, email='test3@email.com',
                                             user_type=Kayttajatyyppi.PAAKAYTTAJA.value)
        ...
        for organisaatio in organisaatio_puutteelliset_list:
            ...
            target_email_set = (set(organisaatio.message_targets.values_list('email', flat=True)) or
                                {settings.OPH_EMAIL})
            message_email_set = set(Z11_MessageLog.objects
                                    .filter(organisaatio=organisaatio)
    ...
    def test_puutteelliset_tiedot_message_permissions(self):
        ...
        email = 'test@example.com'
        Z11_MessageTarget.objects.create(organisaatio=organisaatio, email=email,
                                         user_type=Kayttajatyyppi.PAAKAYTTAJA.value)

        message_count = 0
        message_qs = Z11_MessageLog.objects.filter(organisaatio=organisaatio,
    ...
    def test_no_transfers_message(self):
        ...
        for yritysmuoto, error_limit in (('41', 30,), ('16', 180,),):
            ...
            assert_status_code(client.patch(f'/api/v1/lapset/{lapsi.id}/', {'tunniste': '%&#?!'}),
            ...
            assert_status_code(client.patch(f'/api/henkilosto/v1/tyoskentelypaikat/{tyoskentelypaikka.id}/',
                                            {'paattymis_pvm': 'testitunniste'}),
                               status.HTTP_400_BAD_REQUEST)
            update_last_request_table_task(init=True)
            message_qs.update(timestamp=old_date)
            send_no_transfers_message()
            ...
            Z6_LastRequest.objects.all().delete()
            assert_status_code(client.patch(f'/api/v1/lapset/{lapsi.id}/', {'tunniste': 'testitunniste'}),
                               status.HTTP_200_OK)
            update_last_request_table_task(init=True)
            message_qs.update(timestamp=old_date)
            send_no_transfers_message()
