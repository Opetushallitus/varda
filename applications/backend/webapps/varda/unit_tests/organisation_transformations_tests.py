...
class OrganisationTransformationsTests(TestCase):
    ...
    def test_toimipaikat_transfer_simple(self):
        ...
        self._assert_organization_has_permissions(
            old_vakajarjestaja_oid, permission_id_list_nested_before_old, has_permissions=True
        )
        self._assert_organization_has_permissions(
            new_vakajarjestaja_oid, permission_id_list_nested_before_old, has_permissions=False
        )
        ...
        self._assert_organization_has_permissions(
            old_vakajarjestaja_oid, permission_id_list_nested_before_old, has_permissions=False
        )
        self._assert_organization_has_permissions(
            new_vakajarjestaja_oid, permission_id_list_nested_before_old, has_permissions=True
        )
        ...
        for index in range(0, len(permission_id_list_nested_before_old)):
            self.assertEqual(
                len(permission_id_list_nested_after_new[index]),
    ...
    def test_transfer_toimipaikka_has_paos_lapsi(self):
        ...
        self.assertCountEqual((paos_lapsi_id,), object_ids_organization_has_permissions_to(oma_vakajarjestaja_oid, Lapsi))
    ...
    def test_transfer_new_vakajarjestaja_has_existing_tyontekija(self):
        ...
        old_tutkinto_id = Tutkinto.objects.get(
            henkilo=tyontekija_henkilo, vakajarjestaja=old_vakajarjestaja, tutkinto_koodi=tutkinto_code
        ).id
        ...
        self.assertEqual(1, Tyontekija.objects.filter(henkilo=tyontekija_henkilo, vakajarjestaja=new_vakajarjestaja).count())
        ...
        self.assertEqual(len(set(taydennyskoulutus.taydennyskoulutukset_tyontekijat.values_list("tyontekija", flat=True))), 1)
        self.assertEqual(taydennyskoulutus.tyontekijat.all()[0].id, tyontekija_qs.first().id)

    def test_transfer_new_vakajarjestaja_has_existing_vuokrattu_henkilosto(self):
        old_vakajarjestaja_oid = "1.2.246.562.10.57294396385"
        new_vakajarjestaja_oid = "1.2.246.562.10.52966755795"
        ...
        old_vuokrattu_henkilosto = VuokrattuHenkilosto.objects.get(tunniste="testing-vuokrattuhenkilosto2")
        old_vuokrattu_henkilosto.kuukausi = datetime.date(year=2020, month=9, day=1)
        old_vuokrattu_henkilosto.save()
        existing_vuokrattu_henkilosto = VuokrattuHenkilosto.objects.get(tunniste="testing-vuokrattuhenkilosto3")
        existing_vuokrattu_henkilosto.kuukausi = datetime.date(year=2020, month=9, day=2)
        existing_vuokrattu_henkilosto.save()

        transfer_toimipaikat_to_vakajarjestaja(new_vakajarjestaja, old_vakajarjestaja)

        # Old VuokrattuHenkilosto object has been deleted
        self.assertFalse(VuokrattuHenkilosto.objects.filter(id=old_vuokrattu_henkilosto.id).exists())

        # Information has been united
        vuokrattu_henkilosto = VuokrattuHenkilosto.objects.get(id=existing_vuokrattu_henkilosto.id)
        self.assertEqual(
            vuokrattu_henkilosto.tuntimaara, old_vuokrattu_henkilosto.tuntimaara + existing_vuokrattu_henkilosto.tuntimaara
        )
        self.assertEqual(
            vuokrattu_henkilosto.tyontekijamaara,
            old_vuokrattu_henkilosto.tyontekijamaara + existing_vuokrattu_henkilosto.tyontekijamaara,
        )
    ...
    def _get_id_list_nested_for_vakajarjestaja(self, vakajarjestaja):
        ...
        henkilo_qs = Henkilo.objects.filter(
            Q(lapsi__vakatoimija=vakajarjestaja) | Q(tyontekijat__vakajarjestaja=vakajarjestaja)
        ).distinct()
        ...
        vuokrattu_henkilosto_qs = VuokrattuHenkilosto.objects.filter(vakajarjestaja=vakajarjestaja)
        permission_id_list_nested.append(list(vuokrattu_henkilosto_qs.values_list("id", flat=True)))
    ...
    def _assert_organization_has_permissions(self, organisaatio_oid, permission_id_list_nested, has_permissions=True):
        ...
        existing_permission_id_list_nested = [
            object_ids_organization_has_permissions_to(organisaatio_oid, model) for model in model_list
        ]
