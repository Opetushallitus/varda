...
from varda.models import (Organisaatio, Toimipaikka, PaosOikeus, Huoltaja, Huoltajuussuhde, Henkilo,
                          ...
                          KieliPainotus, PaosToiminta, Z12_DataAccessLog, Z2_Code, MaksutietoHuoltajuussuhde,
                          Z4_CasKayttoOikeudet, TukipaatosAikavali, Tukipaatos)
...
class VardaViewsTests(TestCase):
    ...
    def test_api_get_user_data_authenticated(self):
        ...
        result_json = {'toimintakieli_koodi': 'sv',
                       'email': '',
    ...
    def test_api_external_permissions_no_virkailija(self):
        ...
        resp = client.post('/api/onr/external-permissions/', post_data,
                           **{'HTTP_X_PROXIED_FROM': settings.VARDA_FRONTEND_SECRET})
        assert_status_code(resp, status.HTTP_200_OK)
        self.assertEqual(json.loads(resp.content), expected_response)
    ...
    def test_api_get_vakajarjestaja_yhteenveto(self):
        ...
        TukipaatosAikavali.objects.create(alkamis_pvm='1900-01-01', paattymis_pvm='1900-12-31',
                                          tilastointi_pvm='1900-12-31')
    ...
    def test_lapsi_kooste_oma(self):
        ...
        self.assertEqual(len(result['varhaiskasvatussuhteet']),
        ...
        self.assertEqual(len(result['maksutiedot']),
                         lapsi.huoltajuussuhteet.values_list('maksutiedot__id').distinct().count())
    ...
    def test_put_toimipaikat_v3_organisaatio_oid_disallowed(self):
        ...
        resp = client.put(f'/api/v3/toimipaikat/{lahdejarjestelma}:{tunniste}/',
    ...
    def test_post_tukipaatos_outside_timeframe(self):
        ...
        TukipaatosAikavali.objects.create(alkamis_pvm='1900-11-01', paattymis_pvm='1900-12-31',
                                          tilastointi_pvm='1900-12-31')
    ...
    @mock_date_decorator_factory('varda.serializers.datetime', '2022-10-31')
    def tukipaatos_update_expired(self, client, resp):
        ...
        resp = client.put(f'/api/v1/tukipaatokset/{tukipaatos_id}/', tukipaatos_put)
        assert_status_code(resp, status.HTTP_400_BAD_REQUEST)
        validation_error_msg = 'Tukipaatos cannot be modified after the timeframe of the reporting date.'
        assert_validation_error(resp, 'errors', 'TT003', validation_error_msg)
    ...
    def test_tukipaatos_update_within_timeframe_after_statistical_date(self):
        ...
        # Timeframe which is active over year change.
        TukipaatosAikavali.objects.create(alkamis_pvm='1900-12-01', paattymis_pvm='1900-01-31',
                                          tilastointi_pvm='1900-12-31')
        tukipaatos = {
            'vakajarjestaja': '/api/v1/vakajarjestajat/1/',
            'paatosmaara': '15',
    ...
    def test_get_tukipaatos_specific_timeframe_success(self):
        ...
        # Set old timeframe tukipaatokset
        self.set_old_timeframe_tukipaatokset(client)

        # Should have 10 total tukipaatokset now
        resp = client.get('/api/v1/tukipaatokset/')
        assert_status_code(resp, status.HTTP_200_OK)
        self.assertEqual(len(json.loads(resp.content)['results']), 10)

        # We should get only one tukipaatos with the given parameters
        resp = client.get('/api/v1/tukipaatokset/?yksityinen_jarjestaja=false&ikaryhma_koodi=IR01&tuentaso_koodi=TT01&tilastointi_pvm=2023-12-31')
        assert_status_code(resp, status.HTTP_200_OK)
    ...
    @mock_date_decorator_factory('varda.serializers.datetime', '2020-10-31')
    def set_old_timeframe_tukipaatokset(self, client):
        for i in range(1, 6):
            tukipaatos = {
                'vakajarjestaja': '/api/v1/vakajarjestajat/1/',
                'paatosmaara': '15',
    ...
    def test_get_tukipaatos_lists_success(self):
        ...
        # Timeframe which is active the whole year.
        TukipaatosAikavali.objects.create(alkamis_pvm='1900-05-01', paattymis_pvm='1900-06-30',
                                          tilastointi_pvm='1900-05-31')
    ...
    @mock_date_decorator_factory('varda.viewsets.datetime', '2020-03-15')
    def get_tukipaatos_lists_next_timeframe_success(self, client):
        resp = client.get('/api/v1/tukipaatokset/lists/')
        assert_status_code(resp, status.HTTP_200_OK)

        self.assertEqual(json.loads(resp.content)['next_aikavali']['alkamis_pvm'], '2020-05-01')
        self.assertEqual(json.loads(resp.content)['next_aikavali']['paattymis_pvm'], '2020-06-30')
        self.assertEqual(json.loads(resp.content)['next_aikavali']['tilastointi_pvm'], '2020-05-31')
