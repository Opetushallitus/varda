...
from varda.custom_signal_handlers import (
    cas_logout_handler,
    celery_setup_logging_signal_handler,
    celery_task_failure_signal_handler,
    celery_task_prerun_signal_handler,
    login_handler,
    logout_handler,
)
from varda.migrations.production.setup import (
    clear_old_permissions,
    create_extra_template_groups,
    create_henkilosto_template_groups,
    create_huoltajatiedot_template_groups,
    create_oph_luovutuspalvelu_group,
    create_paos_template_groups,
    create_raportit_template_groups,
    create_toimijatiedot_template_groups,
    load_initial_data,
)
from varda.migrations.testing.setup import (
    create_initial_koodisto_data,
    create_postinumero_koodisto_data,
    create_virhe_koodisto_data,
    create_yritysmuoto_koodisto_data,
    load_testing_data,
)
...
def run_post_migration_tasks(sender, **kwargs):
    if "plan" in kwargs:
        ...
        for migration_plan_tuple in kwargs["plan"]:
            ...
            if (
                migration_plan_tuple[0].app_label == "varda"
                and not migration_plan_tuple[1]
                and isinstance(migration_action_list, list)
            ):
...
def load_dev_testing_data():
    ...
    if env_type != "env-varda-prod" and not settings.ANONYMIZATION_ENV:
        load_testing_data()
    # Do load test data if env is production like (QA) and robot test related (hostname contains 'robot')
    elif settings.ROBOT_ENV:
        load_testing_data()
...
def tutkinto_tyontekija_id_lookup(instance):
    ...
    return (
        Tyontekija.objects.filter(vakajarjestaja=instance.vakajarjestaja, henkilo=instance.henkilo)
        .values_list("id", flat=True)
        .first()
    )


def update_related_object_change(
    instance,
    timestamp,
    history_type,
    model_name="",
    ...
    additional_values = (
        {"parent_model_name": parent_model_name, "parent_instance_id": parent_instance_id} if parent_instance_id else {}
    )

    Z9_RelatedObjectChanged.objects.create(
...
def update_related_object_change_henkilo(instance, timestamp, history_type):
    ...
    for lapsi in instance.lapsi.all():
        Z9_RelatedObjectChanged.objects.create(
            model_name=Lapsi.get_name(),
            instance_id=lapsi.id,
            changed_timestamp=timestamp,
            history_type=history_type,
            trigger_model_name=Lapsi.get_name(),
            trigger_instance_id=lapsi.id,
        )

    for tyontekija in instance.tyontekijat.all():
        Z9_RelatedObjectChanged.objects.create(
            model_name=Tyontekija.get_name(),
            instance_id=tyontekija.id,
            changed_timestamp=timestamp,
            history_type=history_type,
...
def handle_related_object_change(instance, history_type):
    ...
    receiver_dict = {
        Organisaatio.get_name(): partial(update_related_object_change, model_name=Organisaatio.get_name(), path_to_id=("id",)),
        Toimipaikka.get_name(): partial(
            update_related_object_change,
            model_name=Toimipaikka.get_name(),
            path_to_id=("id",),
            parent_model_name=Organisaatio.get_name(),
            path_to_parent_id=("vakajarjestaja_id",),
        ),
        ToiminnallinenPainotus.get_name(): partial(
            update_related_object_change,
            model_name=Toimipaikka.get_name(),
        ...
        KieliPainotus.get_name(): partial(
            update_related_object_change,
            model_name=Toimipaikka.get_name(),
            path_to_id=("toimipaikka_id",),
            parent_model_name=Organisaatio.get_name(),
        ...
        Varhaiskasvatuspaatos.get_name(): partial(
            update_related_object_change,
        ...
        Varhaiskasvatussuhde.get_name(): partial(
            update_related_object_change,
            model_name=Varhaiskasvatuspaatos.get_name(),
        ...
        Huoltajuussuhde.get_name(): partial(update_related_object_change, model_name=Lapsi.get_name(), path_to_id=("lapsi_id",)),
        Maksutieto.get_name(): update_related_object_change_maksutieto,
        MaksutietoHuoltajuussuhde.get_name(): partial(
            update_related_object_change,
            model_name=Lapsi.get_name(),
            path_to_id=(
                "huoltajuussuhde",
        ...
        Tyontekija.get_name(): partial(update_related_object_change, model_name=Tyontekija.get_name(), path_to_id=("id",)),
        Tutkinto.get_name(): partial(
            update_related_object_change, model_name=Tyontekija.get_name(), custom_id_lookup=tutkinto_tyontekija_id_lookup
        ),
        Palvelussuhde.get_name(): partial(
            update_related_object_change,
            model_name=Palvelussuhde.get_name(),
            path_to_id=("id",),
            parent_model_name=Tyontekija.get_name(),
            path_to_parent_id=("tyontekija_id",),
        ),
        Tyoskentelypaikka.get_name(): partial(
            update_related_object_change,
            model_name=Palvelussuhde.get_name(),
            path_to_id=("palvelussuhde_id",),
            parent_model_name=Tyontekija.get_name(),
            path_to_parent_id=(
                "palvelussuhde",
                "tyontekija_id",
        ...
        PidempiPoissaolo.get_name(): partial(
            update_related_object_change,
            model_name=Palvelussuhde.get_name(),
            path_to_id=("palvelussuhde_id",),
            parent_model_name=Tyontekija.get_name(),
        ...
        TaydennyskoulutusTyontekija.get_name(): partial(
            update_related_object_change,
            model_name=Tyontekija.get_name(),
        ...
        VuokrattuHenkilosto.get_name(): partial(
            update_related_object_change, model_name=Organisaatio.get_name(), path_to_id=("vakajarjestaja_id",)
        ),
    }
...
def receiver_save(**kwargs):
    ...
    if isinstance(instance, Lapsi):
        ...
        change_paos_tallentaja_organization_task.delay(
            instance.jarjestaja_kunta_organisaatio_id, instance.tuottaja_organisaatio_id
        )
...
class VardaConfig(AppConfig):
    ...
    def ready(self):
        ...
        task_failure.connect(celery_task_failure_signal_handler)
        # Connect to setup_logging signal to override default Celery logging
        # https://docs.celeryq.dev/en/stable/userguide/signals.html#setup-logging
        setup_logging.connect(celery_setup_logging_signal_handler)
        ...
        post_save.connect(receiver_save, sender="varda.VuokrattuHenkilosto")
        post_save.connect(receiver_save, sender="varda.Tukipaatos")
        ...
        pre_delete.connect(receiver_pre_delete, sender="varda.VuokrattuHenkilosto")
        pre_delete.connect(receiver_pre_delete, sender="varda.Tukipaatos")
        ...
        # Additional operations run on startup
        init_alive_log()
