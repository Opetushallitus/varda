...
def fetch_permissions_roles_for_organization(user_id, henkilo_oid, organisation, permission_group_list):
    ...
    role_vuokrattuhenkilosto = select_highest_kayttooikeusrooli(permission_group_list,
                                                                organization_oid,
    ...
    role_tukipaatos = select_highest_kayttooikeusrooli(permission_group_list,
                                                       organization_oid,
    ...
    roles = [role_vakatiedot, role_huoltajatiedot, role_paakayttaja, role_tyontekija, role_taydennyskoulutus,
             role_vuokrattuhenkilosto, role_tukipaatos, role_toimijatiedot, role_raportit]
...
def set_active_groups_for_user(user, organisaatio_obj=None):
    ...
    toimipaikka_oid_list = (Toimipaikka.objects.filter(vakajarjestaja=organisaatio_obj).exclude(organisaatio_oid='')
...
def get_all_paakayttaja_users():
    ...
    while True:
        ...
        """
        response = get_json_from_external_service(SERVICE_NAME, f'/kayttooikeus/kayttaja?palvelu=VARDA&offset={offset}',
                                                  large_query=True)
            ...
            for organisaatio in user_data['organisaatiot']:
                ...
                for permission in organisaatio['kayttooikeudet']:
                    if (permission['palvelu'] == 'VARDA' and
                            permission['oikeus'] == Z4_CasKayttoOikeudet.PAAKAYTTAJA and
                            (organisaatio_obj := organisaatio_qs.first())):
