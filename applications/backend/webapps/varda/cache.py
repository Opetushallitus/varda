...
def set_localisation_cache(category, locale, data, cached_time=settings.DEFAULT_CACHE_INVALIDATION_TIME):
    cache_data = {"data": data, "created": datetime.datetime.now()}
    # keys are hashed to prevent malformed keys in cache
    category = hash_string(category.lower())
    if locale:
...
def set_pulssi_cache(data):
    # Data cached for 23 hours
    cache.set("varda_pulssi", data, 60 * 60 * 23)
...
def get_queryset_count_cache_key(request):
    querydict = request.GET.copy()
    user_id = request.user.id

    # Remove obsolete query-params
    for param in ("cursor", "page", "page_size"):
        querydict.pop(param, None)

    # Remove keys with empty values
    querydict = {k: v for k, v in querydict.items() if str(v).strip()}

    query_string = urlencode(querydict, doseq=True)

    full_path_stripped = f"{request.path}?{query_string}" if query_string else request.path
    return create_cache_key(user_id, full_path_stripped)
