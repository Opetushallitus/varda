...
class ErrorMessages(enum.Enum):
    ...
    GE024 = get_error_dict("GE024", "This field must be an email address.")
    GE025 = get_error_dict("GE025", "This field must be a number.")
    GE026 = get_error_dict("GE026", "Check the datetime format.")
    GE027 = get_error_dict("GE027", "Request was throttled. Try again after a short while.")  # AWS WAF
    ...
    VJ001 = get_error_dict(
        "VJ001", "Cannot delete Vakajarjestaja. There are objects referencing it that need to be deleted first."
    )
    ...
    VJ010 = get_error_dict("VJ010", "Organisaatio is missing sahkopostiosoite.")
    ...
    TP005 = get_error_dict(
        "TP005", "It is not allowed to change the toimintamuoto_koodi of a Toimipaikka that has no organisaatio_oid."
    )
    ...
    TP020 = get_error_dict(
        "TP020", "varhaiskasvatuspaikat cannot be zero when Toimipaikka has related Varhaiskasvatussuhde objects that are active."
    )
    TP021 = get_error_dict(
        "TP021",
        "There are Varhaiskasvatussuhde objects linked to this Toimipaikka which are active longer than Toimipaikka itself.",
    )
    TP022 = get_error_dict(
        "TP022", "There are Tyoskentelypaikka objects linked to this Toimipaikka which are active longer than Toimipaikka itself."
    )
    ...
    TO001 = get_error_dict(
        "TO001", "ToiminnallinenPainotus with this toimintapainotus_koodi already exists for toimipaikka on the given date range."
    )
    ...
    TO004 = get_error_dict(
        "TO004", "toiminnallinenpainotus_kytkin should be true because Toimipaikka has ToiminnallinenPainotus objects."
    )
    TO005 = get_error_dict(
        "TO005", "toiminnallinenpainotus_kytkin should be false because Toimipaikka does not have ToiminnallinenPainotus objects."
    )

    # KieliPainotus errors, prefix: KP
    KP001 = get_error_dict(
        "KP001", "KieliPainotus with this kielipainotus_koodi already exists for toimipaikka on the given date range."
    )
    ...
    KP005 = get_error_dict(
        "KP005", "kielipainotus_kytkin should be false because Toimipaikka does not have KieliPainotus objects."
    )
    KP006 = get_error_dict("KP006", "No kielipainotustyyppi_koodi in Kielipainotus.")
    ...
    LA003 = get_error_dict(
        "LA003", "Cannot delete Lapsi. There are Maksutieto objects referencing it that need to be deleted first."
    )
    ...
    VP002 = get_error_dict(
        "VP002", "Varhaiskasvatussuhde alkamis_pvm must be equal to or after Varhaiskasvatuspaatos alkamis_pvm."
    )
    VP003 = get_error_dict(
        "VP003", "Varhaiskasvatuspaatos paattymis_pvm must be equal to or after Varhaiskasvatussuhde paattymis_pvm."
    )
    VP004 = get_error_dict(
        "VP004", "Cannot delete Varhaiskasvatuspaatos. There are objects referencing it that need to be deleted first."
    )
    ...
    VS011 = get_error_dict(
        "VS011", "Varhaiskasvatussuhde alkamis_pvm must be before or equal to Varhaiskasvatuspaatos paattymis_pvm."
    )
    VS012 = get_error_dict(
        "VS012", "Varhaiskasvatussuhde must have paattymis_pvm because Varhaiskasvatuspaatos has paattymis_pvm."
    )
    ...
    MA005 = get_error_dict(
        "MA005", "Maksutieto alkamis_pvm must be equal to or after earliest Varhaiskasvatuspaatos alkamis_pvm."
    )
    MA006 = get_error_dict(
        "MA006", "Maksutieto alkamis_pvm must be before or equal to latest Varhaiskasvatuspaatos paattymis_pvm."
    )
    MA007 = get_error_dict(
        "MA007", "Maksutieto paattymis_pvm must be before or equal to latest Varhaiskasvatuspaatos paattymis_pvm."
    )
    MA008 = get_error_dict(
        "MA008", "Lapsi does not have Varhaiskasvatuspaatos. Add Varhaiskasvatuspaatos before adding Maksutieto."
    )
    MA009 = get_error_dict(
        "MA009", "Lapsi does not have Varhaiskasvatussuhde. Add Varhaiskasvatussuhde before adding Maksutieto."
    )
    ...
    MA020 = get_error_dict("MA020", f"Maximum amount of asiakasmaksu is {MAXIMUM_ASIAKASMAKSU} for kunnallinen Lapsi.")
    MA021 = get_error_dict(
        "MA021", "Maksutieto palveluseteli_arvo is 0 for lapsi with varhaiskasvatuspäätös järjestämismuoto set to palveluseteli."
    )
    ...
    PO001 = get_error_dict(
        "PO001", "tallentaja_organisaatio must be either jarjestaja_kunta_organisaatio or tuottaja_organisaatio."
    )
    ...
    TY001 = get_error_dict(
        "TY001", "Cannot delete Tyontekija. There are Tutkinto objects referencing it that need to be deleted first."
    )
    ...
    TU001 = get_error_dict(
        "TU001", "Cannot delete Tutkinto. There are Palvelussuhde objects referencing it that need to be deleted first."
    )
    ...
    TU005 = get_error_dict("TU005", "Combination of henkilo, vakajarjestaja and tutkinto_koodi fields should be unique.")

    # Palvelussuhde errors, prefix: PS
    PS001 = get_error_dict(
        "PS001", "Cannot delete Palvelussuhde. There are objects referencing it that need to be deleted first."
    )
    ...
    PS009 = get_error_dict(
        "PS009",
        "There must be at least one active Tyoskentelypaikka or PidempiPoissaolo object related to an active Palvelussuhde.",
    )
    PS010 = get_error_dict("PS010", "Identical Palvelussuhde already exists.")
    PS011 = get_error_dict("PS011", "alkamis_pvm must be equal to or after 1950-01-01.")
    ...
    TA002 = get_error_dict(
        "TA002", "Cannot delete Tyoskentelypaikka. Taydennyskoulutus objects with this tehtavanimike_koodi must be deleted first."
    )
    TA003 = get_error_dict(
        "TA003", "Cannot delete Tyoskentelypaikka. There are objects referencing it that need to be deleted first."
    )
    ...
    TA015 = get_error_dict(
        "TA015", "Cannot change tehtavanimike_koodi. There are Taydennyskoulutus objects that use this tehtavanimike_koodi."
    )
    TA016 = get_error_dict("TA016", "Tyoskentelypaikka is active longer than related Toimipaikka.")

    # PidempiPoissaolo errors, prefix: PP
    PP001 = get_error_dict(
        "PP001", "Cannot delete PidempiPoissaolo. There are objects referencing it that need to be deleted first."
    )
    ...
    # VuokrattuHenkilosto errors, prefix: TH
    TH001 = get_error_dict("TH001", "VuokrattuHenkilosto already exists for this month.")
    ...
    TH007 = get_error_dict("TH007", "VuokrattuHenkilosto can be saved for either the current period or in the past.")
    TH008 = get_error_dict("TH008", "kuukausi must be equal to or after 2020-09-01.")
    TH009 = get_error_dict("TH009", "tuntimaara and tyontekijamaara must be 0 if VuokrattuHenkilosto is saved in advance.")

    # Taydennyskoulutus errors, prefix: TK
    TK001 = get_error_dict(
        "TK001", "Tyontekija not specified. Use (tyontekija), (henkilo_oid, vakajarjestaja_oid) or (lahdejarjestelma, tunniste)."
    )
    ...
    TK009 = get_error_dict(
        "TK009",
        "taydennyskoulutus_tyontekijat_add and taydennyskoulutus_tyontekijat_remove fields cannot be used "
    ...
    TK016 = get_error_dict(
        "TK016", "At least 1 valid Tyontekija is required (Tyontekija not found or tehtavanimike_koodi is incorrect)."
    )
    TK017 = get_error_dict(
        "TK017", "The combination of nimi, suoritus_pvm, and koulutuspaivia must be unique for each Tyontekija."
    )
    TK018 = get_error_dict("TK018", "suoritus_pvm must be equal to or before 2024-12-31.")

    # Tukipaatos errors, prefix: TT
    TT001 = get_error_dict("TT001", "Tukipaatos can only be created or modified within defined timeframes.")
    TT002 = get_error_dict("TT002", "There are currently no active timeframes.")
    TT003 = get_error_dict("TT003", "Tukipaatos cannot be modified after the timeframe of the reporting date.")
    TT004 = get_error_dict(
        "TT004",
        "The combination of vakajarjestaja_oid, yksityinen_jarjestaja, ikaryhma_koodi, tilastointi_pvm, tuentaso_koodi must be unique.",
    )
    ...
    # Luovutuspalvelu errors, prefix: LU
    LU001 = get_error_dict("LU001", "User does not have correct permissions.")
    LU002 = get_error_dict("LU002", "No file uploaded.")
    LU003 = get_error_dict("LU003", "Uploaded file is not PEM encoded CSR.")
    LU004 = get_error_dict("LU004", "CSR public key is not of type RSAPublicKey.")
    LU005 = get_error_dict("LU005", "CSR does not use SHA-2 family signature hash.")
    LU006 = get_error_dict("LU006", "CSR signature is not valid.")
    LU007 = get_error_dict("LU007", "CSR Subject C (Country) must be FI.")
    LU008 = get_error_dict("LU008", "CSR Subject CN is missing.")
    LU009 = get_error_dict("LU009", "CSR Subject CN is not matching with the Organization data.")
    LU010 = get_error_dict("LU010", "Error in AWS ACM PCA issue_certificate.")
    LU011 = get_error_dict("LU011", "System error. Please try again later.")
    LU012 = get_error_dict("LU012", "Invalid value in change_private_key.")
    ...
    MI022 = get_error_dict(
        "MI022", "Postinumero is invalid. Third value '0' not allowed and have to end with '0' or '5'. '00099' is allowed."
    )
