...
import { VardaVuokrattuHenkilostoDTO } from '../../utilities/models/dto/varda-vuokrattu-henkilosto-dto.model';
...
export class VardaHenkilostoApiService {
  ...
  // tyontekija
  getVakajarjestajaTyontekijat(
    vakajarjestajaId: number, tyontekijaSearchFilter: HenkiloSearchFilter
  ): Observable<VardaPageDto<HenkiloListDTO>> {
  ...
  getTaydennyskoulutuksetTyontekijat(searchParams: Record<string, unknown>): Observable<Array<VardaTaydennyskoulutusTyontekijaListDTO>> {
    return this.http.getAllResults(
      `${this.henkilostoApiPath}/taydennyskoulutukset/tyontekija-list/`,
  ...
  getVuokrattuHenkilostoByYear(year: number, vakajarjestaja_oid: string): Observable<VardaPageDto<VardaVuokrattuHenkilostoDTO>> {
    return this.http.get(`${this.henkilostoApiPath}/tilapainen-henkilosto/?vuosi=${year}&vakajarjestaja=${vakajarjestaja_oid}`);
  }

  saveVuokrattuHenkilostoByMonth(vuokrattuHenkilostoByMonth: Record<string, any>): Observable<VardaVuokrattuHenkilostoDTO> {
    if (vuokrattuHenkilostoByMonth.url) {
      return this.http.put(`${this.henkilostoApiPath}/tilapainen-henkilosto/${vuokrattuHenkilostoByMonth.id}/`, vuokrattuHenkilostoByMonth);
    } else {
      return this.http.post(`${this.henkilostoApiPath}/tilapainen-henkilosto/`, vuokrattuHenkilostoByMonth);
    }
  }
