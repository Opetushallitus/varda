...
export class VardaVakajarjestajaApiService {
  ...
  private toimipaikatApiPath = `${environment.vardaV2ApiUrl}/toimipaikat`;
  ...
  getToimipaikkaUrl(id) {
    return `/api/v2/toimipaikat/${id}/`;
  }

  getOrganisaatioUrl(id) {
   ...
   * initUserPermission will run setFilteredToimipaikat() using results. Consider if necessary.
   * */
  getToimipaikat(
    vakajarjestajaId: number,
    searchFilters?: Record<string, any>,
    initPermissions: boolean = true
  ): Observable<Array<VardaToimipaikkaMinimalDto>> {
    return this.http.getAllResults(
      ...
      catchError(() => of([])),
      tap(toimipaikat => {
        if (initPermissions) this.authService.initUserPermissions(toimipaikat);
      })
    );
  }
  ...
  setActiveOrganisaatio(organisaatioId: number): Observable<void> {
    return this.http.post(`${this.vardaUiPath}/active-organisaatio/`,
      { organisaatio: this.getOrganisaatioUrl(organisaatioId) });
  }
