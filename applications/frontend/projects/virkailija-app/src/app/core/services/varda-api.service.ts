...
import { LoadingHttpService, VardaUserDTO, VardaApiServiceInterface } from 'varda-shared';
...
export class VardaApiService implements VardaApiServiceInterface {
  ...
  getLogoutCasUrl(): string {
    const opintopolkuUrl = this.vardaUtilityService.getOpintopolkuUrl(window.location.hostname);
    const encodedOpintopolkuUrl = encodeURIComponent(opintopolkuUrl);
    return `${opintopolkuUrl}/cas/logout?service=${encodedOpintopolkuUrl}`;
  }
