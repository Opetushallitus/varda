...
export class VardaFormValidators {
  ...
  static validSSN(fc: FormControl) {
    ...
    const isValidSSN = ssnValue.match(/^[0-9]{6}[\-\+ABCDEFYXWVU]{1}[0-9]{3}[0-9ABCDEFHJKLMNPRSTUVWXY]$/);
  ...
  static validOppijanumero(fc: FormControl) {
    try {
      ...
      let oidLengthCorrect = false;
      let isHenkiloOid = false;

      const isProduction = window.location.hostname === 'virkailija.opintopolku.fi';
      if (isProduction) {
        oidLengthCorrect = oppijanumeroValue.length === 26;
        // Production: only .24. allowed
        isHenkiloOid = oppijanumeroValue.substring(11, 15) === '.24.';
      } else {
        oidLengthCorrect = (
          oppijanumeroValue.length === 26 ||
          oppijanumeroValue.length === 27
        );
        isHenkiloOid = (
          oppijanumeroValue.substring(11, 15) === '.24.' ||
          oppijanumeroValue.substring(11, 15) === '.98.' ||
          oppijanumeroValue.substring(11, 16) === '.198.' ||
          oppijanumeroValue.substring(11, 16) === '.298.'
        );
      }
