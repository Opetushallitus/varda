...
export class VardaDropdownFilterComponent<T> implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.trigger.menuClosed.subscribe({
      next: () => this.showDropdown = false,
      error: err => console.error(err),
    });


    this.trigger.menuOpened.subscribe({
      next: () => this.dropdownFilterInput ?
        setTimeout(() => this.dropdownFilterInput.nativeElement.focus(), 300) : null,
      error: err => console.error(err),
    });

    this.subscriptions.push(
      this.searchFieldChanged.pipe(debounceTime(500)).subscribe((enter: boolean) => this.filterList(this.filterText, enter))
    );
  ...
  filterList(text: string, enter = false) {
