...
export class PaosAddedToimijatComponent
  extends AbstractPaosListToimintainfoComponentDirective<PaosToimintatietoDto>
  ...
  pushToimintaOrganisaatioId = paosToiminta => (
    this.paosToimintaService.pushToimintaOrganisaatio(paosToiminta.vakajarjestaja_id, PaosCreateEvent.Toimija)
  );
  ...
  getPaosToiminnatOnCompleteHook() {
    this.paosToiminnat.sort(
      (a: PaosToimintatietoDto, b: PaosToimintatietoDto) => a.vakajarjestaja_nimi.localeCompare(b.vakajarjestaja_nimi, 'fi')
    );
