...
export class PaosAddedToimipaikatComponent
  extends AbstractPaosListToimintainfoComponentDirective<PaosToimipaikkatietoDto>
  ...
  pushToimintaOrganisaatioId = (paosToiminta: PaosToimipaikkatietoDto) => (
    this.paosToimintaService.pushToimintaOrganisaatio(paosToiminta.toimipaikka_id, PaosCreateEvent.Toimipaikka)
  );
  ...
  changeTallennusvastuu(tallennusvastuuId: string, paosToiminta: PaosToimijaInternalDto) {
    const newTallentaja = (
      parseInt(tallennusvastuuId, 10) === this.selectedVakajarjestaja.id ? this.selectedVakajarjestaja.id : paosToiminta.toimijaId
    );
  ...
  isTallentajaChanged(paosToiminta: PaosToimijaInternalDto) {
    return (
      !this.changedTallennusvastuu.newTallentaja ||
      this.changedTallennusvastuu.newTallentaja === paosToiminta.paosOikeus.tallentaja_organisaatio_id);
  }
