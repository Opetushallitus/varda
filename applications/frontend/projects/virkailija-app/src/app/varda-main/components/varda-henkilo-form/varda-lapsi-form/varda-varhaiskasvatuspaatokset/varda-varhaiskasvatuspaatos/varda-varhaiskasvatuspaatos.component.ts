...
export class VardaVarhaiskasvatuspaatosComponent extends VardaFormAccordionAbstractComponent<LapsiKoosteVakapaatos> implements OnInit {
  ...
  initForm() {
    this.formGroup = new FormGroup({
      ...
      hakemus_pvm: new FormControl(
        this.currentObject
          ? DateTime.fromFormat(this.currentObject.hakemus_pvm, VardaDateService.vardaApiDateFormat, VardaDateService.uiDateTimezone)
          : null,
        Validators.required
      ),
      ...
      tuntimaara_viikossa: new FormControl(
        this.currentObject?.tuntimaara_viikossa,
        [Validators.pattern('^\\d+([,.]\\d{1})?$'), Validators.min(1), Validators.max(120), Validators.required]
      ),
    });
  ...
  saveVarhaiskasvatuspaatos(form: FormGroup): void {
    ...
    if (VardaErrorMessageService.formIsValid(form)) {
      const varhaiskasvatuspaatosDTO: VardaVarhaiskasvatuspaatosDTO = {
        ...form.value,
        hakemus_pvm: form.value.hakemus_pvm.toFormat(VardaDateService.vardaApiDateFormat),
  ...
  hakemusDateChange(hakemusDate: DateTime) {
    this.minStartDate = hakemusDate?.toJSDate();
    this.minEndDate = this.minEndDate || hakemusDate?.toJSDate();
