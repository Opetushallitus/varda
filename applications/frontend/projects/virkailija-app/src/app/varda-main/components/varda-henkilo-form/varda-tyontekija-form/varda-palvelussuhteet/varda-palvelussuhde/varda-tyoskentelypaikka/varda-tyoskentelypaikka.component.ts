...
export class VardaTyoskentelypaikkaComponent extends VardaFormAccordionAbstractComponent<TyontekijaTyoskentelypaikka> implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.toimipaikkaAccess.tyontekijatiedot.tallentaja) {
      this.toimipaikat = this.authService.getAuthorizedToimipaikat(
        this.vardaVakajarjestajaService.getFilteredToimipaikat().tallentajaToimipaikat, SaveAccess.tyontekijatiedot
      );
  ...
  initForm() {
    this.formGroup = new FormGroup({
      ...
      alkamis_pvm: new FormControl(
        this.dateService.vardaDateToLuxon(this.currentObject?.alkamis_pvm),
        Validators.required
      ),
      paattymis_pvm: new FormControl(
        this.objectExists() ? this.dateService.vardaDateToLuxon(this.currentObject?.paattymis_pvm) 
        : this.dateService.vardaDateToLuxon(this.palvelussuhde.paattymis_pvm)
      ),
  ...
  startDateChange(startDate: DateTime) {
    this.endDateRange.min = startDate?.toJSDate() || new Date(this.palvelussuhde.alkamis_pvm);
