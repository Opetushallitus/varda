...
export class VardaToimipaikkaFormComponent extends VardaFormAccordionAbstractComponent<ToimipaikkaKooste> implements OnInit, OnDestroy {
  ...
  kielipainotustyyppikoodisto: Array<CodeDTO>;
  ...
  ngOnInit() {
    ...
    if (this.toimipaikka?.id) {
      ...
      this.tallentajaAccess = (
        this.tallentajaAccess || this.authService.getUserAccess(this.toimipaikka.organisaatio_oid).lapsitiedot.tallentaja
      );
    ...
    const isKunnallinen = this.vakajarjestajaService.getSelectedVakajarjestaja().kunnallinen_kytkin;
    const excludeJarjestamismuotoCodes = isKunnallinen ? ['jm04', 'jm05'] : ['jm01'];
    this.subscriptions.push(
      ...
      this.koodistoService.getKoodisto(KoodistoEnum.kielipainotustyyppi, KoodistoSortBy.name).subscribe(result =>
        this.kielipainotustyyppikoodisto = result.codes),
    );
  }
  ...
  initForm() {
    ...
    this.formGroup = new FormGroup({
      ...
      nimi: new FormControl(
        this.currentObject?.nimi,
        [
          Validators.required,
          Validators.minLength(3),
          Validators.maxLength(200),
          VardaFormValidators.hasCharacters(),
          VardaFormValidators.rejectSpecialChars]
      ),
      organisaatio_oid: new FormControl(this.currentObject?.organisaatio_oid),
      kayntiosoite: new FormControl(
        this.currentObject?.kayntiosoite,
        [
          Validators.required,
          Validators.minLength(3),
      ...
      postiosoite: new FormControl(
        this.currentObject?.postiosoite,
        [Validators.required,
          Validators.minLength(3),
          Validators.maxLength(100),
          VardaFormValidators.hasCharacters(),
      ...
      puhelinnumero: new FormControl(
        this.currentObject?.puhelinnumero,
        [Validators.required, VardaFormValidators.validStringFormat.bind(null, { regex: '^(\\+358)[1-9]\\d{5,10}$' })]
      ),
      sahkopostiosoite: new FormControl(
        this.currentObject?.sahkopostiosoite,
      ...
      jarjestamismuoto_koodi: new FormControl(
        this.currentObject?.jarjestamismuoto_koodi.map(koodi => koodi.toLocaleUpperCase()), [Validators.required]
      ),
      toimintakieli_koodi: new FormControl(
        this.currentObject?.toimintakieli_koodi.map(koodi => koodi.toLocaleUpperCase()), [Validators.required]
      ),
      kasvatusopillinen_jarjestelma_koodi: new FormControl(
        this.currentObject?.kasvatusopillinen_jarjestelma_koodi.toLocaleUpperCase() || 'KJ98', [Validators.required]
      ),
      varhaiskasvatuspaikat: new FormControl(
        this.currentObject?.varhaiskasvatuspaikat, [Validators.required, Validators.min(0)]
      ),
  ...
  saveToimipaikka(form: FormGroup) {
    ...
    if (VardaErrorMessageService.formIsValid(form)) {
      const toimipaikkaDTO: VardaToimipaikkaDTO = {
        ...
        paattymis_pvm: form.value.paattymis_pvm?.isValid ? form.value.paattymis_pvm.toFormat(VardaDateService.vardaApiDateFormat) : null
      };
  ...
  checkPostiosoiteToggle() {
    ...
    if (kayntiosoite && postiosoite && kayntiosoitePostitoimipaikka && postitoimipaikka && kayntiosoitePostinumero && postinumero) {
      this.postiosoiteToggleBoolean = (
        kayntiosoite === postiosoite &&
        kayntiosoitePostitoimipaikka === postitoimipaikka &&
        kayntiosoitePostinumero === postinumero
      );
    } else {
  ...
  startDateChange(startDate: DateTime) {
    this.minEndDate = startDate?.toJSDate();
    setTimeout(() => this.formGroup.controls.paattymis_pvm?.updateValueAndValidity(), 100);
  }
  
  endDateChange(endDate: DateTime) {
    this.maxEndDate = endDate?.toJSDate();
  }
