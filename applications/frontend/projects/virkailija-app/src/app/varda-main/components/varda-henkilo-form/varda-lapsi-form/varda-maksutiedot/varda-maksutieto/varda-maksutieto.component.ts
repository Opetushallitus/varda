...
export class VardaMaksutietoComponent extends VardaFormAccordionAbstractComponent<LapsiKoosteMaksutieto> implements OnInit {
  ...
  initForm() {
    this.formGroup = new FormGroup({
      ...
      palveluseteli_arvo: new FormControl(
        this.currentObject?.palveluseteli_arvo,
        [Validators.min(0), Validators.max(9999), Validators.pattern('^\\d+([,.]\\d{1,2})?$')]
      ),
      asiakasmaksu: new FormControl(
        this.currentObject?.asiakasmaksu,
        [Validators.required, Validators.min(0), Validators.max(9999), Validators.pattern('^\\d+([,.]\\d{1,2})?$')]
      ),
      perheen_koko: new FormControl(
        this.currentObject?.perheen_koko, this.yksityinenBoolean ? null : [Validators.required, Validators.min(2), Validators.max(50)]
      ),
  ...
  saveMaksutieto(form: FormGroup): void {
    ...
    if (this.huoltajat.invalid) {
      ...
      const maksutietoDTO: VardaMaksutietoSaveDTO = {
        ...form.value,
        alkamis_pvm: form.value.alkamis_pvm?.toFormat(VardaDateService.vardaApiDateFormat) || this.currentObject?.alkamis_pvm,
  ...
  startDateChange(startDate: DateTime) {
    this.minEndDate = startDate ? startDate.toJSDate() : null;
