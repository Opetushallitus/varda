...
export class VardaTaydennyskoulutusFormComponent implements OnInit {
  ...
  lastAllowedDate = VardaDateService.taydennyskoulutusLatestDate;
  ...
  initForm() {
    this.taydennyskoulutusForm = new FormGroup({
      ...
      lahdejarjestelma: new FormControl(
        this.taydennyskoulutus?.lahdejarjestelma || Lahdejarjestelma.kayttoliittyma
      ),
      nimi: new FormControl(this.taydennyskoulutus?.nimi, Validators.required),
      koulutuspaivia: new FormControl(
        this.taydennyskoulutus?.koulutuspaivia,
        [
          Validators.required,
          Validators.min(0.5),
          Validators.max(160),
          VardaFormValidators.remainderOf(0.5)
        ]
      ),
      suoritus_pvm: new FormControl(
        this.taydennyskoulutus?.suoritus_pvm
          ? DateTime.fromFormat(this.taydennyskoulutus.suoritus_pvm, VardaDateService.vardaApiDateFormat, VardaDateService.uiDateTimezone)
          : null,
        Validators.required
      ),
    });
    ...
    if (this.taydennyskoulutus?.taydennyskoulutus_tyontekijat) {
      ...
      this.limitedEditAccess = (
        this.taydennyskoulutus?.taydennyskoulutus_tyontekijat_count !== this.taydennyskoulutus?.taydennyskoulutus_tyontekijat.length
      );
    }
  ...
  saveTaydennyskoulutus(form: FormGroup) {
    ...
    if (VardaErrorMessageService.formIsValid(form) && this.checkRoster()) {
      ...
      const taydennyskoulutusJson: VardaTaydennyskoulutusSaveDTO = {
        ...form.value,
        suoritus_pvm: form.value.suoritus_pvm.toFormat(VardaDateService.vardaApiDateFormat)
      };
  ...
  fillTyontekijat(taydennyskoulutusJson: VardaTaydennyskoulutusSaveDTO): void {
    const toRemove = this.taydennyskoulutus.taydennyskoulutus_tyontekijat.filter(existingTyontekija =>
      !this.currentOsallistujat.find(
        osallistuja => (
          osallistuja.henkilo_oid === existingTyontekija.henkilo_oid &&
          osallistuja.tehtavanimike_koodi === existingTyontekija.tehtavanimike_koodi
        )
      )
