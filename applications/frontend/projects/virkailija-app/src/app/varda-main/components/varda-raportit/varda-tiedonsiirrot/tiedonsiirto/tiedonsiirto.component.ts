...
export class VardaTiedonsiirtoComponent extends AbstractTiedonsiirrotSectionsComponent implements OnChanges {
  ...
  openJSON(tiedonsiirto: VardaTiedonsiirtoDTO) {
    ...
    this.dialog.open(TiedonsiirtoDialogComponent, { data: tiedonsiirtoData });
  }
    ...
    const tiedonsiirrot = results.map(tiedonsiirto => {
      ...
      Object.entries(jsonResponse).forEach(([key, value]) => {
        if (Array.isArray(value)) {
          value?.forEach(
            error => tiedonsiirto.response_list.push({ field: key, error_code: error.error_code, expand: this.expandResponse })
          );
        }
      });
  ...
  resetTimestampFormGroup() {
    this.timestampFormGroup = new FormGroup({
      timestampAfter: new FormControl(DateTime.now()),
