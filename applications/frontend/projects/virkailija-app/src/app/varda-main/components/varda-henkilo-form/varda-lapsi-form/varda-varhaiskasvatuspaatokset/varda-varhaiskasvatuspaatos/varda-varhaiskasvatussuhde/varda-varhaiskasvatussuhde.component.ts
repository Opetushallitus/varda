...
export class VardaVarhaiskasvatussuhdeComponent extends VardaFormAccordionAbstractComponent<LapsiKoosteVakasuhde> implements OnInit {
  ...
  initForm() {
    this.formGroup = new FormGroup({
      ...
      varhaiskasvatuspaatos: new FormControl(
        this.lapsiService.getVarhaiskasvatuspaatosUrl(this.varhaiskasvatuspaatos.id), Validators.required
      ),
      ...
      paattymis_pvm: new FormControl(
        this.objectExists()
          ? this.dateService.vardaDateToLuxon(this.currentObject?.paattymis_pvm)
          : this.dateService.vardaDateToLuxon(this.varhaiskasvatuspaatos.paattymis_pvm),
        this.varhaiskasvatuspaatos.paattymis_pvm ? [Validators.required] : null
      ),
      toimipaikka_oid: new FormControl(
        this.currentObject?.toimipaikka_oid || this.henkilonToimipaikka?.organisaatio_oid, Validators.required
      ),
    });
  ...
  startDateChange(startDate: DateTime | null) {
    this.endDateRange.min = startDate
      ? startDate.toJSDate()
      : new Date(this.varhaiskasvatuspaatos.alkamis_pvm);
