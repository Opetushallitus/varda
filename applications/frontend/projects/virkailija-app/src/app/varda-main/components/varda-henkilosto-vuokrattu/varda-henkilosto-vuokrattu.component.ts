...
enum VuokrattuPrefix {
  spring = VirkailijaTranslations.vuokrattu_henkilosto_first_half,
  autumn = VirkailijaTranslations.vuokrattu_henkilosto_last_half
}

interface VuokrattuYearPicker {
  earliest: number;
  latest: number;
  year: number;
  prefix: VuokrattuPrefix;
}

@Component({
    selector: 'app-varda-henkilosto-vuokrattu',
    templateUrl: './varda-henkilosto-vuokrattu.component.html',
    styleUrls: ['./varda-henkilosto-vuokrattu.component.css'],
    standalone: false
})
export class VardaHenkilostoVuokrattuComponent implements OnInit {
  ...
  vuodet: Array<VuokrattuYearPicker> = [];
  vuosiArvot = { hours: 0, employees: 0, months: 0 };
  henkilostoVuosi: VuokrattuYearPicker;
  ...
  initiateYears(vakajarjestaja: VardaVakajarjestaja) {
    ...
    const vakajarjestajaStartDate = (
      vakajarjestaja.alkamis_pvm ? new Date(vakajarjestaja.alkamis_pvm) : VardaDateService.henkilostoReleaseDate
    );
    ...
    for (let i = earliestAllowedDate.getUTCFullYear(); i <= lastAllowedDate.getUTCFullYear(); ++i) {
      for (let j = 0; j < 2; j++) {
        ...
        const pickerData: VuokrattuYearPicker = {
          earliest: monthComparison,
          latest: monthComparison + 5,
          year: i,
          prefix: monthComparison < july ? VuokrattuPrefix.spring : VuokrattuPrefix.autumn
        };
  ...
  initiateMonths(vuosi: VuokrattuYearPicker) {
    ...
    if (!this.toimijaAccess.vuokrattuHenkilosto.tallentaja) {
      this.henkilostoKytkin.disabled = true;
    }

    this.henkilostoService.getVuokrattuHenkilostoByYear(vuosi.year, this.selectedVakajarjestaja.organisaatio_oid).subscribe({
      next: data => {
  ...
  saveHenkilosto(formArray: FormArray) {
    const requestsToSend = formArray.controls.filter(formGroup => formGroup.valid).map(formGroup => {
      if (!formGroup.pristine) {
        ...
        return this.henkilostoService.saveVuokrattuHenkilostoByMonth(formGroup.value);
      }
      return null;
    }).filter(Boolean);
    ...
    forkJoin(requestsToSend).pipe(catchError(err => {
      ...
      next: () => {
        this.initiateMonths(this.henkilostoVuosi);
        this.snackBarService.success(this.i18n.vuokrattu_henkilosto_save_success);
      },
      error: (err) => this.henkilostoErrorService.handleError(err, this.snackBarService)
    });
  }
