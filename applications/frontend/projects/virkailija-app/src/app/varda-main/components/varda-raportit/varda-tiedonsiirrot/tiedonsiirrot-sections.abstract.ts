...
export abstract class AbstractTiedonsiirrotSectionsComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  getSearchFilter(): Record<string, any> {
    const returnFilter: Record<string, any> = {
      page_size: 20,
      timestamp_after: this.timestampFormGroup.controls.timestampAfter.value.format(`${VardaDateService.vardaApiDateFormat}T00:00:00Z`),
      timestamp_before: this.timestampFormGroup.controls.timestampBefore.value.format(`${VardaDateService.vardaApiDateFormat}T23:59:59Z`)
    };
