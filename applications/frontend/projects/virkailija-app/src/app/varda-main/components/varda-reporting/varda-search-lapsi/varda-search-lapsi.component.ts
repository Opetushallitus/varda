...
export class VardaSearchLapsiComponent extends VardaSearchAbstractComponent implements OnInit {
  ...
  private savedFilters: {
    jarjestamismuoto: CodeDTO | null;
    toimintamuoto: CodeDTO | null;
  } = {
    jarjestamismuoto: null,
    toimintamuoto: null
  };
  ...
  search(paginatorParams?: PaginatorParams): void {
    ...
    // Handle temporary filter clearing when using maksutiedot-rajaus
    if (this.isRajausMaksutiedot()) {
      this.isAllToimipaikatSelected = true;
      this.saveFilters2();
    } else {
      if (this.selectedToimipaikat.length > 0 && this.isAllToimipaikatSelected) {
        this.isAllToimipaikatSelected = false;
      }
      this.restoreFilters2();
    }
  ...
  isRajausMaksutiedot(): boolean {
    return this.filterParams.rajaus === this.rajaus.MAKSUTIEDOT;
  }
  ...
  clearFilters2() {
    ...
    this.savedFilters.jarjestamismuoto = null;
    this.savedFilters.toimintamuoto = null;
  }
  ...
  private saveFilters2(): void {
    if (this.filterParams.jarjestamismuoto) {
      this.savedFilters.jarjestamismuoto = this.filterParams.jarjestamismuoto;
      this.filterParams.jarjestamismuoto = null;
    }
    if (this.filterParams.toimintamuoto) {
      this.savedFilters.toimintamuoto = this.filterParams.toimintamuoto;
      this.filterParams.toimintamuoto = null;
    }
  }

  private restoreFilters2(): void {
    if (this.savedFilters.jarjestamismuoto) {
      this.filterParams.jarjestamismuoto = this.savedFilters.jarjestamismuoto;
      this.savedFilters.jarjestamismuoto = null;
    }
    if (this.savedFilters.toimintamuoto) {
      this.filterParams.toimintamuoto = this.savedFilters.toimintamuoto;
      this.savedFilters.toimintamuoto = null;
    }
  }
}
