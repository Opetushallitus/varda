...
export class PalvelussuhdeComponent implements OnInit {
  ...
  constructor(private translateService: TranslateService) {
    this.translateService.get(
      this.i18n.tyoskentelypaikka_kiertava_kytkin).subscribe(translation => this.kiertavaTyontekijaText = translation
    );
  }

  ngOnInit(): void {
    const tyoskentelypaikat = new Set(
      this.palvelussuhde.tyoskentelypaikat.map(tyoskentelypaikka => tyoskentelypaikka.toimipaikka_nimi || this.kiertavaTyontekijaText)
    );
