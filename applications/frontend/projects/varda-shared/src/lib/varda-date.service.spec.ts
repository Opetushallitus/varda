...
describe('VardaDateService', () => {
  ...
  it('Should convert Luxon object to vardaApiDate-style date YYYY-MM-DD', inject([VardaDateService],
    (service: VardaDateService) => {
      const date = DateTime.fromFormat('2009-07-15', VardaDateService.vardaApiDateFormat);
      const vardaApiDate = service.luxonToVardaDate(date);
      expect(vardaApiDate).toEqual('2009-07-15');
    }
  ));

  it('Should convert vardaApiDate-style date YYYY-MM-DD to Luxon object', inject([VardaDateService],
    (service: VardaDateService) => {
      const datepickerDateObj = service.vardaDateToLuxon('2009-07-15');
      expect(datepickerDateObj.year).toEqual(2009);
      expect(datepickerDateObj.month).toEqual(7);
      expect(datepickerDateObj.day).toEqual(15);
    }
  ));
