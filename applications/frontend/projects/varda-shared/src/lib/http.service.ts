...
export abstract class AbstractHttpService {
  ...
  httpRetry(maxRetry = 10, minDelayMs = 1000, maxDelayMs = 2000) {
...
export class HttpService extends AbstractHttpService {
  ...
  put(url: string, formData: any, options?: any): Observable<any> {
    const authToken = ` Token ${this.apiKey}`;
    const httpHeaders = new HttpHeaders({ Authorization: authToken });
    return this.http.put(url, formData, { headers: httpHeaders })
      .pipe(
        this.httpRetry(3),
        map((resp: any) => resp),
        catchError((e) => observableThrowError(e))
  ...
  patch(url: string, formData: any, options?: any): Observable<any> {
    const authToken = ` Token ${this.apiKey}`;
    const httpHeaders = new HttpHeaders({ Authorization: authToken });
    return this.http.patch(url, formData, { headers: httpHeaders })
      .pipe(
        this.httpRetry(3),
        map((resp: any) => resp),
        catchError((e) => observableThrowError(e))
  ...
  options(url: string, options?: any): Observable<any> {
    const authToken = ` Token ${this.apiKey}`;
    const httpHeaders = new HttpHeaders({ Authorization: authToken });
    return this.http.options(url, { headers: httpHeaders })
      .pipe(
        this.httpRetry(3),
        map((resp: any) => resp),
        catchError((e) => observableThrowError(e)));
