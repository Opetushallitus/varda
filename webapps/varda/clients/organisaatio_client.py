...
def get_organisaatiopalvelu_info(y_tunnus):
    url = settings.OPINTOPOLKU_DOMAIN + '/' + SERVICE_NAME + ORGANISAATIOPALVELU_API_V4 + y_tunnus
...
def get_y_tunnus(organisaatio_oid):
    ...
    if "ytunnus" not in organization_data:
        logger.error('Problem with ytunnus: /' + SERVICE_NAME + organisaatio_url)
        return None
    else:
        return organization_data["ytunnus"]
