...
elif [[ $VARDA_ENVIRONMENT_TYPE == *"pulssi"* ]]; then
  sed -i "s/VARDA_FRONTEND_HOSTNAME/$VARDA_FRONTEND_HOSTNAME/g" /usr/share/nginx/html/varda/pulssi/main.*.js;
  sed -i "s/VARDA_BACKEND_HOSTNAME/$VARDA_BACKEND_HOSTNAME/g" /usr/share/nginx/html/varda/pulssi/main.*.js
else
