...
export class HakuListComponent implements OnInit, OnChanges {
  ...
  editHenkilo(result: HenkiloHakuResultWithToimipaikka) {
    ...
    const toimipaikka = this.vardaVakajarjestajaService.getVakajarjestajaToimipaikat().allToimipaikat
      .find(_toimipaikka => _toimipaikka.organisaatio_oid === result.toimipaikka_oid);
